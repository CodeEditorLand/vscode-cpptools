...
const htmlFilesPatterns = ["ui/**/*.html", "Reinstalling the Extension.md"];
...
const walkthroughHtmlFilesPatterns = ["walkthrough/**/*.md"];
...
const traverseHtml = (
	...
	const htmlTree = isFragment
		? parse5.parseFragment(contents)
		: parse5.parse(contents);
	traverse(htmlTree, {
		pre(node, parent) {
			if (node.attrs) {
				// Check if content text should be localized based on presense of data-loc-id attribute
				let locId = node.attrs.find(
					(attribute) =>
						attribute.name.toLowerCase() == dataLocIdAttribute,
				);
				if (locId) {
					let locHint = node.attrs.find(
						(attribute) =>
							attribute.name.toLowerCase() ==
							dataLocHintAttribute,
					);
				...
				node.attrs.forEach((attribute) => {
					if (attribute.name.startsWith(`${dataLocIdAttribute}-`)) {
						let targetAttributeName = attribute.name.substring(
							dataLocIdAttribute.length + 1,
						);
						let targetAttribute = node.attrs.find(
							(a) => a.name == targetAttributeName,
						);
						if (targetAttribute) {
							let hint = node.attrs.find(
								(a) =>
									a.name.toLowerCase() ==
									`${dataLocHintAttribute}-${targetAttributeName}`,
							);
							attributeCallback(
								attribute.value,
								hint?.value,
...
const processHtmlFiles = () => {
	return es.through(function (file) {
		...
		let nodeCallback = (locId, locHint, node) => {
			...
			localizationMetadataContents.keys.push(
				locHint ? { key: locId, comment: [locHint] } : locId,
			);
...
gulp.task("translations-export", (done) => {
	...
	let htmlStream = gulp
		.src([...htmlFilesPatterns, ...walkthroughHtmlFilesPatterns])
...
const generateLocalizedHtmlFilesImpl = (
	file,
	relativePath,
	language,
	isFragment,
	...
	let nodeCallback = (locId, locHint, node) => {
		...
		if (locString) {
			let nonTextChildNodes = node.childNodes.filter(
...
const generateLocalizedHtmlFiles = () => {
	return es.through(function (file) {
		...
		languages.map((language) => {
			let newContent = generateLocalizedHtmlFilesImpl(
				file,
				relativePath,
				language,
				false,
...
const generateWalkthroughHtmlLoc = () => {
	return gulp
		.src(walkthroughHtmlFilesPatterns)
			...
			let descriptionCallback = (path, value, parent) => {
				if (stringTable[keyPrefix + path]) {
					if (!parent.markdownDescription)
						...
						parent.markdownDescription =
							stringTable[keyPrefix + path];
				}
			};
			traverseJson(jsonTree, descriptionCallback, "");
...
gulp.task(
	...
	gulp.series(
		...
		generateAdditionalLocFiles,
		generateHtmlLoc,
		generateWalkthroughHtmlLoc,
