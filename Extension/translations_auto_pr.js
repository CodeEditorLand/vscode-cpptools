...
if (
	...
	console.error(
		`ERROR: Usage: ${path.parse(process.argv[0]).base} ${path.parse(process.argv[1]).base} repo_owner repo_name auth_token user_full_name user_email loc_root_path loc_sub_path`,
	);
	console.error(
		`   repo_owner - The owner of the repo on GitHub.  i.e. microsoft`,
	);
	console.error(
		`   repo_name - The name of the repo on GitHub.  i.e. vscode-cpptools`,
	);
	console.error(
		`   auth_user - User account with permission to post a pull request against the GitHub repo.`,
	);
