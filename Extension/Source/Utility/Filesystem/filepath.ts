...
export const normalize = isWindows
	? (p: string) =>
			norm(p)
				.toLowerCase()
...
export function pathsFromVariable(
...
export class filepath {
	static async stats(
		...
		baseFolder?: string,
	): Promise<[string, Stats | undefined] | [undefined, undefined]> {
		if (is.promise(name)) {
			name = await name;
		}
	...
	static async isFile(
		name: any | string | undefined | Promise<string | undefined>,
		baseFolder?: string,
	...
	static parent(
		...
		return is.promise(name)
			? name.then(filepath.parent)
			: name
				? normalize(resolve(name, ".."))
				: undefined;
	}
}
