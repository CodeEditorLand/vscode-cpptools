...
const cache = new Map<
	string,
	File | FolderWithChildren | Promise<FolderWithChildren | undefined>
>();
...
async function readDirectory(
	fullPath: string,
	executableExtensions: Set<string> = process.platform === "win32"
		? new Set([".exe" /* ,'.cmd','.bat' */])
		: new Set(),
): Promise<Map<string, File | FolderWithChildren> | undefined> {
	...
	if (!folder.children) {
		...
		await foreach(
			readdir(fullPath, { withFileTypes: true }).catch(returns.none),
			async (direntry: Dirent) => {
				...
				if (entry.isFile) {
					if (isWindows) {
						...
						const stats = await stat(entry.fullPath).catch(
							returns.undefined,
						);
...
export async function scanFolder(
	folder: string,
	scanDepth: number,
	filePredicate?: (file: File) => Promise<boolean> | boolean,
	folderPredicate?: (
		folder: FolderWithChildren,
	) => Promise<boolean> | boolean,
	files = accumulator<string>(),
): Promise<void> {
	...
	await foreach(readDirectory(folder), async ([_name, entry]) => {
		...
		if (
			scanDepth &&
			entry.isFolder &&
			(!folderPredicate || (await folderPredicate(entry)))
		) {
			await scanFolder(
				entry.fullPath,
				scanDepth - 1,
...
export class Finder implements AsyncIterable<string> {
	#excludedFolders = new Set<string | RegExp>([
		"winsxs",
		"syswow64",
		"system32",
	]);
	...
	constructor(
		binary: string | RegExp | ((file: File) => Promise<boolean> | boolean),
	) {
		switch (typeof binary) {
			case "string":
				this.match = (file: File) =>
					file.isExecutable && file.basename === binary;
			...
			case "object":
				this.match = (file: File) =>
					file.isExecutable && !!file.basename.match(binary);
		...
		this.promises.push(
			...location.map((each) =>
				scanFolder(
					...
					depth,
					this.match,
					(f) => !this.#excludedFolders.has(f.name),
					this.#files,
				),
			),
		);
