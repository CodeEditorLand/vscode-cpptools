...
export class FastFinder implements AsyncIterable<string> {
	...
	constructor(
		...
		options?: { executable?: boolean; executableExtensions?: string[] },
	) {
		...
		if (this.keepOnlyExecutables && process.platform === "win32") {
			this.executableExtensions = options?.executableExtensions ?? [
				".exe",
				".bat",
				".cmd",
				".ps1",
	...
	scan(...location: (string | number)[]): FastFinder {
		const depth =
			(typeof location[0] === "number"
				? (location.shift() as number)
				: 0) + 1;

		const globs = this.executableExtensions.length
			? this.fileGlobs
					.map((glob) =>
						this.executableExtensions.map((ext) =>
							glob.includes("**") ? glob : `**/${glob}${ext}`,
						),
					)
					.flat()
			: this.fileGlobs.map((glob) =>
		...
		if (globs.length && location.length) {
			...
			void ripgrep!(
				...
				"-L",
				"--files",
				...location.map((each) => each.toString()),
			)
				.then(async (proc) => {
					...
					for await (const line of process.stdio) {
						...
						if (
							!this.keepOnlyExecutables ||
							(await filepath.isExecutable(line))
						) {
							this.#files.add(line);
						}
					}
