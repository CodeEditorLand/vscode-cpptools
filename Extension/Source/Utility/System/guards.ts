...
export class is {
	...
	static iterable<T = unknown>(instance: any): instance is Iterable<T> {
		return (
			!(is.nullish(instance) || is.string(instance)) &&
	...
	static Constructor(instance: any): instance is Constructor {
		return (
			...
			!!instance.prototype &&
			!Object.getOwnPropertyNames(instance).includes("arguments") &&
			instance
				.toString()
				.match(/^function.*\{ \[native code\] \}|^class/g)
		);
	}
