...
export abstract class Emitter {
	...
	protected newEvent<TData, TOutput>(
		eventName: string,
		options?: EventOptions,
	): (input: TData) => Promise<TOutput | EventStatus>;
	...
	protected newEvent<TText extends string, TData, TOutput>(
		...
		const descriptors = options?.descriptors
			? new Descriptors(this.descriptors, options.descriptors)
			: this.descriptors;
	...
	protected newNotification<TData>(
		eventName: string,
		options?: EventOptions,
	): (input: TData) => void;
	...
	subscribe<T extends Record<string, any>>(
		...
		if (subscribers.length === 0) {
			return (
				subscribe(this as unknown as Subscription, {
					eventSource: this,
				}) && undefined
			);
		}
		if (typeof subscribers[0] === "string") {
			return Promise.all(
				subscribers.map((each) =>
					subscribe(each as string, { eventSource: this }),
				),
	...
	wait(eventExpression: string, _timeout?: number): Promise<void> {
		return new Promise<void>((e: () => void) =>
			this.on(eventExpression, e),
		);
	}
}
