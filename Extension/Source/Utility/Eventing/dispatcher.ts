...
async function dispatch<TResult>(event: Event<any, TResult>): Promise<void> {
	...
	if (!is.promise(event.completed)) {
		...
		for (const [callback, captures] of getHandlers(event, asyncHandlers)) {
			...
			try {
				void callback(event as EventData, ...captures);
			} catch (e: any) {
				console.error(e);
				/* ignore */
			}
			...
			results.push(
				is.promise(r)
					? r
							...
							.finally(() => current.delete(callback))
					: r,
			);
		} catch (e: any) {
	...
	return event.completed.resolve(
		(await Promise.all(results)).find((each: any) => each !== Continue),
	);
}

function* getHandlers<TResult>(
	event: Event<any, TResult>,
	category: Map<string, Subscriber[]>,
): Iterable<[Callback, string[]]> {
	if (category.size === 0) {
		return;
	}

	loop: for (const subscriber of [
		...(category.get(event.name) || []),
		...(category.get("*") || []),
	]) {
		...
		if (
			...
			subscriber.eventSource.deref() !== event.source
		) {
			continue;
		}
...
export function on<T>(
	...
	callback: Callback<T>,
	eventSrc?: ArbitraryObject,
): Unsubscribe {
	// parse the event expression into a chain of checks
	const [isSync, once, filters, eventSource] = parse(
		triggerExpression,
		eventSrc,
	);
...
export function subscribe<T extends Record<string, any>>(
	...
	if (options.folder) {
		...
		return (async () => {
			// this has to be async - we may be pulling data from a file...
			...
			for (const [name, _type] of [...properties, ...fields]) {
				...
				try {
					const filename = await filepath.isFile(
						text,
						options.folder,
					);
					...
					if (filename) {
						...
						const fn = sandbox.createFunction(code, ["event"], {
							filename,
						});
					...
					const fn = sandbox.createFunction(text, ["event"], {
						filename: `launch.json/${name}`,
					});
...
function expandVariableArgs<TInput = any, TResult = void>(
	variableArgs: any[],
	event: Event<TInput, TResult>,
): Event<TInput, TResult> {
...
function isSubscribed(name: string) {
	return (
		syncHandlers.has(name) ||
		asyncHandlers.has(name) ||
		asyncHandlers.has("*") ||
		syncHandlers.has("*")
	);
}
...
export async function emit<TResult>(
	...
	if (isSubscribed(name)) {
		const descriptors =
			variableArgs[0] instanceof Descriptors
				? variableArgs.shift()
				: Descriptors.none;
		...
		queue.push(
			expandVariableArgs(variableArgs, {
				name,
				variableArgs,
				descriptors,
				completed: result,
