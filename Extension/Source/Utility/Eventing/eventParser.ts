...
export function parse(
	...
	sourceToBindTo: ArbitraryObject | undefined,
): [boolean, boolean, Map<string, Filter | true>, ArbitraryObject?] {
	//
...
function generateFilterFn(scanner: Scanner): Filter {
	...
	const inner = [
		...scanner.takeUntil(Kind.CloseBracket, {
					...
					while (inner.length) {
						...
						switch (token.kind) {
							case Kind.Slash:
								...
								expression.push(
									`(!!$strings.find( ($text)=> { const r = ${rxExpression.map((token) => token.text).join("")}.exec($text); if( r ) { $captures.push( ...r ); return true; } } ))`,
								);
						...
						if (!inner.length) {
							throw new Error(
								`unterminated regular expression ${inner}`,
							);
						}
					}
				}
