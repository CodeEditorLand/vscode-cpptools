...
export const searchPaths = lazy(() =>
	filterToFolders(pathsFromVariable("PATH")),
);
...
function options<T extends Record<string, any>>(
	args?: [...Primitive[]] | [...Primitive[], T],
): Partial<T> {
	return args?.length
		? is.primitive(args[args.length - 1])
			? {}
			: (args.pop() as T)
		: {};
}
...
export interface ProcessFunction extends Launcher {
	(
		...commandLineArguments: ArrayPlusOptions<Primitive, ProgramOptions>
	): Promise<Process>;
}
...
async function processFactory(
	executable: string | Launcher,
	...initialArgs: [...Primitive[], ProgramOptions & { noninteractive: true }]
): Promise<CommandFunction>;
async function processFactory(
	executable: string | Launcher,
	...initialArgs: ArrayPlusOptions<Primitive, ProgramOptions>
): Promise<ProcessFunction>;
async function processFactory(
	executable: string | Launcher,
	...initialArgs: ArrayPlusOptions<Primitive, ProgramOptions>
): Promise<ProcessFunction | CommandFunction> {
	...
	if (typeof executable === "string") {
		fullPath = lazy<string>(async () => {
			if (!(await filepath.isExecutable(executable))) {
				...
				opts.choices ??= lazy(
					async () =>
						new Finder(executable).scan(...(await searchPaths))
							.results,
				);
				// but before we look at any of that, let's see if someone else wants to take that off our hands
				const bin = await emitNow<string>(
					"select-binary",
					Descriptors.none,
					executable,
					is.promise(opts.choices) ? await opts.choices : new Set(),
				);

				return (
					(await filepath.isExecutable(bin)) || // we have a good one coming back from the event
					(await filepath.isExecutable(first(opts.choices))) || // we're gonna pick the first one in the choices, if there are any.
					fail(
						new Error(
							`Unable to find full path to binary '${executable}'`,
						),
					)
	...
	if (opts.noninteractive) {
		...
		result = (async (
			...moreArgs: ArrayPlusOptions<Primitive, ProgramOptions>
		): Promise<CommandResult> => {
...
export const Command = Factory(
	(
		executable: string | Launcher,
		...initialArgs: ArrayPlusOptions<Primitive, ProgramOptions>
	) =>
		processFactory(executable, ...primitives(initialArgs), {
			...options(initialArgs),
			noninteractive: true,
		}),
);
