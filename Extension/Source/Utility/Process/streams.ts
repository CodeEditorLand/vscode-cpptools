	...
	implements AsyncIterable<string>, AsyncIterator<string>
{
	#current = 0;
	constructor(
		private lineBuffer: ReadableLineStream,
		private initial: number,
		private stopExpression?: string | RegExp,
	) {
		...
		for (let line of incoming) {
			...
			if (this.streamRead) {
				// if we have a reading event, then we queue it up
					...
					this.streamRead(line).then((text) =>
						!is.cancelled(text)
							? this.push(text !== undefined ? text : line)
							: undefined,
					),
				);
			} else {
	...
	async write(...text: string[]): Promise<void> {
		...
		return new Promise((resolve, reject) => {
			...
			this.writeable.write(
				this.#encoder.encode(content.join("")),
