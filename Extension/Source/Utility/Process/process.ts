...
export class Process extends Async(
	class Process extends ProcessEvents {
		...
		started = this.newNotification(notifications.started, {
			now: true,
			once: true,
		});

		/** Event signals when the process has stopped */
		exited = this.newNotification<number>(notifications.exited, {
			now: true,
			once: true,
		});
		...
		constructor(
			...
			const process = (this.#process = spawn(
				...
				.on("close", (code: number, signal: NodeJS.Signals) => {
					...
					verbose(
						`Ending   '${this.name}' ${args.map((each) => each.toString()).join(" ")} // exiting with code ${code}. in ${Date.now() - startTime}ms}`,
					);
			...
			this.stdio.setReadNotifier(
				this.newNotification<string>(notifications.read, {
