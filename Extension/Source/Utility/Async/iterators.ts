...
export async function foreach<T, TResult>(
	items:
		| undefined
		| Iterable<T>
		| Promise<Iterable<T> | undefined>
		| Promise<undefined>
		| AsyncIterable<T>
		| Promise<AsyncIterable<T>>,
	predicate: (item: T) => Promise<TResult>,
): Promise<TResult[]> {
			...
			return {
				async next() {
					...
					if (!is.promise(nextElement)) {
						nextElement = iterable[Symbol.asyncIterator]()
							.next()
							.then((element) => {
...
export function accumulator<T>(...iterables: Some<T>[]): AsynchIterable<T> {
	...
	result.add = (
		...
		for (const iterable of iterables) {
			iterators.set(
				iterators.size + 1,
				awaitNext({
					identity: iterators.size + 1,
					iterator: asyncOf(iterable)[Symbol.asyncIterator](),
				}),
			);
		}
		signal.resolve(true);
	...
	async function* combiner(iterables: Some<T>[]) {
		...
		result.add = () => {
			throw new Error("AsyncIterable is finished");
		};
	}
}
...
export type Some<T> =
	...
	| AsyncIterable<Promise<T> | Promise<undefined>>
	| Iterable<T>
	| Iterable<Promise<T>>;
...
export function race(promises: Promise<any>[]): Promise<any> {
	const addMisbehavingPromise: <T>(p: Promise<T>) => Promise<T> = (
		global as any
	).addMisbehavingPromise;
	return addMisbehavingPromise
		? addMisbehavingPromise(Promise.race(promises))
