...
type WithInit<TOutput> = TOutput & {
	init?(...args: any[]): any | Promise<any>;
};

export function Factory<
	TFactory extends (...args: Parameters<TFactory>) => ReturnType<TFactory>,
>(
	factory: TFactory,
): new (
	...
	async function initialize(
		instance:
			| WithInit<ReturnType<TFactory>>
			| Promise<WithInit<ReturnType<TFactory>>>,
		args: any[],
	) {
		...
		constructor(...args: any[]) {
			...
			super((resolve: Resolve<ReturnType<TFactory>>, reject: Reject) => {
				try {
					...
					initialize(factory(...(args as any)) as any, args)
						.then(resolve)
						.catch(reject);
				} catch (error) {
	...
	return AsyncFactory as any as new (
		...args: Parameters<TFactory>
	) => Promise<Awaited<ReturnType<TFactory>>>;
}
