...
export const messages = {
	...
	HexDigitExpected: {
		code: 1101,
		category: MessageCategory.Error,
		text: "Hex Digit expected (0-F,0-f)",
	},
	BinaryDigitExpected: {
		code: 1102,
		category: MessageCategory.Error,
...
export function format(text: string, ...args: (string | number)[]): string {
	return text.replace(
		/{(\d+)}/g,
		(_match, index: string) => "" + args[+index] || "<ARGMISSING>",
	);
}
...
export class Scanner implements Token {
		...
		if (!(this.eof || isNaN(this.#ch))) {
			switch (this.#ch) {
				case CharacterCodes.carriageReturn:
					return this.newLine(
						this.#chNext === CharacterCodes.lineFeed ? 2 : 1,
					);
				...
				case CharacterCodes.$:
					return isIdentifierPart(this.#chNext)
						? this.scanVariable()
						: this.next(Kind.Dollar);
				...
				case CharacterCodes.caret:
					return this.#chNext === CharacterCodes.equals
						? this.next(Kind.CaretEquals, 2)
						: this.next(Kind.Caret);

				case CharacterCodes.percent:
					return this.#chNext === CharacterCodes.equals
						? this.next(Kind.PercentEquals, 2)
						: this.next(Kind.Percent);

				case CharacterCodes.backslash:
					return this.next(Kind.Backslash);

				case CharacterCodes.question:
					return this.#chNext === CharacterCodes.dot &&
						!isDigit(this.#chNextNext)
				...
				case CharacterCodes.dot:
					return isDigit(this.#chNext)
						? this.scanNumber()
						: this.#chNext === CharacterCodes.dot &&
							  this.#chNextNext === CharacterCodes.dot
				...
				case CharacterCodes.backtick:
					return this.#column === 0 &&
						this.#chNext === CharacterCodes.backtick &&
						this.#chNextNext === CharacterCodes.backtick
						? this.scanCodeFence()
						: this.scanString();
				...
				default:
					...
					return isIdentifierStart(this.#ch)
						? this.scanIdentifier()
						: this.next(Kind.Unknown);
			}
		}
	...
	*takeUntil(
		endToken: Kind,
		options?: { escape?: Kind[]; nestable?: [Kind, Kind][] },
		yieldFinalClose?: boolean,
	): Iterable<Token> {
	...
	private isConflictMarker() {
		...
		if (
			this.#offset === 0 ||
			isLineBreak(this.#text.charCodeAt(this.#offset - 1))
		) {
			if (this.#offset + mergeConflictMarkerLength < this.#length) {
				...
				return (
					...
					this.#text.charCodeAt(
						this.#offset + mergeConflictMarkerLength,
	...
	private scanBinaryNumber() {
		this.assert(
			isBinaryDigit(this.#chNextNext),
			"ParseError: Binary Digit expected (0,1)",
		);
	...
	private get widthOfCh() {
		return this.#ch === CharacterCodes.tab
			? this.#column % this.tabWidth || this.tabWidth
			: 1;
	}
	...
	private scanUntil(
		...
		do {
			...
			if (isLineBreak(this.#ch)) {
				this.advance(
					this.#ch === CharacterCodes.carriageReturn &&
						this.#chNext === CharacterCodes.lineFeed
						? 2
						: 1,
				);
				this.#line++;
			...
			if (this.eof) {
				this.assert(
					!expectedClose,
					`Unexpected end of file while searching for '${expectedClose}'`,
				);
				break;
			}
	...
	private scanMultiLineHashComment() {
		this.text = this.scanUntil(
			(ch, chNext) =>
				...
				chNext === CharacterCodes.greaterThan,
			"#>",
			2,
		);
	...
	private scanString() {
		...
		const text = this.scanUntil(
			(ch, chNext, _chNextNext) => {
				if (isEscaping) {
					isEscaping = false;
					return false;
				}
	...
	private scanCodeFence() {
		...
		const fenceStart = this.scanUntil(
			(ch, chNext) =>
				ch === CharacterCodes.lineFeed ||
				(ch === CharacterCodes.carriageReturn &&
					chNext === CharacterCodes.lineFeed),
		);

		// end when the first column is triple backtick
		const code = this.scanUntil(
			(ch, chNext, chNextNext) =>
				this.#column === 0 &&
				ch === CharacterCodes.backtick &&
				chNext === CharacterCodes.backtick &&
				chNextNext === CharacterCodes.backtick,
		);
