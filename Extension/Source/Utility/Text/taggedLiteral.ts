		...
		switch (result.state) {
			...
			case "substitution":
				...
				if (expression) {
					...
					isIdentifierStart(char.codePointAt(0)!) ||
					char === "-" ||
					char === "/"
				) {
					expression += char;
...
function resolveValue(
	...
	customResolver = (_prefix: string, _expression: string) => "",
): string {
	...
	if (prefix) {
		...
		if (variable !== undefined && variable !== null) {
			// did we get back an actual value
			// it's a child of a variable
			return joinIfArray(
				suffix.includes(":") // is the suffix another expression?
					? resolveValue(suffix, variable) // Yeah, resolve it
					: (variable[suffix] ??
							customResolver(prefix, suffix) ??
							""),
			); // No, return the member of the variable, or dynamic, or empty string
		}
...
export function render(
	...
	if (Array.isArray(templateString)) {
		return templateString.map((each) =>
			render(each, context, customResolver, asJs),
		);
	}
	...
	return state === "error"
		...
		: template.reduce(
				(result, each, index) =>
					`${result}${stabilize(resolveValue(expressions[index - 1], context, customResolver))}${each}`,
			); // resolve the inline expressions and join the template
}
...
export function evaluateExpression(
	...
	const result = expression.match(/\!|==|!=|>=|<=|>|<|\?|\|\||&&/)
		? (safeEval(
				render(expression, context, customResolver, true),
			) as Primitive)
		: render(expression, context, customResolver);
...
export function recursiveRender<T extends Record<string, any>>(
	obj: T,
	context: Record<string, any>,
	customResolver = (_prefix: string, _expression: string) => "",
): T {
	...
	for (const [key, value] of Object.entries(obj)) {
		const newKey =
			is.string(key) && key.includes("${")
				? render(key, context, customResolver)
				: key;
