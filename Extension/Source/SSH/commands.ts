...
export async function rsync(
	...
	return runSshTerminalCommandWithLogin(host, {
		systemInteractor: defaultSystemInteractor,
		nickname: "rsync",
		command: `"${rsyncPath || "rsync"}" ${args.join(" ")}`,
		token: cancellationToken,
	});
}
...
export function ssh(
	...
	sshPath?: string,
	jumpHosts?: ISshHostInfo[],
	localForwards?: ISshLocalForwardInfo[],
	continueOn?: string,
	cancellationToken?: vscode.CancellationToken,
): Promise<ProcessReturnType> {
	const args: string[] = [];
...
function localForwardToArgs(localForward: ISshLocalForwardInfo): string[] {
	...
	if (
		localForward.localSocket &&
		(localForward.bindAddress || localForward.port)
	) {
		throw Error(
			localize(
