	...
	async onData(
		...
		if (
			...
			const confirmation: string | undefined =
				await this.confirmationProvider(
					this.hostName,
					fingerprintMatch[1],
					cancelToken,
				);
			...
			isWindows &&
			(data.includes("The authenticity of host ") ||
				(data === "" && extraDetails?.detectedServerKey))
		) {
			// hack for #1195
	...
	async onData(
		...
		if (
			...
			const message: string = data
				.substring(data.indexOf("Warning"), data.indexOf("Are"))
				.trim();

			const confirmation: string | undefined =
				await this.confirmationProvider(message, cancelToken);
...
export class PassphraseInteractor implements IInteractor {
	...
	async onData(
		...
		if (lines.some((l) => l.indexOf("Enter passphrase for") >= 0)) {
			...
			const passphrase: string | undefined =
				await this.passphraseProvider(
					undefined,
					undefined,
					cancelToken,
...
function getPasswordPrompt(
	...
	const match: RegExpMatchArray | null = stripEscapeSequences(data).match(
		/([a-zA-Z0-9\-_@\.]*)'s password:/,
	);
...
export class PasswordInteractor implements IInteractor {
	...
	constructor(
		private readonly host: ISshHostInfo,
		private readonly passwordProvider: IStringProvider,
	) {}
	...
	async onData(
		...
		const pwPrompt: { user?: string; message?: string } | undefined =
			getPasswordPrompt(data, extraDetails);

		if (pwPrompt && typeof pwPrompt.user === "string") {
			result.postAction = "consume";

			const actualUser: string =
				pwPrompt.user === ""
					? getFullHostAddress(this.host)
					: pwPrompt.user;

			const passwordCacheKey: string = `SSH:${actualUser}`;

			const cachedPassword: string | undefined =
	...
	async onData(
		...
		if (data.includes("Verification code:")) {
			...
			const verificationCode: string | undefined =
				await this.verificationCodeProvider(
