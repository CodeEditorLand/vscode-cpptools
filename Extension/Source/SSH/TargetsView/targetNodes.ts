...
export const workspaceState_activeSshTarget: string =
...
export class TargetLeafNode extends LabelLeafNode {
	constructor(
		...
		readonly sshConfigHostInfo: ISshConfigHostInfo,
	) {
		super(name);
	}
	...
	async getTreeItem(): Promise<TreeItem> {
		...
		const removable: boolean = await isWritable(
			this.sshConfigHostInfo.file,
		);

		if (_activeTarget === this.name) {
			item.description = localize(
				"ssh.target.active.description",
				"[Active]",
			);
			...
			item.contextValue =
				"CppSshTargetsView.targetLeafRemovableCanSetActive";
		} else {
