...
let _targets: Map<string, ISshConfigHostInfo> = new Map<
	string,
	ISshConfigHostInfo
>();

export class SshTargetsProvider
	implements vscode.TreeDataProvider<BaseNode>, vscode.Disposable
{
	...
	async getChildren(node?: BaseNode): Promise<BaseNode[]> {
		...
		if (children.length === 0) {
			return [
				new LabelLeafNode(localize("no.ssh.targets", "No SSH targets")),
			];
		}
	...
	private async getTargets(): Promise<BaseNode[]> {
		...
		const cachedActiveTarget: string | undefined =
			await getActiveSshTarget(false);

		for (const host of Array.from(_targets.keys())) {
			const sshConfigHostInfo: ISshConfigHostInfo | undefined =
				_targets.get(host);
...
export async function getActiveSshTarget(
	selectWhenNotSet: boolean = true,
	...
	if (!_activeTarget && selectWhenNotSet) {
		...
		if (!name) {
			throw Error(
				localize(
					"active.ssh.target.selection.cancelled",
					"Active SSH target selection cancelled.",
				),
			);
		}
...
const addNewSshTarget: string = localize(
	"add.new.ssh.target",
	"{0} Add New SSH Target...",
	"$(plus)",
);
...
export async function selectSshTarget(): Promise<string | undefined> {
	...
	const selection: string | undefined = await vscode.window.showQuickPick(
		items,
		{ title: localize("select.ssh.target", "Select an SSH target") },
	);
