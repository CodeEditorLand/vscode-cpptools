...
const userSshConfigurationFile: string = path.resolve(
	os.homedir(),
	".ssh/config",
);

const ProgramData: string =
	process.env.ALLUSERSPROFILE || process.env.PROGRAMDATA || "C:\\ProgramData";

const systemSshConfigurationFile: string = isWindows
...
// Map: host -> info
export async function getSshConfigHostInfos(): Promise<
	Map<string, ISshConfigHostInfo>
> {
	const hostInfos: Map<string, ISshConfigHostInfo> = new Map<
		string,
		ISshConfigHostInfo
	>();
...
function extractHostNames(parsedConfig: Configuration): {
	[host: string]: string;
} {
 ...
 * @returns
 */
export async function getSshConfiguration(
...
export async function writeSshConfiguration(
	configurationPath: string,
...
): c is HostConfigurationDirective {
	return (
		isDirective(c) &&
		c.param === "Host" &&
		!!c.value &&
		!!(c as HostConfigurationDirective).config
	);
}
...
function caseNormalizeConfigProps(config: Configuration): Configuration {
	const caseNormalizeDirective: (entry: { param: string }) => void = (entry: {
		param: string;
	}) =>
		(entry.param =
			CASE_NORMALIZED_PROPS.get(entry.param.toLowerCase()) ||
			entry.param);
...
function getHostsFromHostConfig(hostValue: string | string[]): string[] {
	...
	return hosts.filter(
		(h) => !containsWildcard(h) && !h.match(/^\s*$/) && !h.match(/^!/),
	);
}
