...
const flags: {
	[flag: string]:
		| ((entries: { [key: string]: string }, value: string) => void)
		| null;
} = {
	...
	L: (entries, args) => {
		 ...
		 */
		const parsedArgs: RegExpMatchArray | null = args.match(
			/^((.*):?\d+)?:(.+?)?$/,
		);
		if (parsedArgs) {
	...
	o: (entries, option) => {
		...
		if (delimiter === -1) {
			...
			throw new CommandParseError(
				`Argument missing for option ${option}`,
			);
		}
 ...
 */
const getOptDirective: string =
	"1246ab:c:e:fgi:kl:m:no:p:qstvxAB:CD:E:F:GI:J:KL:MNO:PQ:R:S:TVw:W:XYy";
 ...
 */
export function sshCommandToConfig(
	command: string,
	name?: string,
): { [key: string]: string } {
...
function parseFlags(
	...
	const parser: BasicParser = new BasicParser(
		`:${getOptDirective}`,
		input,
		0,
	);
	...
	while (true) {
		...
		if (next.option === ":") {
			throw new CommandParseError(
				`Expected flag -${next.optopt} to have an argument but it did not`,
			);
		}
