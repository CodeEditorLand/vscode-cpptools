...
export function showPasswordInputBox(
	...
	const msg: string = user
		? localize(
				"ssh.enter.password.for.user",
				'Enter password for user "{0}"',
				user,
			)
		: localize("ssh.message.enter.password", "Enter password");
	return showInputBox(msg, prompt, cancelToken);
}
...
const continueConfirmationPlaceholder: string = localize(
...
export function runInteractiveSshTerminalCommand(
	args: ITerminalCommandArgs,
): Promise<ProcessReturnType> {
	const disposables: vscode.Disposable[] = [];
	const { systemInteractor, command, interactors, nickname, token } = args;
	let logIsPaused: boolean = false;
	const loggingLevel: number = getNumericLoggingLevel(
		new CppSettings().loggingLevel,
	);
	...
	const done = (
		...
		if (cancel) {
			if (continueWithoutExiting) {
				const warningMessage: string = localize(
	...
	const failed = (error?: any) => {
		clean();
		const errorMessage: string = localize(
			"ssh.process.failed",
			'"{0}" process failed: {1}',
			nickname,
			error,
	...
	const handleOutputLogging = (data: string): void => {
		...
		if (args.marker) {
			...
			const resumeIdx: number = data.lastIndexOf(
				getResumeLogMarker(args.marker),
			);
			if (resumeIdx >= 0) {
			...
			for (const interactor of interactors) {
				try {
					const interaction: IInteraction =
						await interactor.onData(stdout);
	...
	const terminalIsWindows: boolean =
		typeof args.terminalIsWindows === "boolean"
			? args.terminalIsWindows
			: isWindows;
	try {
		...
		terminalListener = systemInteractor.onDidWriteTerminalData(
			async (e) => {
				...
				terminalDataHandlingQueue = terminalDataHandlingQueue.finally(
					() => void handleTerminalOutput(e),
				);
			},
		);
		...
		if (args.sendText) {
			const sendText: string = terminalIsWindows
				? `(${args.sendText})\nexit /b %ErrorLevel%`
				: `${args.sendText}\nexit $?`;
...
function getShellPath(_isWindows: boolean): string {
	if (_isWindows) {
		...
		if (process.env.SystemRoot) {
			...
			const cmdPath: string = path.join(
