...
import {
	getLocaleId,
	getLocalizedHtmlPath,
} from "./LanguageServer/localization";
...
export async function activate(
	...
	class SchemaProvider implements vscode.TextDocumentContentProvider {
		public async provideTextDocumentContent(
			...
			console.assert(
				uri.path[0] === "/",
				"A preceding slash is expected on schema uri path",
			);
			const fileName: string = uri.path.substring(1);
			const locale: string = getLocaleId();
			let localizedFilePath: string = util.getExtensionFilePath(
				path.join("dist/schema/", locale, fileName),
			);
			const fileExists: boolean =
				await util.checkFileExists(localizedFilePath);
			if (!fileExists) {
	...
	const ignoreRecommendations: boolean | undefined = vscode.workspace
		...
		.get<boolean>("ignoreRecommendations");
	...
	const isIntelliSenseEngineDisabled: boolean =
		settings.intelliSenseEngine === "disabled";
	const shouldActivateLanguageServer: boolean =
		!isIntelliSenseEngineDisabled && !isOldMacOs;
	...
	if (isOldMacOs) {
		...
		void vscode.window.showErrorMessage(
			localize(
				...
				"Versions of the C/C++ extension more recent than {0} require at least macOS version {1}.",
				"1.9.8",
				"10.12",
			),
		);
	...
	if (shouldActivateLanguageServer) {
		...
		log(
			localize("intellisense.disabled", "intelliSenseEngine is disabled"),
		);
	}
...
async function makeBinariesExecutable(): Promise<void> {
	...
	if (process.platform !== "win32") {
		...
		if (process.platform === "darwin") {
			...
			promises.push(
				util.allowExecution(util.getExtensionFilePath("./bin/libc.so")),
			);
		}
	}
					...
					isPlatformCompatible =
						...
						(platformInfo.architecture === "x64" ||
							(platformInfo.architecture === "arm64" &&
								semver.gte(os.release(), "10.0.22000")));
					break;
				case "win32-ia32":
					isPlatformMatching =
						platformInfo.platform === "win32" &&
