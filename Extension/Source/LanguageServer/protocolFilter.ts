...
export function createProtocolFilter(): Middleware {
	...
	const invoke1 = (a: any, next: (a: any) => any): any =>
		clients.ActiveClient.enqueue(() => next(a));
	const invoke2 = (a: any, b: any, next: (a: any, b: any) => any): any =>
		clients.ActiveClient.enqueue(() => next(a, b));
	const invoke3 = (
		a: any,
		b: any,
		c: any,
		next: (a: any, b: any, c: any) => any,
	): any => clients.ActiveClient.enqueue(() => next(a, b, c));
	const invoke4 = (
		a: any,
		b: any,
		c: any,
		d: any,
		next: (a: any, b: any, c: any, d: any) => any,
	): any => clients.ActiveClient.enqueue(() => next(a, b, c, d));
	...
	return {
				...
				if (clients.checkOwnership(client, document)) {
					...
					if (!client.TrackedDocuments.has(uriString)) {
						...
						if (
							...
							shouldChangeFromCToCpp(document)
						) {
						...
						if (!anyFileOpened) {
							...
							await client.onDidChangeVisibleTextEditors(
								cppEditors,
							);
						}
					}
		...
		provideHover: async (
			...
			position,
			token,
			next: (document: any, position: any, token: any) => any,
		) =>
			clients.ActiveClient.enqueue(async () => {
				const me: Client = clients.getClientFor(document.uri);
