...
export class CppBuildTaskProvider implements TaskProvider {
	...
	public async getTasks(
		appendSourceToName: boolean = false,
	): Promise<CppBuildTask[]> {
		...
		const userCompilerPathAndArgs: util.CompilerPathAndArgs | undefined =
			await activeClient.getCurrentCompilerPathAndArgs();
		...
		const isCompilerValid: boolean = userCompilerPath
			? await util.checkFileExists(userCompilerPath)
			: false;
		const userCompilerIsCl: boolean =
			isCompilerValid &&
			!!userCompilerPathAndArgs &&
			userCompilerPathAndArgs.compilerName === "cl.exe";
		...
		let knownCompilers: configs.KnownCompiler[] | undefined =
			await activeClient.getKnownCompilers();
		if (knownCompilers) {
			const compiler_condition: (
				info: configs.KnownCompiler,
			) => boolean = (info) =>
				...
				(!isCompilerValid ||
					(!!userCompilerPathAndArgs &&
						path.basename(info.path) !==
							userCompilerPathAndArgs.compilerName)) &&
				(!isWindows || !info.path.startsWith("/"));
			const cl_to_add: configs.KnownCompiler | undefined =
				userCompilerIsCl
					? undefined
					: knownCompilers.find(
							(info) =>
			...
			knownCompilers = knownCompilers.filter(
				(info) =>
					info === cl_to_add ||
					(path.basename(info.path) !== "cl.exe" &&
						compiler_condition(info)),
		...
		const knownCompilerPaths: string[] | undefined =
			knownCompilerPathsSet.size
				? Array.from(knownCompilerPathsSet)
				: undefined;
		...
		if (isCompilerValid && userCompilerPath) {
			result.push(
				this.getTask(
					userCompilerPath,
					appendSourceToName,
					userCompilerPathAndArgs?.allCompilerArgs,
				),
			);
		}
		...
		if (knownCompilerPaths) {
			result.push(
				...knownCompilerPaths.map<Task>((compilerPath) =>
					this.getTask(compilerPath, appendSourceToName, undefined),
				),
			);
		}
	...
	private getTask: (
		compilerPath: string | util.IQuotedString,
		appendSourceToName: boolean,
		compilerArgs?: (string | util.IQuotedString)[],
		definition?: CppBuildTaskDefinition,
		detail?: string,
	) => Task = (
		compilerPath: string | util.IQuotedString,
		appendSourceToName: boolean,
		...
		const compilerPathString: string = util.isString(compilerPath)
			? compilerPath
			: compilerPath.value;
		const compilerPathBase: string = path.basename(compilerPathString);
		const isCl: boolean = compilerPathBase.toLowerCase() === "cl.exe";
		const isClang: boolean =
			!isCl && compilerPathBase.toLowerCase().includes("clang");
		// Double-quote the command if needed.
		const resolvedCompilerPathString: string = isCl
			? compilerPathBase
			: compilerPathString;
		...
		if (!definition) {
			...
			const taskLabel: string =
				(appendSourceToName &&
				!compilerPathBase.startsWith(ext.configPrefix)
					? ext.configPrefix
					: "") +
				compilerPathBase +
				" " +
				localize("build.active.file", "build active file");
			...
			const cwd: string =
				isWindows &&
				!isCl &&
				!process.env.PATH?.includes(path.dirname(compilerPathString))
					? path.dirname(compilerPathString)
					: "${fileDirname}";
			const options: cp.ExecOptions | cp.SpawnOptions | undefined = {
				cwd: cwd,
			};
			definition = {
		...
		const editor: TextEditor | undefined = window.activeTextEditor;
		const folder: WorkspaceFolder | undefined = editor
		...
		const scope: WorkspaceFolder | TaskScope = folder
			? folder
			: TaskScope.Workspace;
		const task: CppBuildTask = new Task(
			definition,
			scope,
			definition.label,
			ext.CppSourceStr,
		...
		task.detail = detail
			? detail
			: localize("compiler.details", "compiler:") +
				" " +
				resolvedCompilerPathString;
	...
	public async getJsonTasks(): Promise<CppBuildTask[]> {
		...
		const buildTasksJson: CppBuildTask[] = rawTasksJson.map((task: any) => {
			if (
				!task.label ||
				!task.type ||
				task.type !== CppBuildTaskProvider.CppBuildScriptType
			) {
				return null;
			}
			...
			const cppBuildTask: CppBuildTask = new Task(
				definition,
				TaskScope.Workspace,
				task.label,
				ext.CppSourceStr,
	...
	public async writeDefaultBuildTask(
		taskLabel: string,
	...
	public async isExistingTask(
		...
		return rawTasksJson.tasks.find(
			(task: any) => task.label && task.label === taskLabel,
		);
	}

	public async writeBuildTask(
		taskLabel: string,
		workspaceFolder?: WorkspaceFolder,
		...
		const selectedTask: any = buildTasks.find(
			(task) => task.name === taskLabel,
		);
		console.assert(selectedTask);
		if (!selectedTask) {
			throw new Error(
				"Failed to get selectedTask in checkBuildTaskExists()",
			);
		} else {
		...
		if (
			!rawTasksJson.tasks.find(
				(task: any) => task.label === selectedTask?.definition.label,
			)
		) {
			const newTask: any = {
		...
		await util.writeFileText(
			tasksJsonPath,
			JSON.stringify(rawTasksJson, null, settings.editorTabSize),
		);
	}
	...
	public async runBuildTask(taskLabel: string): Promise<void> {
		...
		if (!task) {
			const detectedBuildTasks: CppBuildTask[] =
		...
		if (!task) {
			...
			const resolvedTask: CppBuildTask | undefined =
				this.resolveInsiderTask(task);
			if (resolvedTask) {
				const execution: TaskExecution =
					await tasks.executeTask(resolvedTask);
				return new Promise<void>((resolve) => {
					const disposable: Disposable = tasks.onDidEndTask(
						(endEvent: TaskEndEvent) => {
							if (
								endEvent.execution.task.group ===
									TaskGroup.Build &&
								endEvent.execution === execution
							) {
								disposable.dispose();
								resolve();
							}
				...
				throw new Error(
					"Failed to run resolved task in runBuildTask()",
				);
			}
		}
	}
...
export const cppBuildTaskProvider: CppBuildTaskProvider =
	new CppBuildTaskProvider();
...
class CustomBuildTaskTerminal implements Pseudoterminal {
	...
	public get onDidClose(): Event<number> {
		return this.closeEmitter.event;
	}
	private endOfLine: string = "\r\n";

	constructor(
		private command: string | util.IQuotedString,
		private args: (string | util.IQuotedString)[],
		private options: cp.ExecOptions | undefined,
	...
	async open(
		...
		if (
			this.buildOptions.taskUsesActiveFile &&
			!util.isCppOrCFile(window.activeTextEditor?.document.uri)
		) {
		...
		if (
			this.buildOptions.taskUsesActiveFile &&
			window.activeTextEditor?.document.languageId === "cpp" &&
			this.buildOptions.insertStd
		) {
		...
		this.writeEmitter.fire(
			localize("starting.build", "Starting build...") + this.endOfLine,
		);
		await this.doBuild();
	}
	...
	private async doBuild(): Promise<any> {
		...
		const exePath: string | undefined = util.resolveVariables(
			util.findExePathInArgs(this.args),
		);
		...
		if (this.options.cwd) {
			this.options.cwd = util.resolveVariables(
				this.options.cwd.toString(),
			);
		} else {
			const editor: TextEditor | undefined = window.activeTextEditor;
			let folder: WorkspaceFolder | undefined = editor
				? workspace.getWorkspaceFolder(editor.document.uri)
				: undefined;
		...
		const activeCommand: string = util.buildShellCommandLine(
			resolvedCommand,
			this.command,
			this.args,
		);
		...
		try {
			...
			const result: number = this.printBuildSummary(
				error,
				stdout,
				stderr,
				spawnResult,
	...
	private printBuildSummary(
		...
		if (
			error ||
			(!stdout && stderr && stderr.includes("error")) ||
			(stdout &&
				(stdout.includes("error C") ||
					stdout.includes("LINK : fatal error")))
		) {
			// cl.exe compiler errors
			telemetry.logLanguageServerEvent("cppBuildTaskError");
			this.writeEmitter.fire(
				localize(
					"build.finished.with.error",
			...
			(!stdout && stderr) || // gcc/clang
			(stdout && stdout.includes("warning C"))
		) {
			// cl.exe compiler warnings
