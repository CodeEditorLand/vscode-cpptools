			...
			for (const languageId of ["c", "cpp", "cuda-cpp"]) {
				api.registerRelatedFilesProvider(
					...
					async (
						...
						try {
							const getIncludesHandler = async () =>
								(await getIncludes(1))?.includedFiles.map(
							...
							const getTraitsHandler = async () => {
								...
								let traits: CopilotTrait[] = [
									{
										name: "intelliSenseDisclaimer",
										value: "",
										includeInPrompt: true,
										promptTextOverride: `IntelliSense is currently configured with the following compiler information. It reflects the active configuration, and the project may have more configurations targeting different platforms.`,
									},
									{
										name: "intelliSenseDisclaimerBeginning",
										value: "",
										includeInPrompt: true,
										promptTextOverride: `Beginning of IntelliSense information.`,
									},
								];
								if (projectContext.language) {
									traits.push({
										name: "language",
										value: projectContext.language,
										includeInPrompt: true,
										promptTextOverride: `The language is ${projectContext.language}.`,
									});
								}
								if (projectContext.compiler) {
									traits.push({
										name: "compiler",
										value: projectContext.compiler,
										includeInPrompt: true,
										promptTextOverride: `This project compiles using ${projectContext.compiler}.`,
									});
								}
								if (projectContext.standardVersion) {
									traits.push({
										name: "standardVersion",
										value: projectContext.standardVersion,
										includeInPrompt: true,
										promptTextOverride: `This project uses the ${projectContext.standardVersion} language standard.`,
									});
								}
								if (projectContext.targetPlatform) {
									traits.push({
										name: "targetPlatform",
										value: projectContext.targetPlatform,
										includeInPrompt: true,
										promptTextOverride: `This build targets ${projectContext.targetPlatform}.`,
									});
								}
								if (projectContext.targetArchitecture) {
									traits.push({
										name: "targetArchitecture",
										value: projectContext.targetArchitecture,
										includeInPrompt: true,
										promptTextOverride: `This build targets ${projectContext.targetArchitecture}.`,
									});
								}
								...
								if (projectContext.compiler) {
									...
									const filterMap =
										getCompilerArgumentFilterMap(
											projectContext.compiler,
											context,
										);
									if (filterMap !== undefined) {
										const directAskMap: Record<
											string,
											string
										> = context.flags
											.copilotcppCompilerArgumentDirectAskMap
											? JSON.parse(
													context.flags
														.copilotcppCompilerArgumentDirectAskMap as string,
												)
											: {};
										...
										if (remainingArguments.length > 0) {
											const compilerArgumentsValue =
												remainingArguments.join(", ");
											traits.push({
												name: "compilerArguments",
												value: compilerArgumentsValue,
												includeInPrompt: true,
												promptTextOverride: `The compiler arguments include: ${compilerArgumentsValue}.`,
											});
										}
								...
								traits.push({
									name: "intelliSenseDisclaimerEnd",
									value: "",
									includeInPrompt: true,
									promptTextOverride: `End of IntelliSense information.`,
								});

								const includeTraitsArray = context.flags
									.copilotcppIncludeTraits
									? (context.flags
											.copilotcppIncludeTraits as string[])
									: [];
								const includeTraits = new Set(
									includeTraitsArray,
								);
								telemetryProperties["includeTraits"] =
									includeTraitsArray.join(",");

								// standardVersion trait is enabled by default.
								traits = traits.filter(
									(trait) =>
										includeTraits.has(trait.name) ||
										trait.name === "standardVersion",
								);
							...
							return {
								entries: await includesPromise,
								traits: await traitsPromise,
							};
						} catch (exception) {
							try {
								...
								logger
									...
									.appendLine(
										localize(
											"copilot.relatedfilesprovider.error",
											"Error while retrieving result. Reason: {0}",
											err.message,
										),
									);
							} catch {
							...
							telemetryMetrics["duration"] =
								performance.now() - start;
							telemetry.logCopilotEvent(
