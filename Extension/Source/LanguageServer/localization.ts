...
export function getLocaleId(): string {
	...
	if (typeof process.env.VSCODE_NLS_CONFIG === "string") {
		const vscodeOptions: VSCodeNlsConfig = JSON.parse(
...
export function getLocalizedHtmlPath(originalPath: string): string {
	...
	if (!locale.startsWith("en")) {
		const localizedFilePath: string = getExtensionFilePath(
			path.join("dist/html/", locale, originalPath),
		);
...
export function getLocalizedSymbolScope(scope: string, detail: string): string {
	return localize(
		{
			key: "c.cpp.symbolscope.separator",
			comment: [
