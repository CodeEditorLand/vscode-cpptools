...
import {
	...
	isSameProviderExtensionId,
} from "../LanguageServer/customProviders";
import {
	DebugProtocolParams,
...
import {
	CppInlayHint,
	InlayHintsProvider,
} from "./Providers/inlayHintProvider";
...
function publishRefactorDiagnostics(
	params: PublishRefactorDiagnosticsParams,
): void {
	if (!diagnosticsCollectionRefactor) {
		diagnosticsCollectionRefactor =
			vscode.languages.createDiagnosticCollection(
				configPrefix + "Refactor",
			);
	}
			...
			for (const info of d.relatedInformation) {
				diagnostic.relatedInformation.push(
					new vscode.DiagnosticRelatedInformation(
						makeVscodeLocation(info.location),
						info.message,
					),
				);
			}
...
interface InternalWorkspaceBrowseConfiguration
	extends WorkspaceBrowseConfiguration {
	compilerArgsLegacy?: string[];
}
...
// Requests
const PreInitializationRequest: RequestType<void, string, void> =
	new RequestType<void, string, void>("cpptools/preinitialize");
const InitializationRequest: RequestType<CppInitializationParams, void, void> =
	new RequestType<CppInitializationParams, void, void>("cpptools/initialize");
const QueryCompilerDefaultsRequest: RequestType<
	QueryDefaultCompilerParams,
	configs.CompilerDefaults,
	void
> = new RequestType<QueryDefaultCompilerParams, configs.CompilerDefaults, void>(
	"cpptools/queryCompilerDefaults",
);
const SwitchHeaderSourceRequest: RequestType<
	SwitchHeaderSourceParams,
	string,
	void
> = new RequestType<SwitchHeaderSourceParams, string, void>(
	"cpptools/didSwitchHeaderSource",
);
const GetDiagnosticsRequest: RequestType<void, GetDiagnosticsResult, void> =
...
export const HoverRequest: RequestType<
	...
	vscode.Hover,
	void
> = new RequestType<TextDocumentPositionParams, vscode.Hover, void>(
	"cpptools/hover",
);
export const GetCopilotHoverInfoRequest: RequestType<
	GetCopilotHoverInfoParams,
	GetCopilotHoverInfoResult,
	void
> = new RequestType<GetCopilotHoverInfoParams, GetCopilotHoverInfoResult, void>(
	"cpptools/getCopilotHoverInfo",
);
const CreateDeclarationOrDefinitionRequest: RequestType<
	CreateDeclarationOrDefinitionParams,
	CreateDeclarationOrDefinitionResult,
	void
> = new RequestType<
	CreateDeclarationOrDefinitionParams,
	CreateDeclarationOrDefinitionResult,
	void
>("cpptools/createDeclDef");
const ExtractToFunctionRequest: RequestType<
	ExtractToFunctionParams,
	WorkspaceEditResult,
	void
> = new RequestType<ExtractToFunctionParams, WorkspaceEditResult, void>(
	"cpptools/extractToFunction",
);
const GoToDirectiveInGroupRequest: RequestType<
	GoToDirectiveInGroupParams,
	Position | undefined,
	void
> = new RequestType<GoToDirectiveInGroupParams, Position | undefined, void>(
	"cpptools/goToDirectiveInGroup",
);
const GenerateDoxygenCommentRequest: RequestType<
	GenerateDoxygenCommentParams,
	GenerateDoxygenCommentResult | undefined,
	void
> = new RequestType<
	GenerateDoxygenCommentParams,
	GenerateDoxygenCommentResult,
	void
>("cpptools/generateDoxygenComment");
const ChangeCppPropertiesRequest: RequestType<CppPropertiesParams, void, void> =
	new RequestType<CppPropertiesParams, void, void>(
		"cpptools/didChangeCppProperties",
	);
const IncludesRequest: RequestType<GetIncludesParams, GetIncludesResult, void> =
	new RequestType<GetIncludesParams, GetIncludesResult, void>(
		"cpptools/getIncludes",
	);
const CppContextRequest: RequestType<
	TextDocumentIdentifier,
	ChatContextResult,
	void
> = new RequestType<TextDocumentIdentifier, ChatContextResult, void>(
	"cpptools/getChatContext",
);
const ProjectContextRequest: RequestType<
	TextDocumentIdentifier,
	ProjectContextResult,
	void
> = new RequestType<TextDocumentIdentifier, ProjectContextResult, void>(
	"cpptools/getProjectContext",
);

// Notifications to the server
const DidOpenNotification: NotificationType<DidOpenTextDocumentParams> =
	new NotificationType<DidOpenTextDocumentParams>("textDocument/didOpen");
const FileCreatedNotification: NotificationType<FileChangedParams> =
	new NotificationType<FileChangedParams>("cpptools/fileCreated");
const FileChangedNotification: NotificationType<FileChangedParams> =
	new NotificationType<FileChangedParams>("cpptools/fileChanged");
const FileDeletedNotification: NotificationType<FileChangedParams> =
	new NotificationType<FileChangedParams>("cpptools/fileDeleted");
const ResetDatabaseNotification: NotificationType<void> =
	new NotificationType<void>("cpptools/resetDatabase");
const PauseParsingNotification: NotificationType<void> =
	new NotificationType<void>("cpptools/pauseParsing");
const ResumeParsingNotification: NotificationType<void> =
	new NotificationType<void>("cpptools/resumeParsing");
const DidChangeActiveEditorNotification: NotificationType<DidChangeActiveEditorParams> =
	new NotificationType<DidChangeActiveEditorParams>(
		"cpptools/didChangeActiveEditor",
	);
const RestartIntelliSenseForFileNotification: NotificationType<TextDocumentIdentifier> =
	new NotificationType<TextDocumentIdentifier>(
		"cpptools/restartIntelliSenseForFile",
	);
const DidChangeTextEditorSelectionNotification: NotificationType<Range> =
	new NotificationType<Range>("cpptools/didChangeTextEditorSelection");
const ChangeCompileCommandsNotification: NotificationType<FileChangedParams> =
	new NotificationType<FileChangedParams>(
		"cpptools/didChangeCompileCommands",
	);
const ChangeSelectedSettingNotification: NotificationType<FolderSelectedSettingParams> =
	new NotificationType<FolderSelectedSettingParams>(
		"cpptools/didChangeSelectedSetting",
	);
const IntervalTimerNotification: NotificationType<void> =
	new NotificationType<void>("cpptools/onIntervalTimer");
const CustomConfigurationHighPriorityNotification: NotificationType<CustomConfigurationParams> =
	new NotificationType<CustomConfigurationParams>(
		"cpptools/didChangeCustomConfigurationHighPriority",
	);
const CustomConfigurationNotification: NotificationType<CustomConfigurationParams> =
	new NotificationType<CustomConfigurationParams>(
		"cpptools/didChangeCustomConfiguration",
	);
const CustomBrowseConfigurationNotification: NotificationType<CustomBrowseConfigurationParams> =
	new NotificationType<CustomBrowseConfigurationParams>(
		"cpptools/didChangeCustomBrowseConfiguration",
	);
const ClearCustomConfigurationsNotification: NotificationType<WorkspaceFolderParams> =
	new NotificationType<WorkspaceFolderParams>(
		"cpptools/clearCustomConfigurations",
	);
const ClearCustomBrowseConfigurationNotification: NotificationType<WorkspaceFolderParams> =
	new NotificationType<WorkspaceFolderParams>(
		"cpptools/clearCustomBrowseConfiguration",
	);
const PreviewReferencesNotification: NotificationType<void> =
	new NotificationType<void>("cpptools/previewReferences");
const RescanFolderNotification: NotificationType<void> =
	new NotificationType<void>("cpptools/rescanFolder");
const FinishedRequestCustomConfig: NotificationType<FinishedRequestCustomConfigParams> =
	new NotificationType<FinishedRequestCustomConfigParams>(
		"cpptools/finishedRequestCustomConfig",
	);
const DidChangeSettingsNotification: NotificationType<SettingsParams> =
	new NotificationType<SettingsParams>("cpptools/didChangeSettings");
const DidChangeVisibleTextEditorsNotification: NotificationType<DidChangeVisibleTextEditorsParams> =
	new NotificationType<DidChangeVisibleTextEditorsParams>(
		"cpptools/didChangeVisibleTextEditors",
	);
const DidChangeTextEditorVisibleRangesNotification: NotificationType<DidChangeTextEditorVisibleRangesParams> =
	new NotificationType<DidChangeTextEditorVisibleRangesParams>(
		"cpptools/didChangeTextEditorVisibleRanges",
	);

const CodeAnalysisNotification: NotificationType<CodeAnalysisParams> =
	new NotificationType<CodeAnalysisParams>("cpptools/runCodeAnalysis");
const PauseCodeAnalysisNotification: NotificationType<void> =
	new NotificationType<void>("cpptools/pauseCodeAnalysis");
const ResumeCodeAnalysisNotification: NotificationType<void> =
	new NotificationType<void>("cpptools/resumeCodeAnalysis");
const CancelCodeAnalysisNotification: NotificationType<void> =
	new NotificationType<void>("cpptools/cancelCodeAnalysis");
const RemoveCodeAnalysisProblemsNotification: NotificationType<RemoveCodeAnalysisProblemsParams> =
	new NotificationType<RemoveCodeAnalysisProblemsParams>(
		"cpptools/removeCodeAnalysisProblems",
	);

// Notifications from the server
const ReloadWindowNotification: NotificationType<void> =
	new NotificationType<void>("cpptools/reloadWindow");
const UpdateTrustedCompilersNotification: NotificationType<UpdateTrustedCompilerPathsResult> =
	new NotificationType<UpdateTrustedCompilerPathsResult>(
		"cpptools/updateTrustedCompilersList",
	);
const LogTelemetryNotification: NotificationType<TelemetryPayload> =
	new NotificationType<TelemetryPayload>("cpptools/logTelemetry");
const ReportTagParseStatusNotification: NotificationType<TagParseStatus> =
	new NotificationType<TagParseStatus>("cpptools/reportTagParseStatus");
const ReportStatusNotification: NotificationType<ReportStatusNotificationBody> =
	new NotificationType<ReportStatusNotificationBody>("cpptools/reportStatus");
const DebugProtocolNotification: NotificationType<DebugProtocolParams> =
	new NotificationType<DebugProtocolParams>("cpptools/debugProtocol");
const DebugLogNotification: NotificationType<LocalizeStringParams> =
	new NotificationType<LocalizeStringParams>("cpptools/debugLog");
const CompileCommandsPathsNotification: NotificationType<CompileCommandsPaths> =
	new NotificationType<CompileCommandsPaths>("cpptools/compileCommandsPaths");
const ReferencesNotification: NotificationType<refs.ReferencesResult> =
	new NotificationType<refs.ReferencesResult>("cpptools/references");
const ReportReferencesProgressNotification: NotificationType<refs.ReportReferencesProgressNotification> =
	new NotificationType<refs.ReportReferencesProgressNotification>(
		"cpptools/reportReferencesProgress",
	);
const RequestCustomConfig: NotificationType<string> =
	new NotificationType<string>("cpptools/requestCustomConfig");
const PublishRefactorDiagnosticsNotification: NotificationType<PublishRefactorDiagnosticsParams> =
	new NotificationType<PublishRefactorDiagnosticsParams>(
		"cpptools/publishRefactorDiagnostics",
	);
const ShowMessageWindowNotification: NotificationType<ShowMessageWindowParams> =
	new NotificationType<ShowMessageWindowParams>("cpptools/showMessageWindow");
const ShowWarningNotification: NotificationType<ShowWarningParams> =
	new NotificationType<ShowWarningParams>("cpptools/showWarning");
const ReportTextDocumentLanguage: NotificationType<string> =
	new NotificationType<string>("cpptools/reportTextDocumentLanguage");
const IntelliSenseSetupNotification: NotificationType<IntelliSenseSetup> =
	new NotificationType<IntelliSenseSetup>("cpptools/IntelliSenseSetup");
const SetTemporaryTextDocumentLanguageNotification: NotificationType<SetTemporaryTextDocumentLanguageParams> =
	new NotificationType<SetTemporaryTextDocumentLanguageParams>(
		"cpptools/setTemporaryTextDocumentLanguage",
	);
const ReportCodeAnalysisProcessedNotification: NotificationType<number> =
	new NotificationType<number>("cpptools/reportCodeAnalysisProcessed");
const ReportCodeAnalysisTotalNotification: NotificationType<number> =
	new NotificationType<number>("cpptools/reportCodeAnalysisTotal");
const DoxygenCommentGeneratedNotification: NotificationType<GenerateDoxygenCommentResult> =
	new NotificationType<GenerateDoxygenCommentResult>(
		"cpptools/insertDoxygenComment",
	);
const CanceledReferencesNotification: NotificationType<void> =
	new NotificationType<void>("cpptools/canceledReferences");
const IntelliSenseResultNotification: NotificationType<IntelliSenseResult> =
	new NotificationType<IntelliSenseResult>("cpptools/intelliSenseResult");
...
class ClientModel {
	...
	constructor() {
		...
		this.isParsingWorkspacePaused = new DataBinding<boolean>(
			false,
			500,
			false,
		);
		this.isParsingFiles = new DataBinding<boolean>(false, 500, false);
		this.isUpdatingIntelliSense = new DataBinding<boolean>(
			false,
			500,
			false,
		);
		...
		this.referencesCommandMode =
			new DataBinding<refs.ReferencesCommandMode>(
				refs.ReferencesCommandMode.None,
			);
...
export interface Client {
	...
	updateCustomConfigurations(
		requestingProvider?: CustomConfigurationProvider1,
	): Thenable<void>;
	...
	handleGenerateDoxygenComment(
		args: DoxygenCodeActionCommandArguments | vscode.Uri | undefined,
	): Promise<void>;
	...
	handleRemoveCodeAnalysisProblems(
		refreshSquigglesOnSave: boolean,
		identifiersAndUris: CodeAnalysisDiagnosticIdentifiersAndUri[],
	): Promise<void>;
	handleFixCodeAnalysisProblems(
		workspaceEdit: vscode.WorkspaceEdit,
		refreshSquigglesOnSave: boolean,
		identifiersAndUris: CodeAnalysisDiagnosticIdentifiersAndUri[],
	): Promise<void>;
	handleDisableAllTypeCodeAnalysisProblems(
		code: string,
		identifiersAndUris: CodeAnalysisDiagnosticIdentifiersAndUri[],
	): Promise<void>;
	handleCreateDeclarationOrDefinition(
...
export class DefaultClient implements Client {
	...
	private inactiveRegionsDecorations = new Map<
		string,
	...
	public lastCustomBrowseConfiguration:
		| PersistentFolderState<WorkspaceBrowseConfiguration | undefined>
		| undefined;
	public lastCustomBrowseConfigurationProviderId:
		| PersistentFolderState<string | undefined>
		| undefined;
	public lastCustomBrowseConfigurationProviderVersion:
		| PersistentFolderState<Version>
		| undefined;
	public currentCaseSensitiveFileSupport:
		| PersistentWorkspaceState<boolean>
		| undefined;
	public currentCopilotHoverEnabled:
		| PersistentWorkspaceState<string>
		| undefined;
	private registeredProviders: PersistentFolderState<string[]> | undefined;

	private configStateReceived: ConfigStateReceived = {
		compilers: false,
	...
	private static queue = new Array<
		| [ManualPromise<unknown>, () => Promise<unknown>]
		| [ManualPromise<unknown>]
	>();
	...
	public get InitializingWorkspaceChanged(): vscode.Event<boolean> {
		return this.model.isInitializingWorkspace.ValueChanged;
	}
	public get IndexingWorkspaceChanged(): vscode.Event<boolean> {
		return this.model.isIndexingWorkspace.ValueChanged;
	}
	public get ParsingWorkspaceChanged(): vscode.Event<boolean> {
		return this.model.isParsingWorkspace.ValueChanged;
	}
	public get ParsingWorkspacePausedChanged(): vscode.Event<boolean> {
		return this.model.isParsingWorkspacePaused.ValueChanged;
	}
	public get ParsingFilesChanged(): vscode.Event<boolean> {
		return this.model.isParsingFiles.ValueChanged;
	}
	public get IntelliSenseParsingChanged(): vscode.Event<boolean> {
		return this.model.isUpdatingIntelliSense.ValueChanged;
	}
	public get RunningCodeAnalysisChanged(): vscode.Event<boolean> {
		return this.model.isRunningCodeAnalysis.ValueChanged;
	}
	public get CodeAnalysisPausedChanged(): vscode.Event<boolean> {
		return this.model.isCodeAnalysisPaused.ValueChanged;
	}
	public get CodeAnalysisProcessedChanged(): vscode.Event<number> {
		return this.model.codeAnalysisProcessed.ValueChanged;
	}
	public get CodeAnalysisTotalChanged(): vscode.Event<number> {
		return this.model.codeAnalysisTotal.ValueChanged;
	}
	public get ReferencesCommandModeChanged(): vscode.Event<refs.ReferencesCommandMode> {
		return this.model.referencesCommandMode.ValueChanged;
	}
	public get TagParserStatusChanged(): vscode.Event<string> {
		return this.model.parsingWorkspaceStatus.ValueChanged;
	}
	public get ActiveConfigChanged(): vscode.Event<string> {
		return this.model.activeConfigName.ValueChanged;
	}
	public isInitialized(): boolean {
		return this.innerLanguageClient !== undefined;
	}
	public getShowConfigureIntelliSenseButton(): boolean {
		return this.showConfigureIntelliSenseButton;
	}
	public setShowConfigureIntelliSenseButton(show: boolean): void {
	...
	public get configuration(): configs.CppProperties {
		if (!this.innerConfiguration) {
			throw new Error(
				"Attempting to use configuration before initialized",
			);
		}
	...
	private static readonly configurationProvidersLabel: string =
		"configuration providers";
	private static readonly compileCommandsLabel: string =
		"compile_commands.json";
	private static readonly compilersLabel: string = "compilers";

	public async showSelectIntelliSenseConfiguration(
		paths: string[],
		...
		for (let i: number = 0; i < paths.length; i++) {
			...
			const isCompiler: boolean =
				isCompilerSection && compilerName !== paths[i];

			if (isCompiler) {
				const path: string | undefined = paths[i].replace(
					compilerName,
					"",
				);
				const description: string = localize(
					"found.string",
					"Found at {0}",
					path,
				);
				...
				items.push({
					label: localize("compilers", "compilers"),
					index: i,
					kind: vscode.QuickPickItemKind.Separator,
				});
			} else {
	...
	public async handleIntelliSenseConfigurationQuickPick(
		sender?: any,
		showCompilersOnly?: boolean,
	): Promise<void> {
		const settings: CppSettings = new CppSettings(
			showCompilersOnly ? undefined : this.RootUri,
		);
		const paths: string[] = [];
		const configProviders: CustomConfigurationProvider1[] | undefined =
			showCompilersOnly
				? undefined
		...
		const configProviderCount: number =
			configProvidersIndex === 0 ? 0 : configProvidersIndex - 1;
		...
		const compileCommandsCount: number =
			compileCommandsIndex === configProvidersIndex
				? 0
				: compileCommandsIndex - configProvidersIndex - 1;
		paths.push(DefaultClient.compilersLabel);
		if (compilerDefaults?.knownCompilers !== undefined) {
			const tempPaths: string[] = compilerDefaults.knownCompilers.map(
				function (a: configs.KnownCompiler): string {
					return a.path;
				},
			);
		...
		const compilerCount: number =
			compilersIndex === compileCommandsIndex
				? 0
				: compilersIndex - compileCommandsIndex - 1;
		paths.push(
			localize(
		...
		if (isWindows && util.getSenderType(sender) !== "walkthrough") {
			...
			paths.push(
				localize("installCompiler.string.nix", "Install a compiler"),
			);
		} else {
			installShown = false;
		}
		...
		const index: number = await this.showSelectIntelliSenseConfiguration(
			paths,
			showCompilersOnly,
		);
		let action: string = "";
		...
		try {
			...
			if (index === paths.length - 1) {
				...
				return ui.ShowConfigureIntelliSenseButton(
					false,
					this,
					ConfigurationType.CompilerPath,
					"disablePrompt",
				);
			}
			...
			if (
				index === paths.length - 3 ||
				(!installShown && index === paths.length - 2)
			) {
				...
				if (index < configProvidersIndex && configProviders) {
					...
					const provider: CustomConfigurationProvider1 =
						configProviders[index - 1];
					await this.configuration.updateCustomConfigurationProvider(
						provider.extensionId,
					);
					void this.onCustomConfigurationProviderRegistered(
						provider,
					).catch(logAndReturn.undefined);
					telemetry.logLanguageServerEvent(
						"customConfigurationProvider",
						{ "providerId": provider.extensionId },
					);

					return ui.ShowConfigureIntelliSenseButton(
						false,
						this,
						ConfigurationType.ConfigProvider,
						showButtonSender,
					);
				} else if (index < compileCommandsIndex) {
					action = "select compile commands";
					await this.configuration.setCompileCommands(
						this.compileCommandsPaths[
							index - configProvidersIndex - 1
						],
					);
					return ui.ShowConfigureIntelliSenseButton(
						false,
						this,
						ConfigurationType.CompileCommands,
						showButtonSender,
					);
				} else {
					action = "select compiler";
					const newCompiler: string = util.isCl(paths[index])
						? "cl.exe"
						: paths[index];
			...
			await ui.ShowConfigureIntelliSenseButton(
				false,
				this,
				ConfigurationType.CompilerPath,
				showButtonSender,
			...
			if (showCompilersOnly) {
				telemetry.logLanguageServerEvent(
					"compilerSelection",
					{ action, sender: util.getSenderType(sender) },
					{ compilerCount: compilerCount + 3 },
				); // + 3 is to match what was being incorrectly sent previously
			} else {
				telemetry.logLanguageServerEvent(
			...
			const rootFolder: vscode.WorkspaceFolder | undefined =
				this.RootFolder;
			if (rootFolder && fromStatusBarButton) {
				if (configurationSelected || configProviderCount > 0) {
					const ask: PersistentFolderState<boolean> =
						new PersistentFolderState<boolean>(
				...
				if (configurationSelected || compileCommandsCount > 0) {
					const ask: PersistentFolderState<boolean> =
						new PersistentFolderState<boolean>(
							"CPP.showCompileCommandsSelection",
							true,
							rootFolder,
						);
					ask.Value = false;
	...
	public async rescanCompilers(sender?: any): Promise<void> {
		...
		DefaultClient.updateClientConfigurations();
		if (
			compilerDefaults.knownCompilers !== undefined &&
			compilerDefaults.knownCompilers.length > 0
		) {
	...
	async promptSelectIntelliSenseConfiguration(sender?: any): Promise<void> {
		...
		if (compilerDefaults.compilerPath !== "") {
			const showCompilersOnly: boolean =
				util.getSenderType(sender) === "walkthrough";
			return this.handleIntelliSenseConfigurationQuickPick(
				sender,
				showCompilersOnly,
			);
		}
	...
	constructor(workspaceFolder?: vscode.WorkspaceFolder) {
		if (workspaceFolder !== undefined) {
			this.lastCustomBrowseConfiguration = new PersistentFolderState<
				WorkspaceBrowseConfiguration | undefined
			>("CPP.lastCustomBrowseConfiguration", undefined, workspaceFolder);
			this.lastCustomBrowseConfigurationProviderId =
				new PersistentFolderState<string | undefined>(
					"CPP.lastCustomBrowseConfigurationProviderId",
					undefined,
					workspaceFolder,
				);
			this.lastCustomBrowseConfigurationProviderVersion =
				new PersistentFolderState<Version>(
					"CPP.lastCustomBrowseConfigurationProviderVersion",
					Version.v5,
					workspaceFolder,
				);
			this.registeredProviders = new PersistentFolderState<string[]>(
				"CPP.registeredProviders",
				[],
				workspaceFolder,
			);
			// If this provider did the register in the last session, clear out the cached browse config.
			if (
				!this.isProviderRegistered(
		...
		if (!semanticTokensLegend) {
			...
			semanticTokensLegend = new vscode.SemanticTokensLegend(
				tokenTypesLegend,
		...
		this.rootRealPath = this.RootPath
		...
		this.workspaceStoragePath =
			util.extensionContext?.storageUri?.fsPath ?? "";
		if (this.workspaceStoragePath.length > 0) {
			workspaceHash = path.basename(
				path.dirname(this.workspaceStoragePath),
			);
		} else {
			this.workspaceStoragePath = this.RootPath
				? path.join(this.RootPath, ".vscode")
				: "";
		}
		...
		if (
			...
			this.workspaceStoragePath = path.join(
				this.workspaceStoragePath,
				util.getUniqueWorkspaceStorageName(workspaceFolder),
			);
		}
		...
		try {
			...
			void this.init(rootUri, isFirstClient).catch(
	...
	private async init(
		...
		try {
			const workspaceFolder: vscode.WorkspaceFolder | undefined =
				this.rootFolder;
			this.innerConfiguration = new configs.CppProperties(
				this,
				rootUri,
				workspaceFolder,
			);
			this.innerConfiguration.ConfigurationsChanged((e) =>
				this.onConfigurationsChanged(e),
			);
			this.innerConfiguration.SelectionChanged((e) =>
				this.onSelectedConfigurationChanged(e),
			);
			this.innerConfiguration.CompileCommandsChanged((e) =>
				this.onCompileCommandsChanged(e),
			);
			this.disposables.push(this.innerConfiguration);

			this.innerLanguageClient = languageClient;
			telemetry.logLanguageServerEvent(
				"NonDefaultInitialCppSettings",
				this.settingsTracker.getUserModifiedSettings(),
			);
			...
			if (isFirstClient) {
				...
				this.currentCopilotHoverEnabled =
					new PersistentWorkspaceState<string>(
						"cpp.copilotHover",
						settings.copilotHover,
					);
				if (
					settings.copilotHover === "enabled" ||
					(settings.copilotHover === "default" &&
						(await telemetry.isFlightEnabled("CppCopilotHover")))
				) {
				...
				this.disposables.push(
					vscode.languages.registerRenameProvider(
						util.documentSelector,
						new RenameProvider(this),
					),
				);
				this.disposables.push(
					vscode.languages.registerReferenceProvider(
						util.documentSelector,
						new FindAllReferencesProvider(this),
					),
				);
				this.disposables.push(
					vscode.languages.registerWorkspaceSymbolProvider(
						new WorkspaceSymbolProvider(this),
					),
				);
				this.disposables.push(
				...
				this.disposables.push(
					vscode.languages.registerCallHierarchyProvider(
						util.documentSelector,
						new CallHierarchyProvider(this),
					),
				);
				...
				this.documentFormattingProviderDisposable =
					vscode.languages.registerDocumentFormattingEditProvider(
						util.documentSelector,
						new DocumentFormattingEditProvider(this),
					);
				this.formattingRangeProviderDisposable =
					vscode.languages.registerDocumentRangeFormattingEditProvider(
						util.documentSelector,
						new DocumentRangeFormattingEditProvider(this),
					);
				this.onTypeFormattingProviderDisposable =
					vscode.languages.registerOnTypeFormattingEditProvider(
						util.documentSelector,
						new OnTypeFormattingEditProvider(this),
						";",
						"}",
						"\n",
					);
				...
				this.codeFoldingProviderDisposable =
					vscode.languages.registerFoldingRangeProvider(
						util.documentSelector,
						this.codeFoldingProvider,
					);

				if (
					settings.isEnhancedColorizationEnabled &&
				...
				const cppEditors: vscode.TextEditor[] =
					vscode.window.visibleTextEditors.filter((e) =>
						util.isCpp(e.document),
					);
	...
	private getWorkspaceFolderSettings(
		...
		const result: WorkspaceFolderSettingsParams = {
			...
			intelliSenseCachePath: util.resolveCachePath(
				settings.intelliSenseCachePath,
			...
			clangFormatPath: util.resolveVariables(
				settings.clangFormatPath,
				this.AdditionalEnvironment,
			),
			clangFormatStyle: settings.clangFormatStyle
				? util.resolveVariables(
						settings.clangFormatStyle,
						this.AdditionalEnvironment,
					)
				: undefined,
			...
			clangTidyPath: util.resolveVariables(
				settings.clangTidyPath,
				this.AdditionalEnvironment,
			),
			clangTidyConfig: settings.clangTidyConfig,
			clangTidyFallbackConfig: settings.clangTidyFallbackConfig,
			clangTidyHeaderFilter:
				settings.clangTidyHeaderFilter !== null
					? util.resolveVariables(
							settings.clangTidyHeaderFilter,
							this.AdditionalEnvironment,
						)
					: null,
			clangTidyArgs: util.resolveVariablesArray(
				settings.clangTidyArgs,
				this.AdditionalEnvironment,
			),
			...
			vcFormatIndentMultiLineRelativeTo:
				settings.vcFormatIndentMultiLineRelativeTo,
			vcFormatIndentWithinParentheses:
				settings.vcFormatIndentWithinParentheses,
			vcFormatIndentPreserveWithinParentheses:
				settings.vcFormatIndentPreserveWithinParentheses,
			...
			vcFormatIndentLambdaBracesWhenParameter:
			...
			vcFormatIndentAccesSpecifiers:
				settings.vcFormatIndentAccessSpecifiers,
			vcFormatIndentNamespaceContents:
				settings.vcFormatIndentNamespaceContents,
			vcFormatIndentPreserveComments:
				settings.vcFormatIndentPreserveComments,
			vcFormatNewLineScopeBracesOnSeparateLines:
				settings.vcFormatNewlineScopeBracesOnSeparateLines,
			vcFormatNewLineBeforeOpenBraceNamespace:
				settings.vcFormatNewlineBeforeOpenBraceNamespace,
			vcFormatNewLineBeforeOpenBraceType:
				settings.vcFormatNewlineBeforeOpenBraceType,
			vcFormatNewLineBeforeOpenBraceFunction:
				settings.vcFormatNewlineBeforeOpenBraceFunction,
			vcFormatNewLineBeforeOpenBraceBlock:
				settings.vcFormatNewlineBeforeOpenBraceBlock,
			vcFormatNewLineBeforeOpenBraceLambda:
				settings.vcFormatNewlineBeforeOpenBraceLambda,
			...
			vcFormatNewLineBeforeWhileInDoWhile:
				settings.vcFormatNewlineBeforeWhileInDoWhile,
			vcFormatNewLineCloseBraceSameLineEmptyType:
				settings.vcFormatNewlineCloseBraceSameLineEmptyType,
			vcFormatNewLineCloseBraceSameLineEmptyFunction:
				settings.vcFormatNewlineCloseBraceSameLineEmptyFunction,
			vcFormatSpaceBeforeFunctionOpenParenthesis:
				settings.vcFormatSpaceBeforeFunctionOpenParenthesis,
			vcFormatSpaceWithinParameterListParentheses:
				settings.vcFormatSpaceWithinParameterListParentheses,
			vcFormatSpaceBetweenEmptyParameterListParentheses:
				settings.vcFormatSpaceBetweenEmptyParameterListParentheses,
			vcFormatSpaceAfterKeywordsInControlFlowStatements:
				settings.vcFormatSpaceAfterKeywordsInControlFlowStatements,
			vcFormatSpaceWithinControlFlowStatementParentheses:
				settings.vcFormatSpaceWithinControlFlowStatementParentheses,
			vcFormatSpaceBeforeLambdaOpenParenthesis:
				settings.vcFormatSpaceBeforeLambdaOpenParenthesis,
			vcFormatSpaceWithinCastParentheses:
				settings.vcFormatSpaceWithinCastParentheses,
			vcFormatSpaceAfterCastCloseParenthesis:
				settings.vcFormatSpaceAfterCastCloseParenthesis,
			vcFormatSpaceWithinExpressionParentheses:
				settings.vcFormatSpaceWithinExpressionParentheses,
			vcFormatSpaceBeforeBlockOpenBrace:
				settings.vcFormatSpaceBeforeBlockOpenBrace,
			vcFormatSpaceBetweenEmptyBraces:
				settings.vcFormatSpaceBetweenEmptyBraces,
			vcFormatSpaceBeforeInitializerListOpenBrace:
				settings.vcFormatSpaceBeforeInitializerListOpenBrace,
			vcFormatSpaceWithinInitializerListBraces:
				settings.vcFormatSpaceWithinInitializerListBraces,
			vcFormatSpacePreserveInInitializerList:
				settings.vcFormatSpacePreserveInInitializerList,
			vcFormatSpaceBeforeOpenSquareBracket:
				settings.vcFormatSpaceBeforeOpenSquareBracket,
			vcFormatSpaceWithinSquareBrackets:
				settings.vcFormatSpaceWithinSquareBrackets,
			vcFormatSpaceBeforeEmptySquareBrackets:
				settings.vcFormatSpaceBeforeEmptySquareBrackets,
			vcFormatSpaceBetweenEmptySquareBrackets:
				settings.vcFormatSpaceBetweenEmptySquareBrackets,
			vcFormatSpaceGroupSquareBrackets:
				settings.vcFormatSpaceGroupSquareBrackets,
			vcFormatSpaceWithinLambdaBrackets:
				settings.vcFormatSpaceWithinLambdaBrackets,
			vcFormatSpaceBetweenEmptyLambdaBrackets:
				settings.vcFormatSpaceBetweenEmptyLambdaBrackets,
			vcFormatSpaceBeforeComma: settings.vcFormatSpaceBeforeComma,
			vcFormatSpaceAfterComma: settings.vcFormatSpaceAfterComma,
			vcFormatSpaceRemoveAroundMemberOperators:
				settings.vcFormatSpaceRemoveAroundMemberOperators,
			vcFormatSpaceBeforeInheritanceColon:
				settings.vcFormatSpaceBeforeInheritanceColon,
			vcFormatSpaceBeforeConstructorColon:
				settings.vcFormatSpaceBeforeConstructorColon,
			vcFormatSpaceRemoveBeforeSemicolon:
				settings.vcFormatSpaceRemoveBeforeSemicolon,
			vcFormatSpaceInsertAfterSemicolon:
				settings.vcFormatSpaceInsertAfterSemicolon,
			vcFormatSpaceRemoveAroundUnaryOperator:
				settings.vcFormatSpaceRemoveAroundUnaryOperator,
			vcFormatSpaceAroundBinaryOperator:
				settings.vcFormatSpaceAroundBinaryOperator,
			vcFormatSpaceAroundAssignmentOperator:
				settings.vcFormatSpaceAroundAssignmentOperator,
			vcFormatSpacePointerReferenceAlignment:
				settings.vcFormatSpacePointerReferenceAlignment,
			vcFormatSpaceAroundTernaryOperator:
				settings.vcFormatSpaceAroundTernaryOperator,
			...
			editorParameterHintsEnabled:
				otherSettings.editorParameterHintsEnabled,
	...
	private getAllWorkspaceFolderSettings(): WorkspaceFolderSettingsParams[] {
		...
		if (
			...
			for (const workspaceFolder of vscode.workspace.workspaceFolders) {
				workspaceFolderSettingsParams.push(
					this.getWorkspaceFolderSettings(
						workspaceFolder.uri,
						new CppSettings(workspaceFolder.uri),
						new OtherSettings(workspaceFolder.uri),
					),
				);
			}
		} else {
	...
	private getAllSettings(): SettingsParams {
		...
		const workspaceOtherSettings: OtherSettings = new OtherSettings();
		const workspaceFolderSettingsParams: WorkspaceFolderSettingsParams[] =
			this.getAllWorkspaceFolderSettings();
		if (
			this.currentCaseSensitiveFileSupport &&
			workspaceSettings.isCaseSensitiveFileSupportEnabled !==
				this.currentCaseSensitiveFileSupport.Value
		) {
			void util.promptForReloadWindowDueToSettingsChange();
		}
		if (
			this.currentCopilotHoverEnabled &&
			workspaceSettings.copilotHover !==
				this.currentCopilotHoverEnabled.Value
		) {
			void util.promptForReloadWindowDueToSettingsChange();
		}
		return {
			...
			workspaceParsingPriority:
				workspaceSettings.workspaceParsingPriority,
			workspaceSymbols: workspaceSettings.workspaceSymbols,
			simplifyStructuredComments:
				workspaceSettings.simplifyStructuredComments,
			intelliSenseUpdateDelay: workspaceSettings.intelliSenseUpdateDelay,
			experimentalFeatures: workspaceSettings.experimentalFeatures,
			enhancedColorization:
				workspaceSettings.isEnhancedColorizationEnabled,
			intellisenseMaxCachedProcesses:
				workspaceSettings.intelliSenseMaxCachedProcesses,
			intellisenseMaxMemory: workspaceSettings.intelliSenseMaxMemory,
			referencesMaxConcurrentThreads:
				workspaceSettings.referencesMaxConcurrentThreads,
			referencesMaxCachedProcesses:
				workspaceSettings.referencesMaxCachedProcesses,
			referencesMaxMemory: workspaceSettings.referencesMaxMemory,
			codeAnalysisMaxConcurrentThreads:
				workspaceSettings.codeAnalysisMaxConcurrentThreads,
	...
	private async createLanguageClient(): Promise<void> {
		this.currentCaseSensitiveFileSupport =
			new PersistentWorkspaceState<boolean>(
				"CPP.currentCaseSensitiveFileSupport",
				false,
			);
		...
		const serverOptions: ServerOptions = {
			run: {
				...
				options: {
					detached: false,
					cwd: util.getExtensionFilePath("bin"),
				},
			},
			debug: {
				command: serverModule,
		...
		const databaseStoragePath: string =
			cacheStoragePath.length > 0 && workspaceHash.length > 0
		...
		const cppInitializationParams: CppInitializationParams = {
			...
			caseSensitiveFileSupport:
				workspaceSettings.isCaseSensitiveFileSupportEnabled,
			resetDatabase: resetDatabase,
			edgeMessagesDirectory: path.join(
				util.getExtensionFilePath("bin"),
				"messages",
				getLocaleId(),
			),
		...
		this.loggingLevel = util.getNumericLoggingLevel(
			cppInitializationParams.settings.loggingLevel,
		);
					...
					if (languageClientCrashTimes.length < 5) {
						...
						const elapsed: number =
							languageClientCrashTimes[
								languageClientCrashTimes.length - 1
							] - languageClientCrashTimes[0];
	...
	public async sendDidChangeSettings(): Promise<void> {
		...
		await this.languageClient.sendNotification(
			DidChangeSettingsNotification,
			this.getAllSettings(),
		);
	}

	public async onDidChangeSettings(
		...
		const changedSettings: Record<string, string> =
			this.settingsTracker.getChangedSettings();
		...
		if (Object.keys(changedSettings).length > 0) {
			if (this === defaultClient) {
				...
				if (changedSettings.loggingLevel) {
					const oldLoggingLevelLogged: boolean =
						this.loggingLevel > 1;
					this.loggingLevel = util.getNumericLoggingLevel(
						changedSettings.loggingLevel,
					);
				...
				if (
					[
						"inlayHints.autoDeclarationTypes.enabled",
						...
						"inlayHints.referenceOperator.showSpace",
					].some((setting) => setting in changedSettings)
				) {
					vscode.window.visibleTextEditors.forEach(
						(visibleEditor: vscode.TextEditor) => {
							// The exact range doesn't matter.
							const visibleRange: vscode.Range | undefined =
								visibleEditor.visibleRanges.at(0);
							if (visibleRange !== undefined) {
								void vscode.commands.executeCommand<
									vscode.InlayHint[]
								>(
									"vscode.executeInlayHintProvider",
									visibleEditor.document.uri,
									visibleRange,
								);
							}
						},
				...
				if (
					changedSettings["default.configurationProvider"] !==
					undefined
				) {
					void ui.ShowConfigureIntelliSenseButton(
						false,
						this,
						ConfigurationType.ConfigProvider,
						showButtonSender,
					...
					changedSettings["default.compileCommands"] !== undefined
				) {
					void ui.ShowConfigureIntelliSenseButton(
						false,
						this,
			...
			if (
				changedSettings["default.compilerPath"] !== undefined ||
				changedSettings["default.compileCommands"] !== undefined ||
				changedSettings["default.configurationProvider"] !== undefined
			) {
				void ui
					.ShowConfigureIntelliSenseButton(false, this)
					.catch(logAndReturn.undefined);
			}
	...
	private prepareVisibleRanges(editors: readonly vscode.TextEditor[]): {
		[uri: string]: Range[];
	} {
		...
		editors.forEach((editor) => {
			...
			visibleRanges[uri] = visibleRanges[uri].concat(
				editor.visibleRanges.map(makeLspRange),
			);
		});
		...
		Object.keys(visibleRanges).forEach((uri) => {
			visibleRanges[uri] = util.mergeOverlappingRanges(
				visibleRanges[uri],
			);
		});
	...
	public async onDidChangeVisibleTextEditors(
		editors: readonly vscode.TextEditor[],
	): Promise<void> {
		...
		if (vscode.window.activeTextEditor) {
			if (util.isCpp(vscode.window.activeTextEditor.document)) {
				...
				params.activeSelection = makeLspRange(
		...
		await this.languageClient.sendNotification(
			DidChangeVisibleTextEditorsNotification,
			params,
		);
	}

	public async onDidChangeTextEditorVisibleRanges(
		uri: vscode.Uri,
	): Promise<void> {
		...
		if (editors.length === 1) {
			...
			editors.forEach((editor) => {
				// Use a map, to account for multiple editors for the same file.
				// First, we just concat all ranges for the same file.
				visibleRanges = visibleRanges.concat(
					editor.visibleRanges.map(makeLspRange),
				);
			});
		}
		...
		await this.languageClient.sendNotification(
			DidChangeTextEditorVisibleRangesNotification,
			params,
		);
	}
		...
		if (util.isCpp(textDocumentChangeEvent.document)) {
			...
			if (workspaceReferences.renamePending) {
				workspaceReferences.cancelCurrentReferenceRequest(
					refs.CancellationSender.User,
				);
			}

			const oldVersion: number | undefined = openFileVersions.get(
	...
	public onDidOpenTextDocument(document: vscode.TextDocument): void {
		if (document.uri.scheme === "file") {
			...
			void SessionState.buildAndDebugIsSourceFile.set(
				util.isCppOrCFile(document.uri),
			);
			void SessionState.buildAndDebugIsFolderOpen.set(
				util.isFolderOpen(document.uri),
			);
		} else {
	...
	public isProviderRegistered(extensionId: string | undefined): boolean {
		if (
			...
			this.registeredProviders === undefined
		) {
			return false;
		}
	...
	private async onCustomConfigurationProviderRegistered(
		provider: CustomConfigurationProvider1,
	): Promise<void> {
	...
	public async onRegisterCustomConfigurationProvider(
		provider: CustomConfigurationProvider1,
	): Promise<void> {
		await this.ready;

		if (
			this.registeredProviders === undefined || // Shouldn't happen.
		...
		if (isSameProviderExtensionId(selectedProvider, provider.extensionId)) {
			void this.onCustomConfigurationProviderRegistered(provider).catch(
				logAndReturn.undefined,
			);
			telemetry.logLanguageServerEvent("customConfigurationProvider", {
				"providerId": provider.extensionId,
			});
			...
			await this.configuration.updateCustomConfigurationProvider(
				provider.extensionId,
			); // v0 -> v1 upgrade. Update the configurationProvider in c_cpp_properties.json
		}
	}

	public async updateCustomConfigurations(
		requestingProvider?: CustomConfigurationProvider1,
	): Promise<void> {
		await this.ready;
		...
		const currentProvider: CustomConfigurationProvider1 | undefined =
			getCustomConfigProviders().get(this.configurationProvider);
		if (!currentProvider) {
			return this.clearCustomConfigurations();
		}
		if (
			requestingProvider &&
			requestingProvider.extensionId !== currentProvider.extensionId
		) {
	...
	public async updateCustomBrowseConfiguration(
		requestingProvider?: CustomConfigurationProvider1,
	): Promise<void> {
		await this.ready;
		...
		try {
			if (
				this.RootUri &&
				(await currentProvider.canProvideBrowseConfigurationsPerFolder(
					tokenSource.token,
				))
			) {
				config = await currentProvider.provideFolderBrowseConfiguration(
					this.RootUri,
					tokenSource.token,
				);
			} else if (
				await currentProvider.canProvideBrowseConfiguration(
					tokenSource.token,
				)
			) {
				config = await currentProvider.provideBrowseConfiguration(
					tokenSource.token,
				);
			...
			if (currentProvider.version < Version.v3) {
				...
				for (const c of config.browsePath) {
					if (
						...
						this.sendCustomBrowseConfiguration(
							config,
							currentProvider.extensionId,
		...
		global.setTimeout(() => {
			if (!hasCompleted) {
				...
				if (currentProvider.version >= Version.v2) {
					console.warn(
						`Configuration Provider timed out in ${configProviderTimeout}ms.`,
					);
	...
	public async logDiagnostics(): Promise<void> {
		await this.ready;
		const response: GetDiagnosticsResult =
			await this.languageClient.sendRequest(GetDiagnosticsRequest, null);
		const diagnosticsChannel: vscode.OutputChannel =
			getDiagnosticsChannel();
		...
		if (userModifiedSettings.length > 0) {
			...
			for (const [key] of userModifiedSettings) {
				...
				const realKey = key.endsWith("2")
					? key.slice(0, key.length - 1)
					: key;
				const fullKey = `C_Cpp.${realKey}`;
				settings[fullKey] =
					vscode.workspace.getConfiguration("C_Cpp").get(realKey) ??
...

		{
			...
			settings.editorAutoClosingBrackets =
				editorSettings.editorAutoClosingBrackets;
			...
			settings.filesAutoSaveAfterDelay =
				editorSettings.filesAutoSaveAfterDelay;
			settings.editorInlayHintsEnabled =
				editorSettings.editorInlayHintsEnabled;
			settings.editorParameterHintsEnabled =
				editorSettings.editorParameterHintsEnabled;
			settings.searchExclude = editorSettings.searchExclude;
			settings.workbenchSettingsEditor =
				editorSettings.workbenchSettingsEditor;
			configJson += `Additional Tracked Settings:\n${JSON.stringify(settings, null, 4)}\n`;
		}

		// Get diagnostics for configuration provider info.
		let configurationLoggingStr: string = "";
		const tuSearchStart: number = response.diagnostics.indexOf(
			"Translation Unit Mappings:",
		);
		if (tuSearchStart >= 0) {
			const tuSearchEnd: number = response.diagnostics.indexOf(
				"Translation Unit Configurations:",
			);
			if (tuSearchEnd >= 0 && tuSearchEnd > tuSearchStart) {
				let tuSearchString: string = response.diagnostics.substring(
					tuSearchStart,
					tuSearchEnd,
				);
				let tuSearchIndex: number = tuSearchString.indexOf("[");
				while (tuSearchIndex >= 0) {
					const tuMatch: RegExpMatchArray | null =
						tuSearchString.match(/\[\s(.*)\s\]/);
					if (tuMatch && tuMatch.length > 1) {
						const tuPath: string = vscode.Uri.file(
							tuMatch[1],
						).toString();
						if (this.configurationLogging.has(tuPath)) {
							if (configurationLoggingStr.length === 0) {
								configurationLoggingStr +=
		...
		diagnosticsChannel.appendLine(
			`${header}${version}${configJson}${this.browseConfigurationLogging}${configurationLoggingStr}${response.diagnostics}`,
		);
	...
	public async provideCustomConfiguration(docUri: vscode.Uri): Promise<void> {
		...
		try {
			...
			const provider: CustomConfigurationProvider1 | undefined =
				getCustomConfigProviders().get(providerId);
			...
			telemetry.logLanguageServerEvent("provideCustomConfiguration", {
				providerId,
				resultCode,
			});
		} finally {
	...
	private async provideCustomConfigurationAsync(
		docUri: vscode.Uri,
		provider: CustomConfigurationProvider1,
	): Promise<string> {
		const tokenSource: vscode.CancellationTokenSource =
		...
		const provideConfigurationAsync: () => Thenable<
			SourceFileConfigurationItem[] | undefined
		> = async () => {
			try {
				if (
					!(await provider.canProvideConfiguration(
						docUri,
						tokenSource.token,
					))
				) {
			...
			try {
				configs = await provider.provideConfigurations(
					[docUri],
					tokenSource.token,
				);
			} catch (err) {
					...
					configs.forEach((config) => {
						if (fileConfiguration.includePath) {
							fileConfiguration.includePath.forEach((p) => {
								if (
									!config.configuration.includePath.includes(
										p,
									)
								) {
						...
						if (fileConfiguration.forcedInclude) {
							fileConfiguration.forcedInclude.forEach((i) => {
								if (config.configuration.forcedInclude) {
									if (
										!config.configuration.forcedInclude.includes(
											i,
										)
									) {
										config.configuration.forcedInclude.push(
											i,
										);
									}
								}
		...
		try {
			const configs: SourceFileConfigurationItem[] | undefined =
				await this.callTaskWithTimeout(
			...
			if (
				settings.isConfigurationWarningsEnabled &&
				!this.isExternalHeader(docUri) &&
				!vscode.debug.activeDebugSession
			) {
				const dismiss: string = localize("dismiss.button", "Dismiss");
				const disable: string = localize(
					"disable.warnings.button",
					"Disable Warnings",
				);
				const configName: string | undefined =
					this.configuration.CurrentConfiguration?.name;
				if (!configName) {
					return "noConfigName";
				}
	...
	private handleRequestCustomConfig(file: string): void {
		...
		if (client instanceof DefaultClient) {
			const defaultClient: DefaultClient = client as DefaultClient;
			void defaultClient
				.provideCustomConfiguration(uri)
				.catch(logAndReturn.undefined);
		}
	}
	...
	public async getCurrentConfigCustomVariable(
		variableName: string,
	): Promise<string> {
		await this.ready;
		return (
			this.configuration.CurrentConfiguration
				?.customConfigurationVariables?.[variableName] ?? ""
		);
	}

	public async setCurrentConfigName(
		configurationName: string,
	): Promise<void> {
		await this.ready;

		const configurations: configs.Configuration[] =
			this.configuration.Configurations ?? [];
		const configurationIndex: number = configurations.findIndex(
	...
	public async getCurrentCompilerPathAndArgs(): Promise<
		util.CompilerPathAndArgs | undefined
	> {
		const settings: CppSettings = new CppSettings(this.RootUri);
		await this.ready;
		return util.extractCompilerPathAndArgs(
			!!settings.legacyCompilerArgsBehavior,
	...
	public async getKnownCompilers(): Promise<
		configs.KnownCompiler[] | undefined
	> {
		await this.ready;
	...
	get ready(): Promise<void> {
		if (
			!DefaultClient.dispatching.isCompleted ||
			DefaultClient.queue.length
		) {
	...
	enqueue<T>(task: () => Promise<T>) {
		ok(
			this.isSupported,
			localize("unsupported.client", "Unsupported client"),
		);
	...
	private static async withLspCancellationHandling<T>(
		task: () => Promise<T>,
		token: vscode.CancellationToken,
	): Promise<T> {
		let result: T;
	...
	private registerNotifications(): void {
		console.assert(
			...
			'This method must not be called until this.languageClient is set in "onReady"',
		);

		this.languageClient.onNotification(
			ReloadWindowNotification,
			() => void util.promptForReloadWindowDueToSettingsChange(),
		);
		this.languageClient.onNotification(
			UpdateTrustedCompilersNotification,
			(e) => void this.addTrustedCompiler(e.compilerPath),
		);
		this.languageClient.onNotification(LogTelemetryNotification, (e) =>
			this.logTelemetry(e),
		);
		this.languageClient.onNotification(
			ReportStatusNotification,
			(e) => void this.updateStatus(e),
		);
		this.languageClient.onNotification(
			ReportTagParseStatusNotification,
			(e) => this.updateTagParseStatus(e),
		);
		this.languageClient.onNotification(
			CompileCommandsPathsNotification,
			(e) => void this.promptCompileCommands(e),
		);
		this.languageClient.onNotification(ReferencesNotification, (e) =>
			this.processReferencesPreview(e),
		);
		this.languageClient.onNotification(
			ReportReferencesProgressNotification,
			(e) => this.handleReferencesProgress(e),
		);
		this.languageClient.onNotification(RequestCustomConfig, (e) =>
			this.handleRequestCustomConfig(e),
		);
		this.languageClient.onNotification(
			IntelliSenseResultNotification,
			(e) => this.handleIntelliSenseResult(e),
		);
		this.languageClient.onNotification(
			PublishRefactorDiagnosticsNotification,
			publishRefactorDiagnostics,
		);
		RegisterCodeAnalysisNotifications(this.languageClient);
		this.languageClient.onNotification(
			ShowMessageWindowNotification,
			showMessageWindow,
		);
		this.languageClient.onNotification(
		...
		this.languageClient.onNotification(ReportTextDocumentLanguage, (e) =>
			this.setTextDocumentLanguage(e),
		);
		this.languageClient.onNotification(IntelliSenseSetupNotification, (e) =>
			this.logIntelliSenseSetupTime(e),
		);
		this.languageClient.onNotification(
			SetTemporaryTextDocumentLanguageNotification,
			(e) => void this.setTemporaryTextDocumentLanguage(e),
		);
		this.languageClient.onNotification(
			ReportCodeAnalysisProcessedNotification,
			(e) => this.updateCodeAnalysisProcessed(e),
		);
		this.languageClient.onNotification(
			ReportCodeAnalysisTotalNotification,
			(e) => this.updateCodeAnalysisTotal(e),
		);
		this.languageClient.onNotification(
			DoxygenCommentGeneratedNotification,
			(e) => void this.insertDoxygenComment(e),
		);
		this.languageClient.onNotification(
			CanceledReferencesNotification,
			this.serverCanceledReferences,
		);
	}

	private handleIntelliSenseResult(
		intelliSenseResult: IntelliSenseResult,
	): void {
		const fileVersion: number | undefined = openFileVersions.get(
			intelliSenseResult.uri,
		);
		if (
			fileVersion !== undefined &&
			fileVersion !== intelliSenseResult.fileVersion
		) {
			return;
		}

		if (this.semanticTokensProvider) {
			this.semanticTokensProvider.deliverTokens(
				intelliSenseResult.uri,
				intelliSenseResult.semanticTokens,
				intelliSenseResult.clearExistingSemanticTokens,
			);
		}
		if (this.inlayHintsProvider) {
			this.inlayHintsProvider.deliverInlayHints(
				intelliSenseResult.uri,
				intelliSenseResult.inlayHints,
				intelliSenseResult.clearExistingInlayHint,
			);
		}

		this.updateInactiveRegions(
			intelliSenseResult.uri,
			intelliSenseResult.inactiveRegions,
			intelliSenseResult.clearExistingInactiveRegions,
			intelliSenseResult.isCompletePass,
		);
		if (
			intelliSenseResult.clearExistingDiagnostics ||
			intelliSenseResult.diagnostics.length > 0
		) {
			this.updateSquiggles(
				intelliSenseResult.uri,
				intelliSenseResult.diagnostics,
				intelliSenseResult.clearExistingDiagnostics,
			);
		}
	}

	private updateSquiggles(
		uriString: string,
		diagnostics: IntelliSenseDiagnostic[],
		startNewSet: boolean,
	): void {
	...
	private setTextDocumentLanguage(languageStr: string): void {
		...
		if (cppSettings.autoAddFileAssociations) {
			...
			this.addFileAssociations(
				languageStr,
				is_c ? "c" : is_cuda ? "cuda-cpp" : "cpp",
			);
		}
	}

	private async setTemporaryTextDocumentLanguage(
		params: SetTemporaryTextDocumentLanguageParams,
	): Promise<void> {
		const languageId: string = params.isC
			? "c"
			: params.isCuda
				? "cuda-cpp"
				: "cpp";
		...
		const document: vscode.TextDocument | undefined =
			client.TrackedDocuments.get(params.uri);
		...
		if (this.rootFolder) {
			...
			this.rootPathFileWatcher.onDidCreate(async (uri) => {
				...
				void this.languageClient
					.sendNotification(FileCreatedNotification, {
						uri: uri.toString(),
					})
					.catch(logAndReturn.undefined);
			});
			...
			this.rootPathFileWatcher.onDidChange(async (uri) => {
				...
				if (dotIndex !== -1) {
					...
					if (this.associations_for_did_change?.has(ext)) {
						...
						if (duration < 10000) {
							void this.languageClient
								.sendNotification(FileChangedNotification, {
									uri: uri.toString(),
								})
								.catch(logAndReturn.undefined);
						}
			...
			this.rootPathFileWatcher.onDidDelete((uri) => {
				...
				void this.languageClient
					.sendNotification(FileDeletedNotification, {
						uri: uri.toString(),
					})
					.catch(logAndReturn.undefined);
			});
	...
	private logTelemetry(notificationBody: TelemetryPayload): void {
		if (
			notificationBody.event === "includeSquiggles" &&
			this.configurationProvider &&
			notificationBody.properties
		) {
			notificationBody.properties["providerId"] =
				this.configurationProvider;
		}
		telemetry.logLanguageServerEvent(
			notificationBody.event,
			notificationBody.properties,
			notificationBody.metrics,
		);
	}

	private async updateStatus(
		notificationBody: ReportStatusNotificationBody,
	): Promise<void> {
		...
		if (message.endsWith("Idle")) {
			...
			const status: IntelliSenseStatus = {
				status: Status.IntelliSenseReady,
			};
			testHook.updateStatus(status);
	...
	private updateTagParseStatus(tagParseStatus: TagParseStatus): void {
		this.model.parsingWorkspaceStatus.Value = getLocalizedString(
			tagParseStatus.localizeStringParams,
		);
		this.model.isParsingWorkspacePaused.Value = tagParseStatus.isPaused;
	}

	private updateInactiveRegions(
		uriString: string,
		inactiveRegions: InputRegion[],
		startNewSet: boolean,
		updateFoldingRanges: boolean,
	): void {
		...
		if (
			!(client instanceof DefaultClient) ||
			(!startNewSet && inactiveRegions.length === 0)
		) {
			return;
		}
		const settings: CppSettings = new CppSettings(client.RootUri);
		const dimInactiveRegions: boolean = settings.dimInactiveRegions;
		let currentSet: DecorationRangesPair | undefined =
			this.inactiveRegionsDecorations.get(uriString);
		...
		if (currentSet === undefined) {
			...
			currentSet = {
				decoration: vscode.window.createTextEditorDecorationType({
					opacity:
						opacity === undefined ? "0.55" : opacity.toString(),
		...
		Array.prototype.push.apply(
			...
			inactiveRegions.map(
				(element) =>
					new vscode.Range(element.startLine, 0, element.endLine, 0),
			),
		);
	...
	public logIntelliSenseSetupTime(notification: IntelliSenseSetup): void {
		clients.timeTelemetryCollector.setSetupTime(
			vscode.Uri.parse(notification.uri),
		);
	}

	private compileCommandsPaths: string[] = [];
	private async promptCompileCommands(
		params: CompileCommandsPaths,
	): Promise<void> {
		if (!params.workspaceFolderUri) {
			return;
		}
		const potentialClient: Client = clients.getClientFor(
			vscode.Uri.file(params.workspaceFolderUri),
		);
	...
	public async handleConfigStatus(sender?: string): Promise<void> {
		if (
			!this.configStateReceived.timeout &&
			(!this.configStateReceived.compilers ||
				!this.configStateReceived.compileCommands ||
				!this.configStateReceived.configProviders)
		) {
		...
		const rootFolder: vscode.WorkspaceFolder | undefined = this.RootFolder;
		const settings: CppSettings = new CppSettings(this.RootUri);
		const configProviderNotSet: boolean =
			!settings.defaultConfigurationProvider &&
			!this.configuration.CurrentConfiguration?.configurationProvider &&
			!this.configuration.CurrentConfiguration
				?.configurationProviderInCppPropertiesJson;
		const configProviderNotSetAndNoCache: boolean =
			configProviderNotSet &&
			this.lastCustomBrowseConfigurationProviderId?.Value === undefined;
		const compileCommandsNotSet: boolean =
			!settings.defaultCompileCommands &&
			!this.configuration.CurrentConfiguration?.compileCommands &&
			!this.configuration.CurrentConfiguration
				?.compileCommandsInCppPropertiesJson;

		// Handle config providers
		const provider: CustomConfigurationProvider1 | undefined = !this
			.configStateReceived.configProviders
			? undefined
			: this.configStateReceived.configProviders.length === 0
				? undefined
				: this.configStateReceived.configProviders[0];
		let showConfigStatus: boolean = false;
		if (
			rootFolder &&
			configProviderNotSetAndNoCache &&
			provider &&
			sender === "configProviders"
		) {
			const ask: PersistentFolderState<boolean> =
				new PersistentFolderState<boolean>(
					"Client.registerProvider",
		...
		if (
			...
			configProviderNotSetAndNoCache &&
			!this.configStateReceived.configProviders &&
			compileCommandsNotSet &&
			this.compileCommandsPaths.length > 0 &&
			sender === "compileCommands"
		) {
			const ask: PersistentFolderState<boolean> =
				new PersistentFolderState<boolean>(
					"CPP.showCompileCommandsSelection",
		...
		const compilerPathNotSet: boolean =
			settings.defaultCompilerPath === null &&
			this.configuration.CurrentConfiguration?.compilerPath ===
				undefined &&
			this.configuration.CurrentConfiguration
				?.compilerPathInCppPropertiesJson === undefined;
		const configurationNotSet: boolean =
			configProviderNotSetAndNoCache &&
			compileCommandsNotSet &&
			compilerPathNotSet;
		...
		showConfigStatus =
			...
			(configurationNotSet &&
				...
				(trustedCompilerPaths.length !== 1 ||
					trustedCompilerPaths[0] !== ""));

		const configProviderType: ConfigurationType = this.configuration
			.ConfigProviderAutoSelected
			? ConfigurationType.AutoConfigProvider
			: ConfigurationType.ConfigProvider;
		const compilerType: ConfigurationType = this.configuration
			.CurrentConfiguration?.compilerPathIsExplicit
			? ConfigurationType.CompilerPath
			: ConfigurationType.AutoCompilerPath;
		const configType: ConfigurationType = !configProviderNotSet
			? configProviderType
			: !compileCommandsNotSet
				? ConfigurationType.CompileCommands
				: !compilerPathNotSet
	 ...
	 */
	public async requestSwitchHeaderSource(
		rootUri: vscode.Uri,
		fileName: string,
	): Promise<string> {
		...
		return this.enqueue(async () =>
			this.languageClient.sendRequest(SwitchHeaderSourceRequest, params),
		);
	}

	public async requestCompiler(
		newCompilerPath?: string,
	): Promise<configs.CompilerDefaults> {
		const params: QueryDefaultCompilerParams = {
			newTrustedCompilerPath: newCompilerPath ?? "",
		};
		const results: configs.CompilerDefaults =
			await this.languageClient.sendRequest(
				QueryCompilerDefaultsRequest,
				params,
			);
		void SessionState.scanForCompilersDone.set(true);
		void SessionState.scanForCompilersEmpty.set(
			results.knownCompilers === undefined ||
				!results.knownCompilers.length,
		);
		void SessionState.trustedCompilerFound.set(
			results.trustedCompilerFound,
		);
		return results;
	}
	...
	public updateActiveDocumentTextOptions(): void {
		...
		if (editor && util.isCpp(editor.document)) {
			void SessionState.buildAndDebugIsSourceFile.set(
				util.isCppOrCFile(editor.document.uri),
			);
			void SessionState.buildAndDebugIsFolderOpen.set(
				util.isFolderOpen(editor.document.uri),
			);
			...
			if (settings.useVcFormat(editor.document)) {
				...
				if (
					editorConfigSettings.indent_style === "space" ||
					editorConfigSettings.indent_style === "tab"
				) {
					...
					if (editorConfigSettings.indent_size === "tab") {
						if (!editorConfigSettings.tab_width !== undefined) {
							editor.options.tabSize =
								editorConfigSettings.tab_width;
						}
					} else if (editorConfigSettings.indent_size !== undefined) {
						editor.options.tabSize =
							editorConfigSettings.indent_size;
					}
				}
				if (editorConfigSettings.end_of_line !== undefined) {
					void editor
						.edit((edit) => {
							edit.setEndOfLine(
								editorConfigSettings.end_of_line === "lf"
									? vscode.EndOfLine.LF
	 ...
	 */
	public async didChangeActiveEditor(
		editor?: vscode.TextEditor,
	): Promise<void> {
		...
		return this.languageClient
			.sendNotification(DidChangeActiveEditorNotification, params)
			.catch(logAndReturn.undefined);
	}
	 ...
	 */
	public async restartIntelliSenseForFile(
		document: vscode.TextDocument,
	): Promise<void> {
		await this.ready;
		return this.languageClient
			.sendNotification(
				RestartIntelliSenseForFileNotification,
	...
	public async selectionChanged(selection: Range): Promise<void> {
		return this.languageClient.sendNotification(
			DidChangeTextEditorSelectionNotification,
			selection,
		);
	}
	...
	public async ResumeCodeAnalysis(): Promise<void> {
		...
		return this.languageClient.sendNotification(
			ResumeCodeAnalysisNotification,
		);
	}
	...
	public async CancelCodeAnalysis(): Promise<void> {
		await this.ready;
		return this.languageClient.sendNotification(
			CancelCodeAnalysisNotification,
		);
	}
	...
	private async insertDoxygenComment(
		result: GenerateDoxygenCommentResult,
	): Promise<void> {
		const editor: vscode.TextEditor | undefined =
			vscode.window.activeTextEditor;
		if (!editor) {
		...
		const currentFileVersion: number | undefined = openFileVersions.get(
		...
		if (
			...
			const newPosition: vscode.Position = new vscode.Position(
				result.finalCursorPosition.line,
				result.finalCursorPosition.character,
			);
	...
	private async onConfigurationsChanged(
		cppProperties: configs.CppProperties,
	): Promise<void> {
		...
		configurations.forEach((c) => {
			...
			if (settings.legacyCompilerArgsBehavior) {
				modifiedConfig.compilerArgsLegacy =
					compilerPathAndArgs.allCompilerArgs;
				modifiedConfig.compilerArgs = undefined;
			} else {
				modifiedConfig.compilerArgs =
					compilerPathAndArgs.allCompilerArgs;
			}
		...
		if (
			...
			if (!this.doneInitialCustomBrowseConfigurationCheck) {
				...
				if (this.lastCustomBrowseConfiguration.Value) {
					this.sendCustomBrowseConfiguration(
						this.lastCustomBrowseConfiguration.Value,
						this.lastCustomBrowseConfigurationProviderId.Value,
						this.lastCustomBrowseConfigurationProviderVersion.Value,
					);
		...
		const configName: string | undefined =
			configurations[params.currentConfiguration].name ?? "";
		this.model.activeConfigName.setValueIfActive(configName);
		const newProvider: string | undefined =
			this.configuration.CurrentConfigurationProvider;
		if (
			!isSameProviderExtensionId(newProvider, this.configurationProvider)
		) {
			if (this.configurationProvider) {
				void this.clearCustomBrowseConfiguration().catch(
					logAndReturn.undefined,
				);
			}
			this.configurationProvider = newProvider;
			void this.updateCustomBrowseConfiguration().catch(
				logAndReturn.undefined,
			);
			void this.updateCustomConfigurations().catch(
				logAndReturn.undefined,
			);
		}
	}
	...
	private async onSelectedConfigurationChanged(index: number): Promise<void> {
		...
		await this.ready;
		await this.languageClient.sendNotification(
			ChangeSelectedSettingNotification,
			params,
		);
	...
	private async onCompileCommandsChanged(path: string): Promise<void> {
		...
		await this.ready;
		return this.languageClient.sendNotification(
			ChangeCompileCommandsNotification,
			params,
		);
	}

	private isSourceFileConfigurationItem(
		input: any,
		providerVersion: Version,
	): input is SourceFileConfigurationItem {
		...
		if (providerVersion < Version.v5) {
			...
			areOptionalsValid =
				util.isOptionalString(input.configuration.intelliSenseMode) &&
				util.isOptionalString(input.configuration.standard);
		}
		return (
			input &&
			(util.isString(input.uri) || util.isUri(input.uri)) &&
			input.configuration &&
			areOptionalsValid &&
	...
	private sendCustomConfigurations(
		configs: any,
		providerVersion: Version,
	): void {
		...
		if (!configs || !(configs instanceof Array)) {
			console.warn(
				"discarding invalid SourceFileConfigurationItems[]: " + configs,
			);
			return;
		}
		...
		configs.forEach((item) => {
			if (this.isSourceFileConfigurationItem(item, providerVersion)) {
				...
				this.configurationLogging.set(
					uri,
					JSON.stringify(item.configuration, null, 4),
				);
				if (util.getNumericLoggingLevel(settings.loggingLevel) >= 6) {
					out.appendLine(`  uri: ${uri}`);
					out.appendLine(
						`  config: ${JSON.stringify(item.configuration, null, 2)}`,
					);
				}
				if (
					item.configuration.includePath.some((path) =>
						path.endsWith("**"),
					)
				) {
					console.warn(
						"custom include paths should not use recursive includes ('**')",
					);
				}
				// Separate compiler path and args before sending to language client
				const itemConfig: util.Mutable<InternalSourceFileConfiguration> =
					deepCopy(item.configuration);
				if (util.isString(itemConfig.compilerPath)) {
					const compilerPathAndArgs: util.CompilerPathAndArgs =
						util.extractCompilerPathAndArgs(
							providerVersion < Version.v6,
							itemConfig.compilerPath,
							util.isArrayOfString(itemConfig.compilerArgs)
								? itemConfig.compilerArgs
								: undefined,
						);
					itemConfig.compilerPath =
						compilerPathAndArgs.compilerPath ?? undefined;
					if (itemConfig.compilerPath !== undefined) {
						void this.addTrustedCompiler(
							itemConfig.compilerPath,
						).catch(logAndReturn.undefined);
					}
					if (providerVersion < Version.v6) {
						itemConfig.compilerArgsLegacy =
							compilerPathAndArgs.allCompilerArgs;
				...
				console.warn(
					"discarding invalid SourceFileConfigurationItem: " +
						JSON.stringify(item),
				);
			}
		});
		...
		void this.languageClient
			.sendNotification(
				CustomConfigurationHighPriorityNotification,
				params,
			)
			.catch(logAndReturn.undefined);
		void this.languageClient
			.sendNotification(CustomConfigurationNotification, params)
			.catch(logAndReturn.undefined);
	}

	private browseConfigurationLogging: string = "";
	private configurationLogging: Map<string, string> = new Map<
		string,
		string
	>();
	...
	private sendCustomBrowseConfiguration(
		config: any,
		...
		while (true) {
			...
			if (timeoutOccured || !config || config instanceof Array) {
				...
				const configValue: WorkspaceBrowseConfiguration | undefined =
					this.lastCustomBrowseConfiguration.Value;
				if (configValue) {
			...
			const browseConfig: InternalWorkspaceBrowseConfiguration =
				config as InternalWorkspaceBrowseConfiguration;
			sanitized = deepCopy(browseConfig);
			if (
				!this.isWorkspaceBrowseConfiguration(sanitized) ||
				sanitized.browsePath.length === 0
			) {
				console.log(
					"Received an invalid browse configuration from configuration provider: " +
			...
			if (util.isString(sanitized.compilerPath)) {
				const compilerPathAndArgs: util.CompilerPathAndArgs =
					util.extractCompilerPathAndArgs(
						...
						util.isArrayOfString(sanitized.compilerArgs)
							? sanitized.compilerArgs
							: undefined,
					);
				sanitized.compilerPath =
					compilerPathAndArgs.compilerPath ?? undefined;
				if (sanitized.compilerPath !== undefined) {
					void this.addTrustedCompiler(sanitized.compilerPath).catch(
						logAndReturn.undefined,
					);
				}
				if (providerVersion < Version.v6) {
					sanitized.compilerArgsLegacy =
						compilerPathAndArgs.allCompilerArgs;
		...
		void this.languageClient
			.sendNotification(CustomBrowseConfigurationNotification, params)
			.catch(logAndReturn.undefined);
	}
	...
	private async clearCustomConfigurations(): Promise<void> {
		...
		await this.ready;
		return this.languageClient.sendNotification(
			ClearCustomConfigurationsNotification,
			params,
		);
	}
	...
	private async clearCustomBrowseConfiguration(): Promise<void> {
		...
		await this.ready;
		return this.languageClient.sendNotification(
			ClearCustomBrowseConfigurationNotification,
			params,
		);
	}
	 ...
	 * command handlers
	 */
	public async handleConfigurationSelectCommand(
		config?: string,
	): Promise<void> {
		await this.ready;
		const configNames: string[] | undefined =
			this.configuration.ConfigurationNames;
		if (configNames) {
			const index: number = config
				? configNames.indexOf(config)
				: await ui.showConfigurations(configNames);
			if (index < 0) {
				return;
			}
	...
	public async handleConfigurationProviderSelectCommand(): Promise<void> {
		await this.ready;
		const extensionId: string | undefined =
		...
		if (extensionId) {
			const provider: CustomConfigurationProvider1 | undefined =
				getCustomConfigProviders().get(extensionId);
			void this.updateCustomBrowseConfiguration(provider).catch(
				logAndReturn.undefined,
			);
			void this.updateCustomConfigurations(provider).catch(
				logAndReturn.undefined,
			);
			telemetry.logLanguageServerEvent("customConfigurationProvider", {
	...
	public async handleShowActiveCodeAnalysisCommands(): Promise<void> {
		...
		switch (index) {
			case 0:
				return this.CancelCodeAnalysis();
			case 1:
				return this.PauseCodeAnalysis();
			case 2:
				return this.ResumeCodeAnalysis();
			case 3:
				return this.handleShowIdleCodeAnalysisCommands();
		}
	}
	...
	public async handleShowIdleCodeAnalysisCommands(): Promise<void> {
		...
		switch (index) {
			case 0:
				return this.handleRunCodeAnalysisOnActiveFile();
			case 1:
				return this.handleRunCodeAnalysisOnAllFiles();
			case 2:
				return this.handleRunCodeAnalysisOnOpenFiles();
		}
	}

	public async handleConfigurationEditCommand(
		viewColumn: vscode.ViewColumn = vscode.ViewColumn.Active,
	): Promise<void> {
		await this.ready;
		return this.configuration.handleConfigurationEditCommand(
			undefined,
	...
	public async handleConfigurationEditJSONCommand(
		viewColumn: vscode.ViewColumn = vscode.ViewColumn.Active,
	): Promise<void> {
		await this.ready;
		return this.configuration.handleConfigurationEditJSONCommand(
			undefined,
			vscode.window.showTextDocument,
			viewColumn,
		);
	...
	public async handleConfigurationEditUICommand(
		viewColumn: vscode.ViewColumn = vscode.ViewColumn.Active,
	): Promise<void> {
		await this.ready;
		return this.configuration.handleConfigurationEditUICommand(
			undefined,
			vscode.window.showTextDocument,
			viewColumn,
		);
	...
	public async handleGoToDirectiveInGroup(next: boolean): Promise<void> {
		...
		if (editor) {
			...
			await this.ready;
			const response: Position | undefined =
				await this.languageClient.sendRequest(
					GoToDirectiveInGroupRequest,
					params,
				);
			if (response) {
				const p: vscode.Position = new vscode.Position(
					response.line,
					response.character,
				);
				...
				if (
					currentEditor &&
					editor.document.uri === currentEditor.document.uri
				) {
	...
	public async handleGenerateDoxygenComment(
		args: DoxygenCodeActionCommandArguments | vscode.Uri | undefined,
	): Promise<void> {
		const editor: vscode.TextEditor | undefined =
			vscode.window.activeTextEditor;
		...
		const initCursorPosition: vscode.Position =
			codeActionArguments !== undefined
				? new vscode.Position(
						codeActionArguments.initialCursor.line,
						codeActionArguments.initialCursor.character,
					)
		...
		const params: GenerateDoxygenCommentParams = {
			...
			position:
				codeActionArguments !== undefined
					? new vscode.Position(
							codeActionArguments.adjustedCursor.line,
							codeActionArguments.adjustedCursor.character,
						)
					: editor.selection.start,
			isCodeAction: codeActionArguments !== undefined,
			isCursorAboveSignatureLine:
				codeActionArguments?.isCursorAboveSignatureLine,
		};
		await this.ready;
		const currentFileVersion: number | undefined = openFileVersions.get(
			params.uri,
		...
		const result: GenerateDoxygenCommentResult | undefined =
			await this.languageClient.sendRequest(
				GenerateDoxygenCommentRequest,
				params,
			);
		...
		if (
			...
			const cursorOnEmptyLineAboveSignature: boolean =
				result.isCursorAboveSignatureLine;
			...
			if (
				cursorOnEmptyLineAboveSignature &&
				codeActionArguments !== undefined
			) {
				newPosition = new vscode.Position(
					result.finalCursorPosition.line - 1,
					result.finalCursorPosition.character,
				);
			} else {
				newPosition = new vscode.Position(
	...
	public async handleRunCodeAnalysisOnActiveFile(): Promise<void> {
		await this.ready;
		return this.languageClient.sendNotification(CodeAnalysisNotification, {
			scope: CodeAnalysisScope.ActiveFile,
		});
	}
	...
	public async handleRunCodeAnalysisOnOpenFiles(): Promise<void> {
		await this.ready;
		return this.languageClient.sendNotification(CodeAnalysisNotification, {
			scope: CodeAnalysisScope.OpenFiles,
		});
	}
	...
	public async handleRunCodeAnalysisOnAllFiles(): Promise<void> {
		await this.ready;
		return this.languageClient.sendNotification(CodeAnalysisNotification, {
			scope: CodeAnalysisScope.AllFiles,
		});
	}
	...
	public async handleRemoveAllCodeAnalysisProblems(): Promise<void> {
		...
		if (removeAllCodeAnalysisProblems()) {
			return this.languageClient.sendNotification(
				CodeAnalysisNotification,
				{ scope: CodeAnalysisScope.ClearSquiggles },
			);
		}
	}

	public async handleFixCodeAnalysisProblems(
		workspaceEdit: vscode.WorkspaceEdit,
		refreshSquigglesOnSave: boolean,
		identifiersAndUris: CodeAnalysisDiagnosticIdentifiersAndUri[],
	): Promise<void> {
		if (await vscode.workspace.applyEdit(workspaceEdit)) {
			...
			if (settings.clangTidyCodeActionFormatFixes) {
				...
				const formatEdits: vscode.WorkspaceEdit =
					new vscode.WorkspaceEdit();
				for (const uri of editedFiles) {
					const formatTextEdits: vscode.TextEdit[] | undefined =
			...
			return this.handleRemoveCodeAnalysisProblems(
				refreshSquigglesOnSave,
				identifiersAndUris,
			);
		}
	}

	public async handleRemoveCodeAnalysisProblems(
		refreshSquigglesOnSave: boolean,
		identifiersAndUris: CodeAnalysisDiagnosticIdentifiersAndUri[],
	): Promise<void> {
		await this.ready;
		...
		for (const identifiersAndUri of identifiersAndUris) {
			identifiersAndUrisCopy.push({
				uri: identifiersAndUri.uri,
				identifiers: [...identifiersAndUri.identifiers],
			});
		}
		...
		if (removeCodeAnalysisProblems(identifiersAndUris)) {
			...
			return this.languageClient.sendNotification(
				RemoveCodeAnalysisProblemsNotification,
				{
					identifiersAndUris: identifiersAndUrisCopy,
					refreshSquigglesOnSave: refreshSquigglesOnSave,
				},
			);
		}
	}

	public async handleDisableAllTypeCodeAnalysisProblems(
		code: string,
		identifiersAndUris: CodeAnalysisDiagnosticIdentifiersAndUri[],
	): Promise<void> {
		const settings: CppSettings = new CppSettings(this.RootUri);
	...
	public async handleCreateDeclarationOrDefinition(
		isCopyToClipboard: boolean,
		codeActionRange?: Range,
	): Promise<void> {
		...
		if (cppSettings.useVcFormat(editor.document)) {
			...
			formatParams = {
				...
				useVcFormat: true,
				insertSpaces:
					editorConfigSettings.indent_style !== undefined
						? editorConfigSettings.indent_style === "space"
							? true
							: false
						: true,
				tabSize:
					editorConfigSettings.tab_width !== undefined
						? editorConfigSettings.tab_width
						: 4,
				character: "",
				range: {
			...
			formatParams = {
				...
				useVcFormat: false,
				insertSpaces:
					editorSettings.editorInsertSpaces !== undefined
						? editorSettings.editorInsertSpaces
						: true,
				tabSize:
					editorSettings.editorTabSize !== undefined
		...
		const result: CreateDeclarationOrDefinitionResult =
			await this.languageClient.sendRequest(
		...
		for (const workspaceEdit of result.workspaceEdits) {
			...
			for (const edit of workspaceEdit.edits) {
				...
				if (
					lastEdit &&
					lastEdit.newText.length < 300 &&
					lastEdit.newText.includes("#include") &&
					lastEdit.range.isEqual(range)
				) {
					// Destination file is empty.
					// The edit positions for #include header file and definition or declaration are the same.
					const selectionPositionAdjustment = (
						lastEdit.newText.match(/\n/g) ?? []
					).length;
					range = new vscode.Range(
						new vscode.Position(
							range.start.line + selectionPositionAdjustment,
							range.start.character,
						),
						new vscode.Position(
							range.end.line + selectionPositionAdjustment,
							range.end.character,
						),
					);
				}
				...
				if (
					...
					edit.newText.includes("#pragma once")
				) {
		...
		const selectionPosition: vscode.Position = new vscode.Position(
			startLine,
			0,
		);
		...
		const formatRange: vscode.Range = new vscode.Range(
			selectionRange.start,
			new vscode.Position(selectionRange.start.line + numNewlines, 0),
		);
		const settings: OtherSettings = new OtherSettings(
			vscode.workspace.getWorkspaceFolder(modifiedDocument)?.uri,
		);
		...
		const versionBeforeFormatting: number | undefined =
			openFileVersions.get(modifiedDocument.toString());
		...
		const versionAfterFormatting: number | undefined = openFileVersions.get(
			modifiedDocument.toString(),
		);
		if (
			versionAfterFormatting === undefined ||
			versionAfterFormatting > versionBeforeFormatting
		) {
			return;
		}
		await vscode.workspace.applyEdit(formatEdits);
	...
	public async handleExtractToFunction(
		extractAsGlobal: boolean,
	): Promise<void> {
		const editor: vscode.TextEditor | undefined =
			vscode.window.activeTextEditor;
		...
		for (const workspaceEdit of result.workspaceEdits) {
			...
			for (const edit of workspaceEdit.edits) {
				...
				if (!isSourceFile && headerFileLineOffset) {
					range = new vscode.Range(
						new vscode.Position(
							range.start.line + headerFileLineOffset,
							range.start.character,
						),
						new vscode.Position(
							range.end.line + headerFileLineOffset,
							range.end.character,
						),
					);
				}
				...
				if (isReplace) {
					...
					workspaceEdits.insert(uri, range.start, edit.newText);
					if (edit.newText.length < 300) {
						// Avoid searching large code edits
						if (
							isSourceFile &&
							!hasProcessedReplace &&
							edit.newText.includes("#include")
						) {
							continue;
						}
				...
				const formatRangeStartLine: number =
				...
				const newFormatRange: vscode.Range = new vscode.Range(
					new vscode.Position(
						formatRangeStartLine +
							(nextLineOffset < 0 ? nextLineOffset : 0),
						range.start.character,
					),
					new vscode.Position(
						formatRangeStartLine +
							(nextLineOffset < 0 ? 0 : nextLineOffset),
						isReplace
							? range.end.character
							: (startWithNewLine ? 0 : range.end.character) +
				...
				if (isSourceFile) {
					sourceFormatUriAndRanges.push({
						uri,
						range: newFormatRange,
					});
				} else {
					headerFormatUriAndRanges.push({
						uri,
						range: newFormatRange,
					});
				}
				if (isReplace || !isSourceFile) {
					...
					let currentText: string =
						edit.newText.substring(rangeStartCharacter);
					let currentTextNextLineStart: number =
						currentText.indexOf("\n");
					let currentTextNewFunctionStart: number =
						currentText.indexOf(functionName);
					let currentTextNextLineStartUpdated: boolean = false;
					while (
						currentTextNextLineStart !== -1 &&
						currentTextNextLineStart < currentTextNewFunctionStart
					) {
						++rangeStartLine;
					...
					rangeStartCharacter =
						...
						!currentTextNextLineStartUpdated
							? range.start.character
							: 0) + currentTextNewFunctionStart;
					...
					const replaceEditRange = new vscode.Range(
						...
						new vscode.Position(
							rangeStartLine,
							rangeStartCharacter + functionName.length,
						),
					);
					if (isSourceFile) {
		...
		if (
			sourceReplaceEditRange === undefined ||
			sourceFormatUriAndRanges.length === 0
		) {
			return;
		}
		...
		if (
			headerFormatUriAndRanges.length > 0 &&
			headerReplaceEditRange !== undefined
		) {
			...
			await vscode.window.showTextDocument(
				headerFormatUriAndRanges[0].uri,
				{
					selection: headerReplaceEditRange,
					preserveFocus: false,
				},
			);
		}
		...
		const formatRanges = async (
			formatUriAndRanges: VsCodeUriAndRange[],
		) => {
			if (formatUriAndRanges.length === 0) {
				return;
			}
			const formatUriAndRange: VsCodeUriAndRange = formatUriAndRanges[0];
			const isMultipleFormatRanges: boolean =
				formatUriAndRanges.length > 1;
			const settings: OtherSettings = new OtherSettings(
				vscode.workspace.getWorkspaceFolder(formatUriAndRange.uri)?.uri,
			);
			const formatOptions: vscode.FormattingOptions = {
				insertSpaces: settings.editorInsertSpaces ?? true,
				tabSize: settings.editorTabSize ?? 4,
			...
			const tryFormat = async () => {
				const versionBeforeFormatting: number | undefined =
					openFileVersions.get(formatUriAndRange.uri.toString());
				if (versionBeforeFormatting === undefined) {
					return true;
				}
				...
				const formatTextEdits: vscode.TextEdit[] | undefined =
					isMultipleFormatRanges
						? await vscode.commands.executeCommand<
								vscode.TextEdit[] | undefined
							>(
								"vscode.executeFormatDocumentProvider",
								formatUriAndRange.uri,
								formatOptions,
							)
						: await vscode.commands.executeCommand<
								vscode.TextEdit[] | undefined
							>(
								"vscode.executeFormatRangeProvider",
								formatUriAndRange.uri,
								formatUriAndRange.range,
								formatOptions,
							);

				if (
					!formatTextEdits ||
					formatTextEdits.length === 0 ||
					versionBeforeFormatting === undefined
				) {
					return true;
				}
				// Only apply formatting if the document version hasn't changed to prevent
				// stale formatting results from being applied.
				const versionAfterFormatting: number | undefined =
					openFileVersions.get(formatUriAndRange.uri.toString());
				if (
					versionAfterFormatting === undefined ||
					versionAfterFormatting > versionBeforeFormatting
		...
		await vscode.window.showTextDocument(sourceFormatUriAndRanges[0].uri, {
			selection: sourceReplaceEditRange,
			preserveFocus: false,
		});
	...
	public onInterval(): void {
		...
		if (
			this.innerLanguageClient !== undefined &&
			this.configuration !== undefined
		) {
			void this.languageClient
				.sendNotification(IntervalTimerNotification)
				.catch(logAndReturn.undefined);
	...
	public async handleReferencesIcon(): Promise<void> {
		...
		workspaceReferences.UpdateProgressUICounter(
			this.model.referencesCommandMode.Value,
		);
	...
	private serverCanceledReferences(): void {
		workspaceReferences.cancelCurrentReferenceRequest(
			refs.CancellationSender.LanguageServer,
		);
	}

	private handleReferencesProgress(
		notificationBody: refs.ReportReferencesProgressNotification,
	): void {
		workspaceReferences.handleProgress(notificationBody);
	}

	private processReferencesPreview(
		referencesResult: refs.ReferencesResult,
	): void {
...
class NullClient implements Client {
	...
	private referencesCommandModeEvent =
		new vscode.EventEmitter<refs.ReferencesCommandMode>();
	...
	public get InitializingWorkspaceChanged(): vscode.Event<boolean> {
		return this.booleanEvent.event;
	}
	public get IndexingWorkspaceChanged(): vscode.Event<boolean> {
		return this.booleanEvent.event;
	}
	...
	public get CodeAnalysisProcessedChanged(): vscode.Event<number> {
		return this.numberEvent.event;
	}
	public get CodeAnalysisTotalChanged(): vscode.Event<number> {
		return this.numberEvent.event;
	}
	public get ReferencesCommandModeChanged(): vscode.Event<refs.ReferencesCommandMode> {
		return this.referencesCommandModeEvent.event;
	}
	public get TagParserStatusChanged(): vscode.Event<string> {
		return this.stringEvent.event;
	}
	public get ActiveConfigChanged(): vscode.Event<string> {
		return this.stringEvent.event;
	}
	...
	updateActiveDocumentTextOptions(): void {}
	didChangeActiveEditor(editor?: vscode.TextEditor): Promise<void> {
		return Promise.resolve();
	}
	restartIntelliSenseForFile(document: vscode.TextDocument): Promise<void> {
		return Promise.resolve();
	...
	getIncludes(maxDepth: number): Promise<GetIncludesResult> {
		return Promise.resolve({} as GetIncludesResult);
	}
	getChatContext(
		uri: vscode.Uri,
		token: vscode.CancellationToken,
	...
	getProjectContext(uri: vscode.Uri): Promise<ProjectContextResult> {
		return Promise.resolve({} as ProjectContextResult);
	}
}
