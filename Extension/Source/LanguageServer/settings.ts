...
import {
	...
	isString,
	isValidMapping,
	setCachedClangFormatPath,
	setCachedClangTidyPath,
} from "../common";
import { isWindows } from "../constants";
import * as telemetry from "../telemetry";
import {
	cachedEditorConfigLookups,
	DefaultClient,
	hasTrustedCompilerPaths,
} from "./client";
import {
	getEditorConfigSettings,
	mapIndentationReferenceToEditorConfig,
	mapIndentToEditorConfig,
	mapNewOrSameLineToEditorConfig,
	mapWrapToEditorConfig,
} from "./editorConfig";
...
class Settings {
	...
	protected get Section(): vscode.WorkspaceConfiguration {
	...
	public getArrayOfStringsWithUndefinedDefault(
		...
		if (
			(allowNull && info.workspaceFolderValue === null) ||
			isArrayOfString(info.workspaceFolderValue)
		) {
			return info.workspaceFolderValue;
		}

		if (
			(allowNull && info.workspaceValue === null) ||
			isArrayOfString(info.workspaceValue)
		) {
			return info.workspaceValue;
		}

		if (
			(allowNull && info.globalValue === null) ||
			isArrayOfString(info.globalValue)
		) {
...
function changeBlankStringToUndefined(
...
export class CppSettings extends Settings {
	...
	private getClangPath(isFormat: boolean): string | undefined {
		let path: string | undefined = changeBlankStringToUndefined(
			this.getAsStringOrUndefined(
				isFormat ? "clang_format_path" : "codeAnalysis.clangTidy.path",
			),
		);
		if (!path) {
			const cachedClangPath: string | undefined = isFormat
				? getCachedClangFormatPath()
				: getCachedClangTidyPath();
			if (cachedClangPath !== undefined) {
				return cachedClangPath;
			}
			const clangStr: string = isFormat
				? this.clangFormatStr
				: this.clangTidyStr;
			const clangName: string = isFormat
				? this.clangFormatName
				: this.clangTidyName;
			const setCachedClangPath: (path: string) => void = isFormat
				? setCachedClangFormatPath
				: setCachedClangTidyPath;
			const whichPath: string | null = which.sync(clangName, {
				nothrow: true,
			});
			if (whichPath === null) {
				return undefined;
			...
			if (!path) {
				...
				try {
					const exePath: string = getExtensionFilePath(
						`./LLVM/bin/${clangName}`,
					);
					const output: string[] = execSync(
						quote([exePath, "--version"]),
					)
						.toString()
						.split(" ");
					if (
						...
						output[0] !== clangStr ||
						output[1] !== "version" ||
						!semver.valid(output[2])
					) {
						if (output.length === 3) {
							return path;
						}
						const versionIndex: number = output.findIndex(
							(value: string) => value === "version",
						);
						if (
							versionIndex < 0 ||
							versionIndex + 1 >= output.length ||
							!semver.valid(output[versionIndex + 1].trim())
						) {
							return path;
						}
					}
				...
				try {
					const output: string[] = execSync(`"${path}" --version`)
						.toString()
						.split(" ");
					if (
						output.length < 3 ||
						output[0] !== clangStr ||
						output[1] !== "version" ||
						semver.ltr(output[2], clangVersion)
					) {
						path = "";
						setCachedClangPath(path);
					}
	...
	public get clangTidyFallbackConfig(): string | undefined {
		return changeBlankStringToUndefined(
			this.getAsStringOrUndefined(
				"codeAnalysis.clangTidy.fallbackConfig",
			),
		);
	}
	...
	public set defaultCompilerPath(value: string) {
		...
		const compilerPathInfo: any = this.Section.inspect(
			defaultCompilerPathStr,
		);
		let target: vscode.ConfigurationTarget =
			vscode.ConfigurationTarget.Global;
		if (
			this.resource !== undefined ||
			compilerPathInfo.workspaceFolderValue !== undefined
		) {
	...
	public get defaultCustomConfigurationVariables(): Associations | undefined {
		return (
			this.getAsAssociations(
				"default.customConfigurationVariables",
				true,
			) ?? undefined
		);
	}
	...
	public get isCaseSensitiveFileSupportEnabled(): boolean {
		return (
			!isWindows ||
			this.getAsString("caseSensitiveFileSupport").toLowerCase() ===
				"enabled"
		);
	}
	...
	private getAsStringOrUndefined(settingName: string): string | undefined {
		...
		if (setting.default !== undefined) {
			console.error(
				`Default value for ${settingName} is expected to be undefined.`,
			);
		}
	...
	private getAsBoolean(
		...
		if (!this.isValidDefault(isBoolean, setting.default, allowNull)) {
			console.error(
				`Default value for ${settingName} is expected to be boolean${allowNull ? " or null" : ""}.`,
			);
		}
	...
	private getAsString(
		...
		if (!this.isValidDefault(isString, setting.default, allowNull)) {
			console.error(
				`Default value for ${settingName} is expected to be string${allowNull ? " or null" : ""}.`,
			);
		}
	...
	private getAsNumber(
		...
		if (!this.isValidDefault(isNumber, setting.default, allowNull)) {
			console.error(
				`Default value for ${settingName} is expected to be number${allowNull ? " or null" : ""}.`,
			);
		}
	...
	private getAsArrayOfStringsOrUndefined(
		settingName: string,
		allowUndefinedEnums: boolean = false,
	): string[] | undefined {
		...
		if (isArrayOfString(value)) {
			if (setting.items.enum && !allowUndefinedEnums) {
				if (
					!value.every((x) => this.isValidEnum(setting.items.enum, x))
				) {
					return setting.default;
				}
			}
	...
	private getAsArrayOfStrings(
		...
		if (!isArrayOfString(setting.default)) {
			console.error(
				`Default value for ${settingName} is expected to be string[].`,
			);
		}
	...
	private getAsExcludes(
		...
		if (
			!this.isValidDefault(
				(x) =>
					isValidMapping(
						x,
						isString,
						(val) => isBoolean(val) || isValidWhenObject(val),
					),
				setting.default,
				allowNull,
			)
			...
			console.error(
				`Default value for ${settingName} is expected to be Excludes${allowNull ? " or null" : ""}.`,
			);
		}
	...
	private getAsAssociations(
		...
		if (
			!this.isValidDefault(
				(x) => isValidMapping(x, isString, isString),
				setting.default,
				allowNull,
			)
		) {
			console.error(
				`Default value for ${settingName} is expected to be Associations${allowNull ? " or null" : ""}.`,
			);
		}
	...
	private isValidEnum(enumDescription: any, value: any): value is string {
		if (
			isString(value) &&
			isArray(enumDescription) &&
			enumDescription.length > 0
		) {
			return enumDescription.some(
	...
	private isArrayOfCommentContinuationPatterns(
		x: any,
	): x is (string | CommentPattern)[] {
		return (
			isArray(x) &&
			x.every((y) => isString(y) || this.isCommentPattern(y))
		);
	}
	...
	public populateEditorConfig(document: vscode.TextDocument): void {
		...
		settingMap.set(
			"cpp_indent_braces",
			this.vcFormatIndentBraces.toString(),
		);
		settingMap.set(
			"cpp_indent_multi_line_relative_to",
			mapIndentationReferenceToEditorConfig(
				this.vcFormatIndentMultiLineRelativeTo,
			),
		);
		settingMap.set(
			"cpp_indent_within_parentheses",
			this.vcFormatIndentWithinParentheses.toString(),
		);
		settingMap.set(
			"cpp_indent_preserve_within_parentheses",
			this.vcFormatIndentPreserveWithinParentheses.toString(),
		);
		settingMap.set(
			"cpp_indent_case_labels",
			this.vcFormatIndentCaseLabels.toString(),
		);
		settingMap.set(
			"cpp_indent_case_contents",
			this.vcFormatIndentCaseContents.toString(),
		);
		settingMap.set(
			"cpp_indent_case_contents_when_block",
			this.vcFormatIndentCaseContentsWhenBlock.toString(),
		);
		settingMap.set(
			"cpp_indent_lambda_braces_when_parameter",
			this.vcFormatIndentLambdaBracesWhenParameter.toString(),
		);
		settingMap.set(
			"cpp_indent_goto_labels",
			mapIndentToEditorConfig(this.vcFormatIndentGotoLabels),
		);
		settingMap.set(
			"cpp_indent_preprocessor",
			mapIndentToEditorConfig(this.vcFormatIndentPreprocessor),
		);
		settingMap.set(
			"cpp_indent_access_specifiers",
			this.vcFormatIndentAccessSpecifiers.toString(),
		);
		settingMap.set(
			"cpp_indent_namespace_contents",
			this.vcFormatIndentNamespaceContents.toString(),
		);
		settingMap.set(
			"cpp_indent_preserve_comments",
			this.vcFormatIndentPreserveComments.toString(),
		);
		settingMap.set(
			"cpp_new_line_before_open_brace_namespace",
			mapNewOrSameLineToEditorConfig(
				this.vcFormatNewlineBeforeOpenBraceNamespace,
			),
		);
		settingMap.set(
			"cpp_new_line_before_open_brace_type",
			mapNewOrSameLineToEditorConfig(
				this.vcFormatNewlineBeforeOpenBraceType,
			),
		);
		settingMap.set(
			"cpp_new_line_before_open_brace_function",
			mapNewOrSameLineToEditorConfig(
				this.vcFormatNewlineBeforeOpenBraceFunction,
			),
		);
		settingMap.set(
			"cpp_new_line_before_open_brace_block",
			mapNewOrSameLineToEditorConfig(
				this.vcFormatNewlineBeforeOpenBraceBlock,
			),
		);
		settingMap.set(
			"cpp_new_line_before_open_brace_lambda",
			mapNewOrSameLineToEditorConfig(
				this.vcFormatNewlineBeforeOpenBraceLambda,
			),
		);
		settingMap.set(
			"cpp_new_line_scope_braces_on_separate_lines",
			this.vcFormatNewlineScopeBracesOnSeparateLines.toString(),
		);
		settingMap.set(
			"cpp_new_line_close_brace_same_line_empty_type",
			this.vcFormatNewlineCloseBraceSameLineEmptyType.toString(),
		);
		settingMap.set(
			"cpp_new_line_close_brace_same_line_empty_function",
			this.vcFormatNewlineCloseBraceSameLineEmptyFunction.toString(),
		);
		settingMap.set(
			"cpp_new_line_before_catch",
			this.vcFormatNewlineBeforeCatch.toString().toString(),
		);
		settingMap.set(
			"cpp_new_line_before_else",
			this.vcFormatNewlineBeforeElse.toString().toString(),
		);
		settingMap.set(
			"cpp_new_line_before_while_in_do_while",
			this.vcFormatNewlineBeforeWhileInDoWhile.toString(),
		);
		settingMap.set(
			"cpp_space_before_function_open_parenthesis",
			this.vcFormatSpaceBeforeFunctionOpenParenthesis.toString(),
		);
		settingMap.set(
			"cpp_space_within_parameter_list_parentheses",
			this.vcFormatSpaceWithinParameterListParentheses.toString(),
		);
		settingMap.set(
			"cpp_space_between_empty_parameter_list_parentheses",
			this.vcFormatSpaceBetweenEmptyParameterListParentheses.toString(),
		);
		settingMap.set(
			"cpp_space_after_keywords_in_control_flow_statements",
			this.vcFormatSpaceAfterKeywordsInControlFlowStatements.toString(),
		);
		settingMap.set(
		...
		settingMap.set(
			...
			this.vcFormatSpaceBeforeLambdaOpenParenthesis.toString(),
		);
		settingMap.set(
			"cpp_space_within_cast_parentheses",
			this.vcFormatSpaceWithinCastParentheses.toString(),
		);
		settingMap.set(
			"cpp_space_after_cast_close_parenthesis",
			this.vcFormatSpaceAfterCastCloseParenthesis.toString(),
		);
		settingMap.set(
			"cpp_space_within_expression_parentheses",
			this.vcFormatSpaceWithinExpressionParentheses.toString(),
		);
		settingMap.set(
			"cpp_space_before_block_open_brace",
			this.vcFormatSpaceBeforeBlockOpenBrace.toString(),
		);
		settingMap.set(
			"cpp_space_between_empty_braces",
			this.vcFormatSpaceBetweenEmptyBraces.toString(),
		);
		settingMap.set(
			"cpp_space_before_initializer_list_open_brace",
			this.vcFormatSpaceBeforeInitializerListOpenBrace.toString(),
		);
		settingMap.set(
			"cpp_space_within_initializer_list_braces",
			this.vcFormatSpaceWithinInitializerListBraces.toString(),
		);
		settingMap.set(
			"cpp_space_preserve_in_initializer_list",
			this.vcFormatSpacePreserveInInitializerList.toString(),
		);
		settingMap.set(
			"cpp_space_before_open_square_bracket",
			this.vcFormatSpaceBeforeOpenSquareBracket.toString(),
		);
		settingMap.set(
			"cpp_space_within_square_brackets",
			this.vcFormatSpaceWithinSquareBrackets.toString(),
		);
		settingMap.set(
			"cpp_space_before_empty_square_brackets",
			this.vcFormatSpaceBeforeEmptySquareBrackets.toString(),
		);
		settingMap.set(
			"cpp_space_between_empty_square_brackets",
			this.vcFormatSpaceBetweenEmptySquareBrackets.toString(),
		);
		settingMap.set(
			"cpp_space_group_square_brackets",
			this.vcFormatSpaceGroupSquareBrackets.toString(),
		);
		settingMap.set(
			"cpp_space_within_lambda_brackets",
			this.vcFormatSpaceWithinLambdaBrackets.toString(),
		);
		settingMap.set(
			"cpp_space_between_empty_lambda_brackets",
			this.vcFormatSpaceBetweenEmptyLambdaBrackets.toString(),
		);
		settingMap.set(
			"cpp_space_before_comma",
			this.vcFormatSpaceBeforeComma.toString(),
		);
		settingMap.set(
			"cpp_space_after_comma",
			this.vcFormatSpaceAfterComma.toString(),
		);
		settingMap.set(
			"cpp_space_remove_around_member_operators",
			this.vcFormatSpaceRemoveAroundMemberOperators.toString(),
		);
		settingMap.set(
			"cpp_space_before_inheritance_colon",
			this.vcFormatSpaceBeforeInheritanceColon.toString(),
		);
		settingMap.set(
			"cpp_space_before_constructor_colon",
			this.vcFormatSpaceBeforeConstructorColon.toString(),
		);
		settingMap.set(
			"cpp_space_remove_before_semicolon",
			this.vcFormatSpaceRemoveBeforeSemicolon.toString(),
		);
		settingMap.set(
			"cpp_space_after_semicolon",
			this.vcFormatSpaceInsertAfterSemicolon.toString(),
		);
		settingMap.set(
			"cpp_space_remove_around_unary_operator",
			this.vcFormatSpaceRemoveAroundUnaryOperator.toString(),
		);
		settingMap.set(
			"cpp_space_around_binary_operator",
			this.vcFormatSpaceAroundBinaryOperator.toString(),
		);
		settingMap.set(
			"cpp_space_around_assignment_operator",
			this.vcFormatSpaceAroundAssignmentOperator.toString(),
		);
		settingMap.set(
			"cpp_space_pointer_reference_alignment",
			this.vcFormatSpacePointerReferenceAlignment.toString(),
		);
		settingMap.set(
			"cpp_space_around_ternary_operator",
			this.vcFormatSpaceAroundTernaryOperator.toString(),
		);
		settingMap.set(
			"cpp_wrap_preserve_blocks",
			mapWrapToEditorConfig(this.vcFormatWrapPreserveBlocks),
		);
	...
	public useVcFormat(document: vscode.TextDocument): boolean {
		...
		const findConfigFile: (parentPath: string) => boolean = (
			...
			const editorConfigPath: string = path.join(
				parentPath,
				".editorconfig",
			);
			if (fs.existsSync(editorConfigPath)) {
				...
				for (let i: number = 0; i < keys.length; ++i) {
					if (keys[i].startsWith("cpp_")) {
						...
						if (
							cppCheck.startsWith("indent_") ||
							cppCheck.startsWith("new_line_") ||
							cppCheck.startsWith("space_") ||
							cppCheck.startsWith("wrap_")
						) {
							foundEditorConfigWithVcFormatSettings = true;
							const didEditorConfigNotice: PersistentState<boolean> =
								new PersistentState<boolean>(
									"Cpp.didEditorConfigNotice",
									false,
								);
				...
				switch (typeof editorConfigSettings.root) {
					...
					case "string":
						return (
							editorConfigSettings.root.toLowerCase() === "true"
						);
					default:
						return false;
				}
		...
		do {
			...
			if (findConfigFile(currentParentPath)) {
				cachedEditorConfigLookups.set(
					document.uri.fsPath,
					foundEditorConfigWithVcFormatSettings,
				);
...
export class OtherSettings {
	...
	private logValidationError(
		...
		telemetry.logLanguageServerEvent("settingsValidation", {
			setting: sectionName + "." + settingName,
			error,
		});
	}
	...
	public get filesEncoding(): string {
		return this.getAsString(
			"files",
			"encoding",
			{ uri: this.resource, languageId: "cpp" },
			"utf8",
		);
	}
