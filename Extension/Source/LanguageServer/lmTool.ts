	...
	private async getContext(token: vscode.CancellationToken): Promise<string> {
		try {
			...
			const chatContext: ChatContextResult | undefined =
				await (getClients()?.ActiveClient?.getChatContext(token) ??
					undefined);
			if (!chatContext) {
				return "No configuration information is available for the active document.";
			}

			telemetry.logLanguageModelToolEvent("cpp", {
			...
			for (const key in knownValues) {
				...
				if (knownValues[knownKey] && chatContext[knownKey]) {
					chatContext[knownKey] =
						knownValues[knownKey][chatContext[knownKey]] ||
						chatContext[knownKey];
				}
			}
