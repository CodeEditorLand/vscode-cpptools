...
const knownValues: {
	[Property in keyof ChatContextResult]?: { [id: string]: string };
} = {
	language: {
		"c": "C",
		"cpp": "C++",
...
function formatChatContext(
	context: ChatContextResult | ProjectContextResult,
): void {
	type KnownKeys =
		| "language"
		| "standardVersion"
...
export function getCompilerArgumentFilterMap(
	compiler: string,
		...
		switch (compiler) {
			case MSVC:
				if (
					...
					filterMap = JSON.parse(
						context.flags
							.copilotcppMsvcCompilerArgumentFilter as string,
					);
				}
				break;
			case Clang:
				if (
					context.flags.copilotcppClangCompilerArgumentFilter !==
					undefined
				) {
					filterMap = JSON.parse(
						context.flags
							.copilotcppClangCompilerArgumentFilter as string,
					);
				}
				break;
			case GCC:
				if (
					context.flags.copilotcppGccCompilerArgumentFilter !==
					undefined
				) {
					filterMap = JSON.parse(
						context.flags
							.copilotcppGccCompilerArgumentFilter as string,
					);
				}
				break;
		}
...
function filterCompilerArguments(
	...
	if (filteredCompilerArguments.length > 0) {
		...
		telemetryProperties["filteredCompilerArguments"] =
			filteredCompilerArguments.join(",");
		telemetryProperties["filters"] = Object.keys(filterMap)
...
export async function getProjectContext(
	...
	try {
		const projectContext = await checkDuration<
			ProjectContextResult | undefined
		>(
			async () =>
				(await getClients()?.ActiveClient?.getProjectContext(uri)) ??
				undefined,
		);
		...
		if (projectContext.result.standardVersion) {
			telemetryProperties["standardVersion"] =
				projectContext.result.standardVersion;
		}
		if (projectContext.result.targetPlatform) {
			telemetryProperties["targetPlatform"] =
				projectContext.result.targetPlatform;
		}
		if (projectContext.result.targetArchitecture) {
			telemetryProperties["targetArchitecture"] =
				projectContext.result.targetArchitecture;
		}
		telemetryMetrics["compilerArgumentCount"] =
			projectContext.result.fileContext.compilerArguments.length;
		result.compilerArguments = filterCompilerArguments(
			projectContext.result.compiler,
			projectContext.result.fileContext.compilerArguments,
			context,
			telemetryProperties,
		);
		...
		try {
			...
			logger
				...
				.appendLine(
					localize(
						"copilot.projectcontext.error",
						"Error while retrieving the project context. Reason: {0}",
						err.message,
					),
				);
		} catch {
...
export class CppConfigurationLanguageModelTool
	implements vscode.LanguageModelTool<void>
{
	...
	private async getContext(token: vscode.CancellationToken): Promise<string> {
		...
		try {
			...
			if (
				!currentDoc ||
				(!util.isCpp(currentDoc) && !util.isHeaderFile(currentDoc.uri))
			) {
				return "The active document is not a C, C++, or CUDA file.";
			}

			const chatContext: ChatContextResult | undefined =
				await (getClients()?.ActiveClient?.getChatContext(
					currentDoc.uri,
					token,
				) ?? undefined);
			if (!chatContext) {
	...
	private async reportError(): Promise<void> {
		try {
			logger
				...
				.appendLine(
					localize(
						"copilot.cppcontext.error",
						"Error while retrieving the #cpp context.",
					),
				);
		} catch {
			// Intentionally swallow any exception.
