...
export const cachedEditorConfigSettings: Map<string, any> = new Map<
	string,
	any
>();

export function mapIndentationReferenceToEditorConfig(
...
export function mapNewOrSameLineToEditorConfig(
	value: string | undefined,
): string {
	if (value !== undefined) {
		// Will never actually be undefined, as these settings have default values.
	...
	for (;;) {
		...
		if (fs.existsSync(editorConfigPath)) {
			...
			Object.keys(configData).forEach((section: string) => {
				if (
					section !== "*" &&
					matchesSection(currentDirForwardSlashes, filePath, section)
				) {
					combinedConfig = {
						...combinedConfig,
						...configData[section],
