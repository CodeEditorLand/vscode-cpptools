...
export class ReferencesTreeDataProvider
	...
	getTreeItem(element: TreeNode): vscode.TreeItem {
		...
		switch (element.node) {
			case NodeType.referenceType:
				...
				const resultRefType: vscode.TreeItem = new vscode.TreeItem(
					label,
			...
			case NodeType.fileWithPendingRef:
				...
				if (element.node === NodeType.fileWithPendingRef) {
					...
					const tag: string = getReferenceTagString(
	...
	getChildren(element?: TreeNode): TreeNode[] | undefined {
		...
		if (element instanceof TreeNode) {
			if (element.node === NodeType.file) {
				...
				return this.referencesModel.getReferenceNodes(
					element.filename,
					type,
				);
			}
