...
class PersistentStateBase<T> {
	...
	public get Value(): T {
		return this.state
			? this.state.get<T>(this.key, this.defaultvalue)
			: this.curvalue;
	}
	...
	constructor(key: string, defaultValue: T) {
		super(
			...
			util.extensionContext
				? util.extensionContext.globalState
				: undefined,
		);
	}
}
...
export class PersistentWorkspaceState<T> extends PersistentStateBase<T> {
	constructor(key: string, defaultValue: T) {
		super(
			...
			defaultValue,
			util.extensionContext
				? util.extensionContext.workspaceState
				: undefined,
		);
	}
}
...
export class PersistentFolderState<T> extends PersistentWorkspaceState<T> {
	constructor(key: string, defaultValue: T, folder: vscode.WorkspaceFolder) {
		...
		const old_key: string =
			key +
			(folder ? `-${path.basename(folder.uri.fsPath)}` : "-untitled");
		let old_val: T | undefined;
		if (util.extensionContext) {
			old_val = util.extensionContext.workspaceState.get(old_key);
			if (old_val !== undefined) {
				void util.extensionContext.workspaceState.update(
					old_key,
					undefined,
				);
			}
