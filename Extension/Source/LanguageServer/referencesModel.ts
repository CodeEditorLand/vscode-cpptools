...
export class ReferencesModel {
	...
	constructor(
		...
		const results: ReferenceInfo[] = resultsInput.isFinished
			? resultsInput.referenceInfos.filter(
					(r) => r.type !== ReferenceType.Confirmed,
				)
			: resultsInput.referenceInfos;
		...
		for (const r of results) {
			...
			const noReferenceLocation: boolean =
				r.position.line === 0 && r.position.character === 0;
			if (noReferenceLocation) {
				...
				const range: vscode.Range = new vscode.Range(
					...
					r.position.line,
					r.position.character + this.originalSymbol.length,
				);
	...
	getReferenceTypeNodes(): TreeNode[] {
		...
		for (const n of this.nodes) {
			const i: number = result.findIndex(
				(e) => e.referenceType === n.referenceType,
			);
			if (i < 0) {
				const node: TreeNode = new TreeNode(
					this,
	...
	getFileNodes(refType?: ReferenceType): TreeNode[] {
		...
		if (refType !== undefined) {
			filteredFiles = this.nodes.filter(
				(i) => i.referenceType === refType,
			);
		} else {
		...
		for (const n of filteredFiles) {
			const i: number = result.findIndex(
				(item) => item.filename === n.filename,
			);
			if (i < 0) {
				const nodeType: NodeType =
					n.node === NodeType.fileWithPendingRef
						? NodeType.fileWithPendingRef
						: NodeType.file;
