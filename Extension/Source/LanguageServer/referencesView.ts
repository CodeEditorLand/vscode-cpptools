...
export class FindAllRefsView {
	...
	getResultsAsText(includeConfirmedReferences: boolean): string {
		...
		for (const ref of this.referencesModel.getAllReferenceNodes()) {
			...
			line += ref.filename;
			if (
				ref.referencePosition !== null &&
				ref.referencePosition !== undefined
			) {
				line +=
					":" +
					(ref.referencePosition.line + 1) +
					":" +
					(ref.referencePosition.character + 1) +
					" " +
					ref.referenceText;
			}
			if (
		...
		const fileReferences: TreeNode[] =
			this.referencesModel.getAllFilesWithPendingReferenceNodes();
