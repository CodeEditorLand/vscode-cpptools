...
export function RegisterCodeAnalysisNotifications(
...
const PublishCodeAnalysisDiagnosticsNotification: NotificationType<PublishCodeAnalysisDiagnosticsParams> =
	new NotificationType<PublishCodeAnalysisDiagnosticsParams>(
		"cpptools/publishCodeAnalysisDiagnostics",
	);
const PublishRemoveCodeAnalysisCodeActionFixesNotification: NotificationType<RemoveCodeAnalysisCodeActionFixesParams> =
	new NotificationType<RemoveCodeAnalysisCodeActionFixesParams>(
		"cpptools/publishRemoveCodeAnalysisCodeActionFixes",
	);

export const codeAnalysisFileToCodeActions: Map<
	string,
...
export const codeAnalysisAllFixes: CodeActionAllInfo = {
	...
	removeAllCodeAction: {
		...
		command: {
			title: "RemoveAllCodeAnalysisProblems",
...
function rebuildCodeAnalysisCodeAndAllFixes(): void {
	if (
		codeAnalysisAllFixes.fixAllCodeAction.command?.arguments !== undefined
	) {
	...
	const identifiersAndUrisForAllFixes: CodeAnalysisDiagnosticIdentifiersAndUri[] =
		[];
	const uriToEditsForAll: Map<vscode.Uri, vscode.TextEdit[]> = new Map<
		vscode.Uri,
		vscode.TextEdit[]
	>();
	...
	for (const codeToFixes of codeAnalysisCodeToFixes) {
		const identifiersAndUris: CodeAnalysisDiagnosticIdentifiersAndUri[] =
			[];
		const uriToEdits: Map<vscode.Uri, vscode.TextEdit[]> = new Map<
			vscode.Uri,
			vscode.TextEdit[]
		>();
		codeToFixes[1].uriToInfo.forEach(
			(perUriInfo: CodeActionPerUriInfo, uri: string) => {
				...
				if (
					perUriInfo.workspaceEdits === undefined ||
					perUriInfo.numValidWorkspaceEdits === 0
				) {
					return;
				}
		...
		if (uriToEdits.size > 0) {
			const allTypeWorkspaceEdit: vscode.WorkspaceEdit =
...
export function publishCodeAnalysisDiagnostics(
	...
	if (!diagnosticsCollectionCodeAnalysis) {
		diagnosticsCollectionCodeAnalysis =
			vscode.languages.createDiagnosticCollection("clang-tidy");
	}
	...
	const previousDiagnostics: CodeActionDiagnosticInfo[] | undefined =
		codeAnalysisFileToCodeActions.get(params.uri);
	...
	for (const d of params.diagnostics) {
		...
		if (d.workspaceEdits) {
			...
			for (const workspaceEdit of d.workspaceEdits) {
				codeActionWorkspaceEdit.workspaceEdit.set(
					vscode.Uri.parse(workspaceEdit.file, true),
			...
			const fixThisCodeAction: vscode.CodeAction = {
				...
				command: {
					...
					arguments: [
						nextVersion,
						codeActionWorkspaceEdit.workspaceEdit,
						true,
						[identifiersAndUri],
					],
				},
		...
		if (identifier.code.length !== 0) {
			...
			for (const rootAndRelatedIdentifiersAndUri of rootAndRelatedIdentifiersAndUris) {
				const existingInfo: CodeActionPerUriInfo =
				...
				const rootAndRelatedWorkspaceEdit: CodeActionWorkspaceEdit =
					rootAndRelatedWorkspaceEdits[
						rootAndRelatedWorkspaceEditsIndex
					];
				if (rootAndRelatedWorkspaceEdit !== undefined) {
					if (existingInfo.workspaceEdits === undefined) {
						existingInfo.workspaceEdits = [
							rootAndRelatedWorkspaceEdit,
						];
					} else {
						existingInfo.workspaceEdits.push(
							rootAndRelatedWorkspaceEdit,
						);
					}
			...
			if (!identifier.code.startsWith("clang-diagnostic-")) {
				...
				if (primaryCode === "clang-tidy-nolint") {
					...
					const clangAnalyzerIndex: number =
						primaryCode.indexOf(clangAnalyzerString);
					const dashIndex: number =
						clangAnalyzerIndex === 0
							? clangAnalyzerString.length
							: primaryCode.indexOf("-");
					const checksGroup: string =
						dashIndex > 0
							? `/${primaryCode.substring(0, dashIndex)}`
							: "";
					const checksPage: string =
						dashIndex > 0
							? primaryCode.substring(dashIndex + 1)
							: primaryCode;
				...
				const primaryDocUri: vscode.Uri = vscode.Uri.parse(
					`https://releases.llvm.org/18.1.6/tools/clang/tools/extra/docs/clang-tidy/${docPage}`,
				);
				diagnostic.code = {
...
function removeCodeAnalysisCodeActions(
	identifiersAndUris: CodeAnalysisDiagnosticIdentifiersAndUri[],
	removeFixesOnly: boolean,
): void {
	for (const identifiersAndUri of identifiersAndUris) {
		const codeActionDiagnosticInfo: CodeActionDiagnosticInfo[] | undefined =
			codeAnalysisFileToCodeActions.get(identifiersAndUri.uri);
		...
		for (const identifier of identifiersAndUri.identifiers) {
			...
			for (const codeAction of codeActionDiagnosticInfo) {
				if (
					rangeEquals(codeAction.range, identifier.range) &&
					codeAction.code === identifier.code
				) {
					if (removeFixesOnly) {
						++codeAction.version;
			...
			for (const codeFixes of codeAnalysisCodeToFixes) {
				const codeActionInfo: CodeActionPerUriInfo | undefined =
					codeFixes[1].uriToInfo.get(identifiersAndUri.uri);
				...
				for (
					...
					if (
						identifier.code ===
							codeActionInfo.identifiers[codeActionInfoIndex]
								.code &&
						rangeEquals(
							identifier.range,
							codeActionInfo.identifiers[codeActionInfoIndex]
				...
				if (removedCodeActionInfoIndex !== -1) {
					if (removeFixesOnly) {
						if (codeActionInfo.workspaceEdits !== undefined) {
							codeActionInfo.workspaceEdits[
								removedCodeActionInfoIndex
							].workspaceEdit = undefined;
							--codeActionInfo.numValidWorkspaceEdits;
						}
					} else {
						codeActionInfo.identifiers.splice(
							removedCodeActionInfoIndex,
							1,
						);
						if (codeActionInfo.workspaceEdits !== undefined) {
							codeActionInfo.workspaceEdits.splice(
								removedCodeActionInfoIndex,
								1,
							);
							--codeActionInfo.numValidWorkspaceEdits;
...
export function publishRemoveCodeAnalysisCodeActionFixes(
	params: RemoveCodeAnalysisCodeActionFixesParams,
): void {
...
export function removeCodeAnalysisProblems(
	identifiersAndUris: CodeAnalysisDiagnosticIdentifiersAndUri[],
): boolean {
	if (!diagnosticsCollectionCodeAnalysis) {
		return false;
	...
	for (const identifiersAndUri of identifiersAndUris) {
		...
		const diagnostics: readonly vscode.Diagnostic[] | undefined =
			diagnosticsCollectionCodeAnalysis.get(uri);
		...
		for (const diagnostic of diagnostics) {
			...
			for (const identifier of identifiersAndUri.identifiers) {
				if (
					code !== identifier.code ||
					!rangeEquals(diagnostic.range, identifier.range)
				) {
					continue;
				}
				removed = true;
