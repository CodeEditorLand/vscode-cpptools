...
export function makeLspRange(vscRange: vscode.Range): Range {
	return {
		start: {
			line: vscRange.start.line,
			character: vscRange.start.character,
		},
...
export function makeVscodeRange(cpptoolsRange: Range): vscode.Range {
	return new vscode.Range(
		cpptoolsRange.start.line,
		cpptoolsRange.start.character,
		cpptoolsRange.end.line,
		cpptoolsRange.end.character,
	);
}

export function makeVscodeLocation(
	cpptoolsLocation: Location,
): vscode.Location {
	return new vscode.Location(
		vscode.Uri.parse(cpptoolsLocation.uri),
		makeVscodeRange(cpptoolsLocation.range),
	);
}

export function makeVscodeTextEdits(
	cpptoolsTextEdits: TextEdit[],
): vscode.TextEdit[] {
	return cpptoolsTextEdits.map(
		(textEdit) =>
			new vscode.TextEdit(
				makeVscodeRange(textEdit.range),
				textEdit.newText,
			),
	);
}
...
export function rangeEquals(
	range1: vscode.Range | Range,
	range2: vscode.Range | Range,
): boolean {
	return (
...
export function showInstallCompilerWalkthrough(): void {
	...
	const version =
		platform === "windows" ? SessionState.windowsVersion.get() : "";
