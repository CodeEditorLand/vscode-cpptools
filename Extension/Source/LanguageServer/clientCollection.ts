...
export class ClientCollection {
	...
	public get ActiveClient(): cpptools.Client {
	...
	public get Count(): number {
		return this.languageClients.size;
	}
	...
	constructor() {
		...
		if (
			...
			vscode.workspace.workspaceFolders.forEach((folder) => {
				const newClient: cpptools.Client =
					cpptools.createClient(folder);
			...
			const client: cpptools.Client | undefined =
	...
	public async didChangeActiveEditor(
		...
		const activeClient: cpptools.Client = !editor
			? this.defaultClient
			: this.getClientFor(editor.document.uri);
	 ...
	 */
	public async recreateClients(
		switchToFailsafeMode?: boolean,
	): Promise<void> {
		// Swap out the map, so we are not changing it while iterating over it.
		const oldLanguageClients: Map<string, cpptools.Client> =
		...
		for (const pair of oldLanguageClients) {
			...
			const newClient: cpptools.Client = this.createClient(
				client.RootFolder,
				true,
			);
		...
		if (this.activeDocument) {
			...
			await this.activeClient.didChangeActiveEditor(
	...
	private async onDidChangeWorkspaceFolders(
		...
		const folderCount: number = vscode.workspace.workspaceFolders
			? vscode.workspace.workspaceFolders.length
			: 0;

		if (folderCount > 1) {
			telemetry.logLanguageServerEvent("workspaceFoldersChange", {
				"count": folderCount.toString(),
			});
		}
		...
		if (e !== undefined) {
			...
			if (this.activeDocument) {
				const newActiveClient: cpptools.Client = this.getClientFor(
	...
	public getClientFor(uri: vscode.Uri): cpptools.Client {
		const folder: vscode.WorkspaceFolder | undefined = uri
	...
	public createClient(
		...
		const newClient: cpptools.Client = this.useFailsafeMode
			? cpptools.createNullClient()
			: cpptools.createClient(folder);
		...
		getCustomConfigProviders().forEach(
			(provider) =>
				void newClient.onRegisterCustomConfigurationProvider(provider),
		);
