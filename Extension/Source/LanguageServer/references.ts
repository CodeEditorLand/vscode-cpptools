...
export type ReferencesResultCallback = (
	result: ReferencesResult | null,
) => void;
...
export function referencesCommandModeToString(
	referencesCommandMode: ReferencesCommandMode,
): string {
...
export function convertReferenceTypeToString(
	...
	if (upperCase) {
		switch (referenceType) {
			case ReferenceType.Confirmed:
				return localize(
					"confirmed.reference.upper",
					"CONFIRMED REFERENCE",
				);

			case ReferenceType.ConfirmationInProgress:
				return localize(
					"confirmation.in.progress.upper",
					"CONFIRMATION IN PROGRESS",
				);

			case ReferenceType.Comment:
				return localize("comment.reference.upper", "COMMENT REFERENCE");

			case ReferenceType.String:
				return localize("string.reference.upper", "STRING REFERENCE");

			case ReferenceType.Inactive:
				return localize(
					"inactive.reference.upper",
					"INACTIVE REFERENCE",
				);

			case ReferenceType.CannotConfirm:
				return localize(
					"cannot.confirm.reference.upper",
					"CANNOT CONFIRM REFERENCE",
				);

			case ReferenceType.NotAReference:
				return localize("not.a.reference.upper", "NOT A REFERENCE");
		}
	} else {
		switch (referenceType) {
			case ReferenceType.Confirmed:
				return localize("confirmed.reference", "Confirmed reference");
			...
			case ReferenceType.Comment:
				return localize("comment.reference", "Comment reference");

			case ReferenceType.String:
				return localize("string.reference", "String reference");

			case ReferenceType.Inactive:
				return localize("inactive.reference", "Inactive reference");
			...
			case ReferenceType.NotAReference:
				return localize("not.a.reference", "Not a reference");
		}
	}
}
...
export function getReferenceTagString(
	referenceType: ReferenceType,
	referenceCanceled: boolean,
	upperCase?: boolean,
): string {
	return referenceCanceled &&
		referenceType === ReferenceType.ConfirmationInProgress
		? getReferenceCanceledString(upperCase)
		: convertReferenceTypeToString(referenceType, upperCase);
}
...
export function getReferenceTypeIconPath(referenceType: ReferenceType): {
	...
	const lightPath: string = util.getExtensionFilePath(
		assetsFolder + basePath + postFixLight,
	);

	const lightPathUri: vscode.Uri = vscode.Uri.file(lightPath);

	const darkPath: string = util.getExtensionFilePath(
		assetsFolder + basePath + postFixDark,
	);
...
function getReferenceCanceledIconPath(): {
	...
	const lightPath: string = util.getExtensionFilePath(
		"assets/ref-canceled-light.svg",
	);

	const lightPathUri: vscode.Uri = vscode.Uri.file(lightPath);

	const darkPath: string = util.getExtensionFilePath(
		"assets/ref-canceled-dark.svg",
	);
...
export function getReferenceItemIconPath(
	type: ReferenceType,
	isCanceled: boolean,
): { light: vscode.Uri; dark: vscode.Uri } {
	return isCanceled && type === ReferenceType.ConfirmationInProgress
		? getReferenceCanceledIconPath()
		: getReferenceTypeIconPath(type);
}
...
export class ReferencesManager {
	...
	public updateVisibleRange(visibleRangesLength: number): void {
		this.visibleRangesDecreased =
			visibleRangesLength < this.prevVisibleRangesLength;
	...
	private reportProgress(
		...
		forceUpdate: boolean,
		mode: ReferencesCommandMode,
	): void {
		const helpMessage: string =
			mode !== ReferencesCommandMode.Find
				? ""
				: ` ${localize("click.search.icon", "To preview results, click the search icon in the status bar.")}`;
		...
		if (this.referencesCurrentProgress) {
			switch (this.referencesCurrentProgress.referencesProgress) {
				...
				case ReferencesProgress.StartedCallHierarchy:
					progress.report({
						message: localize("started", "Started."),
						increment: 0,
					});
				...
				case ReferencesProgress.ProcessingTargets:
					...
					for (const targetLocationProgress of this
						.referencesCurrentProgress.targetReferencesProgress) {
					...
					const numTotalToLex: number =
						this.referencesCurrentProgress.targetReferencesProgress
							.length;

					const numFinishedLexing: number =
						numTotalToLex - numWaitingToLex - numLexing;

					const numTotalToParse: number =
						this.referencesCurrentProgress.targetReferencesProgress
							.length - numFinishedWithoutConfirming;

					if (
						numLexing >= numParsing + numConfirmingReferences &&
						numFinishedConfirming === 0
					) {
						if (numTotalToLex === 0) {
							currentMessage = localize(
								"searching.files",
								"Searching files.",
					...
					const currentParseProgress: number =
						(numConfirmingReferences * confirmingWeight +
							numFinishedConfirming) /
						numTotalToParse;

					const averageLexingPercent: number = 25;

					const currentIncrement: number =
						currentLexProgress * averageLexingPercent +
						currentParseProgress * (100 - averageLexingPercent);

					if (
						forceUpdate ||
						currentIncrement >
							this.referencesPrevProgressIncrement ||
						currentMessage !== this.referencesPrevProgressMessage
					) {
						progress.report({
	...
	private handleProgressStarted(
		...
		if (referencesProgress === ReferencesProgress.StartedCallHierarchy) {
			...
			this.visibleRangesDecreased &&
			Date.now() - this.visibleRangesDecreasedTicks <
				this.ticksForDetectingPeek
		) {
	...
	public handleProgress(
		notificationBody: ReportReferencesProgressNotification,
	): void {
	...
	public showResultsInPanelView(referencesResult: ReferencesResult): void {
		...
		if (
			this.client.ReferencesCommandMode === ReferencesCommandMode.Peek &&
			!this.referencesChannel
		) {
		...
		if (this.referencesStartedWhileTagParsing) {
			const msg: string = localize(
				...
				"[Warning] Some references may be missing, because workspace parsing was incomplete when {0} was started.",
			...
			if (
				...
				this.client.ReferencesCommandMode === ReferencesCommandMode.Find
			) {
		...
		if (
			...
			if (
				...
				const showConfirmedReferences: boolean =
					referencesResult.isCanceled;

				if (this.findAllRefsView) {
					const peekReferencesResults: string =
						this.findAllRefsView.getResultsAsText(
							showConfirmedReferences,
						);
	...
	public clearViews(): void {
		...
		if (
			this.client.ReferencesCommandMode !== ReferencesCommandMode.Rename
		) {
			if (this.referencesChannel) {
