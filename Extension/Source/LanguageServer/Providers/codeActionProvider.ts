...
export const GetCodeActionsRequest: RequestType<
	GetCodeActionsRequestParams,
	GetCodeActionsResult,
	void
> = new RequestType<GetCodeActionsRequestParams, GetCodeActionsResult, void>(
	"cpptools/getCodeActions",
);
...
export class CodeActionProvider implements vscode.CodeActionProvider {
	...
	public async provideCodeActions(
		...
		const processCommand = (command: CodeActionCommand) => {
			let title: string = getLocalizedString(
				command.localizeStringParams,
			);

			let wsEdit: vscode.WorkspaceEdit | undefined;

			let codeActionKind: vscode.CodeActionKind =
			...
			if (command.edit) {
				...
				command.command === "C_Cpp.RemoveAllCodeAnalysisProblems" &&
				command.uri !== undefined
			) {
				// The "RemoveAll" message is sent for all code analysis squiggles.
				const vsCodeRange: vscode.Range = makeVscodeRange(r);

				const codeActionDiagnosticInfo:
					| CodeActionDiagnosticInfo[]
					| undefined = codeAnalysisFileToCodeActions.get(
					command.uri,
				);
				...
				const showClear: string = new CppSettings()
					.clangTidyCodeActionShowClear;
				...
				for (const codeAction of codeActionDiagnosticInfo) {
					...
					if (codeAnalysisCodeToFixes.has(codeAction.code)) {
						codeActionCodeInfo = codeAnalysisCodeToFixes.get(
							codeAction.code,
						);
					}
					...
					if (codeAction.fixCodeAction !== undefined) {
						...
						if (codeActionCodeInfo !== undefined) {
							if (
								codeActionCodeInfo.fixAllTypeCodeAction !==
									undefined &&
								(codeActionCodeInfo.uriToInfo.size > 1 ||
									(codeActionCodeInfo.uriToInfo
										.values()
										.next().value?.numValidWorkspaceEdits ??
										0) > 1)
							) {
					...
					if (codeActionCodeInfo !== undefined) {
						...
						if (
							...
							(codeActionCodeInfo.uriToInfo.size > 1 ||
								(codeActionCodeInfo.uriToInfo.values().next()
									.value?.identifiers?.length ?? 0) > 1)
						) {
					...
					if (showClear !== "None") {
						if (
							...
							showClear === "AllAndAllTypeAndThis"
						) {
				...
				if (fixCodeActions.length > 0) {
					...
					if (
						...
						resultCodeActions.push(
							codeAnalysisAllFixes.fixAllCodeAction,
						);
					}
				}
				...
				if (showClear !== "None") {
					...
					if (
						codeActionDiagnosticInfo.length > 1 ||
						codeAnalysisFileToCodeActions.size > 1
					) {
				...
				(command.command === "C_Cpp.CreateDeclarationOrDefinition" ||
					command.command === "C_Cpp.CopyDeclarationOrDefinition") &&
				...
				command.command === "C_Cpp.ConfigurationEdit" &&
				hasSelectIntelliSenseConfiguration
			) {
				if (hasConfigurationSet) {
				...
				command.command === "C_Cpp.ExtractToFunction" ||
		...
		if (
			!hasInlineMacro &&
			context.only?.value === "refactor.inline.macro"
		) {
			const processInlineMacro = async (): Promise<boolean> => {
				...
				if (
					!hoverResult.value.includes(
						localize("expands.to", "Expands to:"),
					)
				) {
					return false;
				}
				...
				try {
					...
					if (
						e instanceof ResponseError &&
						(e.code === RequestCancelled ||
							e.code === ServerCancelled)
					) {
						return false;
					}
