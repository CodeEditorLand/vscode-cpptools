...
export class HoverProvider implements vscode.HoverProvider {
	...
	public async provideHover(
		...
		if (copilotHoverProvider) {
			...
			if (
				!copilotHoverProvider.isNewHover(document, position) &&
				this.lastContent
			) {
		...
		try {
			hoverResult = await this.client.languageClient.sendRequest(
				HoverRequest,
				params,
				token,
			);
		...
		for (const element of hoverResult.contents) {
			const oldMarkdownString: vscode.MarkdownString =
				element as vscode.MarkdownString;
			const newMarkdownString: vscode.MarkdownString =
