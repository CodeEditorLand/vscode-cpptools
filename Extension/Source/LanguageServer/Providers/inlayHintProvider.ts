...
export class InlayHintsProvider implements vscode.InlayHintsProvider {
	...
	public deliverInlayHints(
		uriString: string,
		cppInlayHints: CppInlayHint[],
		startNewSet: boolean,
	): void {
		...
		const newTypeHints: CppInlayHint[] = cppInlayHints.filter(
			(h) => h.inlayHintKind === InlayHintKind.Type,
		);
		...
		if (settings.inlayHintsAutoDeclarationTypes) {
			const resolvedTypeHints: vscode.InlayHint[] = this.resolveTypeHints(
				settings,
				newTypeHints,
			);
	...
	private resolveTypeHints(
		...
		for (const hint of hints) {
			const showOnLeft: boolean =
				settings.inlayHintsAutoDeclarationTypesShowOnLeft &&
				hint.identifierLength > 0;

			const inlayHint: vscode.InlayHint = new vscode.InlayHint(
				new vscode.Position(
					hint.line,
					hint.character + (showOnLeft ? 0 : hint.identifierLength),
				),
