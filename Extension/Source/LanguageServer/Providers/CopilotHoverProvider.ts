...
export class CopilotHoverProvider implements vscode.HoverProvider {
	...
	public async provideHover(
		...
		if (!newHover) {
			if (this.ready) {
				const contentMarkdown = new vscode.MarkdownString(
					`$(sparkle) Copilot\n\n${this.content}`,
					true,
				);
		...
		const commandString =
			"$(sparkle) [" +
			localize(
				"generate.copilot.description",
				"Generate Copilot summary",
			) +
			'](command:C_Cpp.ShowCopilotHover "' +
			localize(
				"copilot.disclaimer",
		...
		commandMarkdown.isTrusted = {
			enabledCommands: ["C_Cpp.ShowCopilotHover"],
		};
	...
	public getCurrentHoverCancellationToken():
	...
	public async getRequestInfo(
		...
		try {
			const response = await this.client.languageClient.sendRequest(
				GetCopilotHoverInfoRequest,
				params,
	...
	public isCancelled(
		...
		if (
			this.cancelled &&
			this.cancelledDocument === document &&
			this.cancelledPosition === position
		) {
	...
	public isNewHover(
		...
		return !(
			...
			(this.currentPosition?.character === position.character ||
				this.currentPosition?.character === position.character - 1)
		);
	}
}
