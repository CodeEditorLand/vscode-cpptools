...
import {
	CancellationSender,
	getReferenceItemIconPath,
	getReferenceTagString,
	ReferencesParams,
	ReferencesResult,
	ReferenceType,
} from "../references";
...
const RenameRequest: RequestType<ReferencesParams, ReferencesResult, void> =
	new RequestType<ReferencesParams, ReferencesResult, void>(
		"cpptools/rename",
	);
...
export class RenameProvider implements vscode.RenameProvider {
	...
	public async provideRenameEdits(
		...
		if (
			settings.renameRequiresIdentifier &&
			!util.isValidIdentifier(newName)
		) {
			void vscode.window.showErrorMessage(
				localize(
					"invalid.identifier.for.rename",
					"Invalid identifier provided for the Rename Symbol operation.",
				),
			);
			return undefined;
		}
		...
		const workspaceEditResult: vscode.WorkspaceEdit =
		...
		if (cancelSource.token.isCancellationRequested || response.isCanceled) {
			...
			void vscode.window.showErrorMessage(
				localize(
					"unable.to.locate.selected.symbol",
			...
			for (const reference of response.referenceInfos) {
				...
				const range: vscode.Range = new vscode.Range(
					...
					reference.position.line,
					reference.position.character + response.text.length,
				);
				const metadata: vscode.WorkspaceEditEntryMetadata = {
					needsConfirmation:
						reference.type !== ReferenceType.Confirmed,
