...
const CallHierarchyItemRequest: RequestType<
	CallHierarchyParams,
	CallHierarchyItemResult,
	void
> = new RequestType<CallHierarchyParams, CallHierarchyItemResult, void>(
	"cpptools/prepareCallHierarchy",
);

const CallHierarchyCallsToRequest: RequestType<
	CallHierarchyParams,
	CallHierarchyCallsItemResult,
	void
> = new RequestType<CallHierarchyParams, CallHierarchyCallsItemResult, void>(
	"cpptools/callHierarchyCallsTo",
);

const CallHierarchyCallsFromRequest: RequestType<
	CallHierarchyParams,
	CallHierarchyCallsItemResult,
	void
> = new RequestType<CallHierarchyParams, CallHierarchyCallsItemResult, void>(
	"cpptools/callHierarchyCallsFrom",
);
...
export class CallHierarchyProvider implements vscode.CallHierarchyProvider {
	...
	public async prepareCallHierarchy(
		...
		const cancelSource: vscode.CancellationTokenSource =
		...
		const requestCanceledListener: vscode.Disposable =
			workspaceReferences.onCancellationRequested((_sender) => {
				cancelSource.cancel();
			});
	...
	public async provideCallHierarchyIncomingCalls(
		...
		if (item === undefined) {
			this.logTelemetry(
				...
				CallHierarchyRequestStatus.Failed,
			);
		...
		if (
			...
			cancelled ||
			requestCanceled !== undefined
		) {
			const requestStatus: CallHierarchyRequestStatus =
		...
		this.logTelemetry(
			CallHierarchyCallsToEvent,
			CallHierarchyRequestStatus.Succeeded,
			progressBarDuration,
		);
	...
	public async provideCallHierarchyOutgoingCalls(
		...
		try {
			await this.client.languageClient.sendRequest(
				CallHierarchyCallsFromRequest,
				params,
				token,
			);
		} catch (e: any) {
	...
	private makeVscodeCallHierarchyItem(
		item: CallHierarchyItem,
	): vscode.CallHierarchyItem {
		const containerDetail: string =
