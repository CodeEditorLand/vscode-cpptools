...
export class FoldingRangeProvider implements vscode.FoldingRangeProvider {
	...
	private pendingRequests: Map<string, FoldingRangeRequestInfo> = new Map<
		string,
	...
	async provideFoldingRanges(
		...
		const pendingRequest: FoldingRangeRequestInfo | undefined =
			this.pendingRequests.get(document.uri.toString());

		if (pendingRequest !== undefined) {
			if (pendingRequest.promise === undefined) {
				pendingRequest.promise = new ManualPromise<
		...
		if (foldingRangeRequestInfo.promise !== undefined) {
			promise.then(
				...
				() => {
					foldingRangeRequestInfo.promise?.reject(
