...
export class SettingsPanel {
	...
	public createOrShow(
		...
		activeConfiguration: config.Configuration,
		...
		this.panel = vscode.window.createWebviewPanel(
			...
			column || vscode.ViewColumn.One,
			{
				...
				localResourceRoots: [
					...
					vscode.Uri.file(
						path.join(util.extensionPath, "dist", "ui"),
					),
				],
			},
		);
		...
		this.panel.iconPath = vscode.Uri.file(
			util.getExtensionFilePath("LanguageCCPP_color_128x.png"),
		);
	...
	public updateConfigUI(
		...
		errors: config.ConfigurationErrors | null,
	): void {
		if (this.panel) {
	...
	public setKnownCompilers(
		knownCompilers?: config.KnownCompiler[],
	...
	private updateConfig(message: any): void {
		const splitEntries: (input: any) => string[] | undefined = (
			input: any,
		) => {
		...
		switch (message.key) {
			...
			case elementId.windowsSdkVersion:
				this.configValues.windowsSdkVersion =
					message.value || undefined;

				break;

			case elementId.macFrameworkPath:
				this.configValues.macFrameworkPath = splitEntries(
					message.value,
				);
			...
			case elementId.configurationProvider:
				this.configValues.configurationProvider =
					message.value || undefined;
	...
	private getHtml(): string {
		...
		content = fs
			.readFileSync(getLocalizedHtmlPath("ui/settings.html"))
			.toString();

		if (this.panel && this.panel.webview) {
			const cppImageUri: vscode.Uri = this.panel.webview.asWebviewUri(
				vscode.Uri.file(
					path.join(
						util.extensionPath,
