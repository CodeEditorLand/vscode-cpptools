...
function getMLContinuePattern(insert: string): string | undefined {
	if (insert) {
		...
		if (match) {
			const right: string = escape(
				insert.substring(insert.trimRight().length),
			);
			return `^(\\t|[ ])*${match}(${right}([^\\*]|\\*(?!\\/))*)?$`;
		}
...
function getMLSplitRule(
	comment: CommentPattern,
): vscode.OnEnterRule | undefined {
...
function getMLContinuationRule(
	comment: CommentPattern,
): vscode.OnEnterRule | undefined {
	const previousLinePattern: string | undefined = getMLPreviousLinePattern(
		comment.begin,
	);
	if (previousLinePattern) {
		const beforePattern: string | undefined = getMLContinuePattern(
			comment.continue,
		);
		if (beforePattern) {
			return {
	...
	if (beforePattern) {
		return {
			...
			action: {
				indentAction: vscode.IndentAction.None,
				removeText:
					comment.continue.length -
					comment.continue.trimLeft().length,
			},
		};
	}
	return undefined;
...
function getMLEmptyEndRule(
	comment: CommentPattern,
): vscode.OnEnterRule | undefined {
	const beforePattern: string | undefined = getMLEmptyEndPattern(
		comment.continue,
	);
	if (beforePattern) {
		return {
...
function getSLEndRule(comment: CommentPattern): vscode.OnEnterRule {
	...
	return {
		...
		action: {
			indentAction: vscode.IndentAction.None,
			removeText:
				comment.continue.length - comment.continue.trimLeft().length,
		},
	};
}
...
export function getLanguageConfig(
	...
	const patterns: (string | CommentPattern)[] | undefined =
		settings.commentContinuationPatterns;
	return getLanguageConfigFromPatterns(languageId, patterns);
}

export function getLanguageConfigFromPatterns(
	languageId: string,
	patterns?: (string | CommentPattern)[],
): vscode.LanguageConfiguration {
	...
	patterns.forEach((pattern) => {
		const c: CommentPattern = isString(pattern)
			? {
					begin: pattern,
					continue: pattern.startsWith("/*") ? " * " : pattern,
				}
	...
	if (duplicates) {
		getOutputChannel().appendLine(
			localize(
				"duplicate.multiline.patterns",
				"Duplicate multiline comment patterns detected.",
			),
		);
	}
	return {
...
function constructCommentRules(
	comment: CommentPattern,
	languageId: string,
): Rules {
	if (
		comment?.begin?.startsWith("/*") &&
		(languageId === "c" ||
			languageId === "cpp" ||
			languageId === "cuda-cpp")
	) {
		const mlBegin1: vscode.OnEnterRule | undefined =
		...
		comment?.begin?.startsWith("//") &&
		(languageId === "cpp" || languageId === "cuda-cpp")
	) {
