...
export class SettingsTracker {
	...
	public getUserModifiedSettings(): { [key: string]: string } {
		const filter: FilterFunction = (
			...
			val: string,
			settings: vscode.WorkspaceConfiguration,
		) => !this.areEqual(val, settings.inspect(key)?.defaultValue);

		return this.collectSettings(filter);
	}

	public getChangedSettings(): { [key: string]: string } {
		const filter: FilterFunction = (key: string, val: string) =>
			!(key in this.previousCppSettings) ||
			!this.areEqual(val, this.previousCppSettings[key]);

		return this.collectSettings(filter);
	}

	private collectSettings(filter: FilterFunction): { [key: string]: string } {
		const settingsResourceScope: vscode.WorkspaceConfiguration =
			vscode.workspace.getConfiguration("C_Cpp", this.resource);

		const settingsNonScoped: vscode.WorkspaceConfiguration =
			vscode.workspace.getConfiguration("C_Cpp");

		const selectCorrectlyScopedSettings = (
			rawSetting: any,
		): vscode.WorkspaceConfiguration =>
			!rawSetting ||
			rawSetting.scope === "resource" ||
			rawSetting.scope === "machine-overridable"
		...
		for (const key in settingsResourceScope) {
			...
			const correctlyScopedSettings: vscode.WorkspaceConfiguration =
				selectCorrectlyScopedSettings(rawSetting);
			...
			const collectSettingsRecursive = (
				key: string,
				...
				for (const subKey in val) {
					...
					const newRawSetting: any = util.getRawSetting(
						"C_Cpp." + newKey,
					);

					const correctlyScopedSubSettings: vscode.WorkspaceConfiguration =
						selectCorrectlyScopedSettings(newRawSetting);

					const subVal: any = this.getSetting(
						correctlyScopedSubSettings,
						newKey,
					);
					...
					if (
						...
						const entry: KeyValuePair | undefined =
							this.filterAndSanitize(
								newKey,
								subVal,
								correctlyScopedSubSettings,
								filter,
			...
			const entry: KeyValuePair | undefined = this.filterAndSanitize(
				key,
				val,
				correctlyScopedSettings,
				filter,
			);
	...
	private getSetting(
		...
		if (settings.inspect(key)?.defaultValue !== undefined) {
			...
			if (curSetting) {
				const type: string | undefined = this.typeMatch(
					val,
					curSetting["type"],
				);
				...
				if (type) {
					...
					if (
						curEnum &&
						curEnum.indexOf(val) === -1 &&
						(key !== "loggingLevel" ||
							util.getNumericLoggingLevel(val) === -1)
					) {
						return "<invalid>";
					}
			...
			switch (key) {
				...
				default: {
					if (
						...
						key === "intelliSenseCachePath" ||
						key.startsWith("default.") ||
						key === "codeAnalysis.clangTidy.path" ||
						key === "codeAnalysis.clangTidy.headerFilter" ||
						key === "codeAnalysis.clangTidy.args" ||
						key === "codeAnalysis.clangTidy.config" ||
						key === "codeAnalysis.clangTidy.fallbackConfig" ||
