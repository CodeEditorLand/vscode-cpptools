...
export async function activate(): Promise<void> {
	...
	const isRelatedFilesApiEnabled = await telemetry.isExperimentEnabled(
		"CppToolsRelatedFilesApi",
	);
			...
			try {
				for (const languageId of ["c", "cpp", "cuda-cpp"]) {
					api.registerRelatedFilesProvider(
						{
							extensionId: util.extensionContext.extension.id,
							languageId,
						},
						async (
							_uri: vscode.Uri,
							_context: { flags: Record<string, unknown> },
							token: vscode.CancellationToken,
						) => ({
							entries:
								(
									await getIncludesWithCancellation(1, token)
				...
				console.log(
					"Failed to register Copilot related files provider.",
				);
			}
		}
	}
...
export function registerCommands(
	enabled: boolean,
	isRelatedFilesApiEnabled: boolean,
): void {
	...
	if (!isRelatedFilesApiEnabled) {
		commandDisposables.push(
			vscode.commands.registerCommand(
				"C_Cpp.getIncludes",
				enabled
					? (maxDepth: number) => getIncludes(maxDepth)
					: () => Promise.resolve(),
			),
		);
	}
...
export async function getIncludesWithCancellation(
	maxDepth: number,
	token: vscode.CancellationToken,
	...
	includes.includedFiles = includes.includedFiles.filter((header) =>
		vscode.Uri.file(header).toString().startsWith(wksFolder),
	);
	return includes;
}
...
async function getIncludes(maxDepth: number): Promise<GetIncludesResult> {
	...
	try {
		const includes = await getIncludesWithCancellation(
			maxDepth,
			tokenSource.token,
		);
		return includes;
...
async function getCopilotApi(): Promise<CopilotApi | undefined> {
	const copilotExtension =
		vscode.extensions.getExtension<CopilotApi>("github.copilot");
	if (!copilotExtension) {
		return undefined;
	}
