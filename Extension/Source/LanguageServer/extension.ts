...
import {
	Client,
	DefaultClient,
	DoxygenCodeActionCommandArguments,
	openFileVersions,
} from "./client";
...
async function initVcpkgDatabase(): Promise<VcpkgDatabase> {
	...
	try {
		const zip = new StreamZip.async({
			file: util.getExtensionFilePath("VCPkgHeadersDatabase.zip"),
		});
		try {
...
function getVcpkgHelpAction(): vscode.CodeAction {
	...
	return {
		command: {
			title: "vcpkgOnlineHelpSuggested",
...
function getVcpkgClipboardInstallAction(port: string): vscode.CodeAction {
	return {
		command: {
			title: "vcpkgClipboardInstallSuggested",
...
async function lookupIncludeInVcpkg(
	document: vscode.TextDocument,
	line: number,
): Promise<string[]> {
	const matches: RegExpMatchArray | null = document
		.lineAt(line)
		.text.match(/#include\s*[<"](?<includeFile>[^>"]*)[>"]/);
...
function isMissingIncludeDiagnostic(diagnostic: vscode.Diagnostic): boolean {
	...
	return (
		diagnostic.code === missingIncludeCode && diagnostic.source === "C/C++"
	);
}
...
function sendActivationTelemetry(): void {
	...
	if (vscode.env.machineId !== "someValue.machineId") {
		const machineIdPersistentState: PersistentState<string | undefined> =
			new PersistentState<string | undefined>("CPP.machineId", undefined);
...
export async function activate(): Promise<void> {
	...
	const checkForConflictingExtensions: PersistentState<boolean> =
		new PersistentState<boolean>(
			"CPP." +
	...
	if (checkForConflictingExtensions.Value) {
		...
		const clangCommandAdapterActive: boolean = vscode.extensions.all.some(
			(extension: vscode.Extension<any>): boolean =>
	...
	clients.forEach((client) => {
		getCustomConfigProviders().forEach(
			(provider) =>
				void client.onRegisterCustomConfigurationProvider(provider),
		);
	});

	disposables.push(
		vscode.workspace.onDidChangeConfiguration(onDidChangeSettings),
	);
	disposables.push(
	...
	disposables.push(
		vscode.window.onDidChangeTextEditorVisibleRanges((e) =>
			clients.ActiveClient.enqueue(async () =>
				onDidChangeTextEditorVisibleRanges(e),
			),
		),
	);
	disposables.push(
	...
	void clients.ActiveClient.ready.then(
	...
	vscode.tasks.onDidEndTask((event) => {
		...
		if (
			event.execution.task.definition.type ===
				CppBuildTaskProvider.CppBuildScriptType ||
			event.execution.task.name.startsWith(configPrefix)
		) {
			if (
				event.execution.task.scope !== vscode.TaskScope.Global &&
				event.execution.task.scope !== vscode.TaskScope.Workspace
			) {
				const folder: vscode.WorkspaceFolder | undefined =
					event.execution.task.scope;
				if (folder) {
					const settings: CppSettings = new CppSettings(folder.uri);
					if (
						settings.codeAnalysisRunOnBuild &&
						settings.clangTidyEnabled
					) {
						void clients
							.getClientFor(folder.uri)
			...
			if (settings.codeAnalysisRunOnBuild && settings.clangTidyEnabled) {
				void clients.ActiveClient.handleRunCodeAnalysisOnAllFiles().catch(
	...
	codeActionProvider = vscode.languages.registerCodeActionsProvider(
		selector,
		{
			provideCodeActions: async (
				...
				const ports: string[] = await lookupIncludeInVcpkg(
					document,
					range.start.line,
				);
				if (ports.length <= 0) {
				...
				telemetry.logLanguageServerEvent("codeActionsProvided", {
					"source": "vcpkg",
				});
				...
				const actions: vscode.CodeAction[] =
					ports.map<vscode.CodeAction>(
						getVcpkgClipboardInstallAction,
					);
				return actions;
			},
		},
	...
	await vscode.commands.executeCommand(
		"setContext",
		"cpptools.msvcEnvironmentFound",
		util.hasMsvcEnvironment(),
	);
	...
	if (util.extensionContext) {
		...
		if (version[0] > 1 || (version[0] === 1 && version[1] >= 95)) {
			const tool = vscode.lm.registerTool(
				"cpptools-lmtool-configuration",
				new CppConfigurationLanguageModelTool(),
			);
...
export function updateLanguageConfigurations(): void {
	...
	languageConfigurations.push(
		vscode.languages.setLanguageConfiguration("c", getLanguageConfig("c")),
	);
	languageConfigurations.push(
 ...
 * workspace events
 */
async function onDidChangeSettings(
	event: vscode.ConfigurationChangeEvent,
): Promise<void> {
	const client: Client = clients.getDefaultClient();
	if (client instanceof DefaultClient) {
		const defaultClient: DefaultClient = client as DefaultClient;
		const changedDefaultClientSettings: Record<string, string> =
			await defaultClient.onDidChangeSettings(event);
		clients.forEach((client) => {
			if (client !== defaultClient) {
				void client
					.onDidChangeSettings(event)
					.catch(logAndReturn.undefined);
			}
		});
		const newUpdateChannel: string =
			changedDefaultClientSettings.updateChannel;
		if (
			newUpdateChannel ||
			event.affectsConfiguration("extensions.autoUpdate")
		) {
...
async function onDidChangeTextEditorVisibleRanges(
	event: vscode.TextEditorVisibleRangesChangeEvent,
): Promise<void> {
	if (util.isCpp(event.textEditor.document)) {
		await clients
			.getDefaultClient()
			.onDidChangeTextEditorVisibleRanges(event.textEditor.document.uri);
	}
}
...
function onDidChangeActiveTextEditor(editor?: vscode.TextEditor): void {
	...
	console.assert(
		clients !== undefined,
		"client should be available before active editor is changed",
	);
	...
	if (!editor) {
		...
		void clients
			.didChangeActiveEditor(undefined)
			.catch(logAndReturn.undefined);
	} else {
		ui.didChangeActiveEditor();
...
async function onDidChangeVisibleTextEditors(
	editors: readonly vscode.TextEditor[],
): Promise<void> {
	const cppEditors: vscode.TextEditor[] = editors.filter((e) =>
...
export async function registerCommands(enabled: boolean): Promise<void> {
	...
	commandDisposables.push(
		vscode.commands.registerCommand(
			...
			enabled ? onSwitchHeaderSource : onDisabledCommand,
		),
	);
	commandDisposables.push(
		vscode.commands.registerCommand(
			"C_Cpp.ResetDatabase",
			enabled ? onResetDatabase : onDisabledCommand,
		),
	);
	commandDisposables.push(
		vscode.commands.registerCommand(
			"C_Cpp.SelectIntelliSenseConfiguration",
			enabled ? selectIntelliSenseConfiguration : onDisabledCommand,
		),
	);
	commandDisposables.push(
		vscode.commands.registerCommand(
			"C_Cpp.InstallCompiler",
			enabled ? installCompiler : onDisabledCommand,
		),
	);
	commandDisposables.push(
		vscode.commands.registerCommand(
			"C_Cpp.ConfigurationSelect",
			enabled ? onSelectConfiguration : onDisabledCommand,
		),
	);
	commandDisposables.push(
		vscode.commands.registerCommand(
			"C_Cpp.ConfigurationProviderSelect",
			enabled ? onSelectConfigurationProvider : onDisabledCommand,
		),
	);
	commandDisposables.push(
		vscode.commands.registerCommand(
			"C_Cpp.ConfigurationEditJSON",
			enabled ? onEditConfigurationJSON : onDisabledCommand,
		),
	);
	commandDisposables.push(
		vscode.commands.registerCommand(
			"C_Cpp.ConfigurationEditUI",
			enabled ? onEditConfigurationUI : onDisabledCommand,
		),
	);
	commandDisposables.push(
		vscode.commands.registerCommand(
			"C_Cpp.ConfigurationEdit",
			enabled ? onEditConfiguration : onDisabledCommand,
		),
	);
	commandDisposables.push(
		vscode.commands.registerCommand(
			"C_Cpp.AddToIncludePath",
			enabled ? onAddToIncludePath : onDisabledCommand,
		),
	);
	commandDisposables.push(
		vscode.commands.registerCommand(
			"C_Cpp.EnableErrorSquiggles",
			enabled ? onEnableSquiggles : onDisabledCommand,
		),
	);
	commandDisposables.push(
		vscode.commands.registerCommand(
			"C_Cpp.DisableErrorSquiggles",
			enabled ? onDisableSquiggles : onDisabledCommand,
		),
	);
	commandDisposables.push(
		vscode.commands.registerCommand(
			"C_Cpp.ToggleDimInactiveRegions",
			enabled ? onToggleDimInactiveRegions : onDisabledCommand,
		),
	);
	commandDisposables.push(
		vscode.commands.registerCommand(
			"C_Cpp.PauseParsing",
			enabled ? onPauseParsing : onDisabledCommand,
		),
	);
	commandDisposables.push(
		vscode.commands.registerCommand(
			"C_Cpp.ResumeParsing",
			enabled ? onResumeParsing : onDisabledCommand,
		),
	);
	commandDisposables.push(
		vscode.commands.registerCommand(
			"C_Cpp.PauseCodeAnalysis",
			enabled ? onPauseCodeAnalysis : onDisabledCommand,
		),
	);
	commandDisposables.push(
		vscode.commands.registerCommand(
			"C_Cpp.ResumeCodeAnalysis",
			enabled ? onResumeCodeAnalysis : onDisabledCommand,
		),
	);
	commandDisposables.push(
		vscode.commands.registerCommand(
			"C_Cpp.CancelCodeAnalysis",
			enabled ? onCancelCodeAnalysis : onDisabledCommand,
		),
	);
	commandDisposables.push(
		vscode.commands.registerCommand(
			"C_Cpp.ShowActiveCodeAnalysisCommands",
			enabled ? onShowActiveCodeAnalysisCommands : onDisabledCommand,
		),
	);
	commandDisposables.push(
		vscode.commands.registerCommand(
			"C_Cpp.ShowIdleCodeAnalysisCommands",
			enabled ? onShowIdleCodeAnalysisCommands : onDisabledCommand,
		),
	);
	commandDisposables.push(
		vscode.commands.registerCommand(
			"C_Cpp.ShowReferencesProgress",
			enabled ? onShowReferencesProgress : onDisabledCommand,
		),
	);
	commandDisposables.push(
		vscode.commands.registerCommand(
			"C_Cpp.TakeSurvey",
			enabled ? onTakeSurvey : onDisabledCommand,
		),
	);
	commandDisposables.push(
		vscode.commands.registerCommand(
			"C_Cpp.LogDiagnostics",
			enabled ? onLogDiagnostics : onDisabledCommand,
		),
	);
	commandDisposables.push(
		vscode.commands.registerCommand(
			"C_Cpp.RescanWorkspace",
			enabled ? onRescanWorkspace : onDisabledCommand,
		),
	);
	commandDisposables.push(
		vscode.commands.registerCommand(
			"C_Cpp.ShowReferenceItem",
			enabled ? onShowRefCommand : onDisabledCommand,
		),
	);
	commandDisposables.push(
		vscode.commands.registerCommand(
			"C_Cpp.referencesViewGroupByType",
			enabled ? onToggleRefGroupView : onDisabledCommand,
		),
	);
	commandDisposables.push(
		vscode.commands.registerCommand(
	...
	commandDisposables.push(
		vscode.commands.registerCommand(
			...
			enabled ? onVcpkgClipboardInstallSuggested : onDisabledCommand,
		),
	);
	commandDisposables.push(
		vscode.commands.registerCommand(
			"C_Cpp.VcpkgOnlineHelpSuggested",
			enabled ? onVcpkgOnlineHelpSuggested : onDisabledCommand,
		),
	);
	commandDisposables.push(
		vscode.commands.registerCommand(
			"C_Cpp.GenerateEditorConfig",
			enabled ? onGenerateEditorConfig : onDisabledCommand,
		),
	);
	commandDisposables.push(
		vscode.commands.registerCommand(
			"C_Cpp.GoToNextDirectiveInGroup",
			enabled ? onGoToNextDirectiveInGroup : onDisabledCommand,
		),
	);
	commandDisposables.push(
		vscode.commands.registerCommand(
			"C_Cpp.GoToPrevDirectiveInGroup",
			enabled ? onGoToPrevDirectiveInGroup : onDisabledCommand,
		),
	);
	commandDisposables.push(
		vscode.commands.registerCommand(
			"C_Cpp.RunCodeAnalysisOnActiveFile",
			enabled ? onRunCodeAnalysisOnActiveFile : onDisabledCommand,
		),
	);
	commandDisposables.push(
		vscode.commands.registerCommand(
			"C_Cpp.RunCodeAnalysisOnOpenFiles",
			enabled ? onRunCodeAnalysisOnOpenFiles : onDisabledCommand,
		),
	);
	commandDisposables.push(
		vscode.commands.registerCommand(
			"C_Cpp.RunCodeAnalysisOnAllFiles",
			enabled ? onRunCodeAnalysisOnAllFiles : onDisabledCommand,
		),
	);
	commandDisposables.push(
		vscode.commands.registerCommand(
			"C_Cpp.RemoveCodeAnalysisProblems",
			enabled ? onRemoveCodeAnalysisProblems : onDisabledCommand,
		),
	);
	commandDisposables.push(
		vscode.commands.registerCommand(
			"C_Cpp.RemoveAllCodeAnalysisProblems",
			enabled ? onRemoveAllCodeAnalysisProblems : onDisabledCommand,
		),
	);
	commandDisposables.push(
		vscode.commands.registerCommand(
			"C_Cpp.FixThisCodeAnalysisProblem",
			enabled ? onFixThisCodeAnalysisProblem : onDisabledCommand,
		),
	);
	commandDisposables.push(
		vscode.commands.registerCommand(
			"C_Cpp.FixAllTypeCodeAnalysisProblems",
			enabled ? onFixAllTypeCodeAnalysisProblems : onDisabledCommand,
		),
	);
	commandDisposables.push(
		vscode.commands.registerCommand(
			"C_Cpp.FixAllCodeAnalysisProblems",
			enabled ? onFixAllCodeAnalysisProblems : onDisabledCommand,
		),
	);
	commandDisposables.push(
		vscode.commands.registerCommand(
			"C_Cpp.DisableAllTypeCodeAnalysisProblems",
			enabled ? onDisableAllTypeCodeAnalysisProblems : onDisabledCommand,
		),
	);
	commandDisposables.push(
		vscode.commands.registerCommand(
			"C_Cpp.ShowCodeAnalysisDocumentation",
			enabled ? (uri) => vscode.env.openExternal(uri) : onDisabledCommand,
		),
	);
	commandDisposables.push(
		vscode.commands.registerCommand(
			"cpptools.activeConfigName",
			enabled ? onGetActiveConfigName : onDisabledCommand,
		),
	);
	commandDisposables.push(
		vscode.commands.registerCommand(
			"cpptools.activeConfigCustomVariable",
			enabled ? onGetActiveConfigCustomVariable : onDisabledCommand,
		),
	);
	commandDisposables.push(
		vscode.commands.registerCommand(
			"cpptools.setActiveConfigName",
			enabled ? onSetActiveConfigName : onDisabledCommand,
		),
	);
	commandDisposables.push(
		vscode.commands.registerCommand(
			"C_Cpp.RestartIntelliSenseForFile",
			enabled ? onRestartIntelliSenseForFile : onDisabledCommand,
		),
	);
	commandDisposables.push(
		vscode.commands.registerCommand(
			"C_Cpp.GenerateDoxygenComment",
			enabled ? onGenerateDoxygenComment : onDisabledCommand,
		),
	);
	commandDisposables.push(
		vscode.commands.registerCommand(
			"C_Cpp.CreateDeclarationOrDefinition",
			enabled ? onCreateDeclarationOrDefinition : onDisabledCommand,
		),
	);
	commandDisposables.push(
		vscode.commands.registerCommand(
			"C_Cpp.CopyDeclarationOrDefinition",
			enabled ? onCopyDeclarationOrDefinition : onDisabledCommand,
		),
	);
	commandDisposables.push(
		vscode.commands.registerCommand(
			"C_Cpp.RescanCompilers",
			enabled ? onRescanCompilers : onDisabledCommand,
		),
	);
	commandDisposables.push(
		vscode.commands.registerCommand(
			"C_Cpp.AddMissingInclude",
			enabled ? onAddMissingInclude : onDisabledCommand,
		),
	);
	commandDisposables.push(
		vscode.commands.registerCommand(
			"C_Cpp.ExtractToFunction",
			enabled
				? () => onExtractToFunction(false, false)
				: onDisabledCommand,
		),
	);
	commandDisposables.push(
		vscode.commands.registerCommand(
			"C_Cpp.ExtractToFreeFunction",
			enabled
				? () => onExtractToFunction(true, false)
				: onDisabledCommand,
		),
	);
	commandDisposables.push(
		vscode.commands.registerCommand(
			"C_Cpp.ExtractToMemberFunction",
			enabled
				? () => onExtractToFunction(false, true)
				: onDisabledCommand,
		),
	);
	commandDisposables.push(
		vscode.commands.registerCommand(
			"C_Cpp.ExpandSelection",
			enabled ? (r: Range) => onExpandSelection(r) : onDisabledCommand,
		),
	);
	commandDisposables.push(
		vscode.commands.registerCommand(
			"C_Cpp.ShowCopilotHover",
			enabled ? () => onCopilotHover() : onDisabledCommand,
		),
	);
}
...
async function onRestartIntelliSenseForFile() {
	...
	return clients.ActiveClient.restartIntelliSenseForFile(
...
async function onSwitchHeaderSource(): Promise<void> {
	...
	let targetFileName: string =
		await clients.ActiveClient.requestSwitchHeaderSource(rootUri, fileName);
	...
	clients.forEach((client) => {
		if (
			!targetFileNameReplaced &&
			client.RootRealPath &&
			client.RootPath !== client.RootRealPath &&
			targetFileName.startsWith(client.RootRealPath)
		) {
			targetFileName =
				client.RootPath +
				targetFileName.substring(client.RootRealPath.length);
			targetFileNameReplaced = true;
		}
	});
	const document: vscode.TextDocument =
		await vscode.workspace.openTextDocument(targetFileName);
	const workbenchConfig: vscode.WorkspaceConfiguration =
	...
	if (workbenchConfig.get("editor.revealIfOpen")) {
		...
		vscode.window.visibleTextEditors.forEach((editor) => {
			if (editor.document === document && !foundEditor) {
				foundEditor = true;
				void vscode.window
					.showTextDocument(document, editor.viewColumn)
...
function onSelectConfigurationProvider(): void {
	if (!isFolderOpen()) {
		...
		void selectClient().then(
			(client) => client.handleConfigurationProviderSelectCommand(),
			logAndReturn.undefined,
		);
	}
}
...
function onEditConfigurationJSON(
	...
	if (!isFolderOpen()) {
		...
		void selectClient().then(
			(client) => client.handleConfigurationEditJSONCommand(viewColumn),
			logAndReturn.undefined,
		);
	}
}
...
function onEditConfigurationUI(
	...
	if (!isFolderOpen()) {
		...
		void selectClient().then(
			(client) => client.handleConfigurationEditUICommand(viewColumn),
			logAndReturn.undefined,
		);
	}
}
...
function onEditConfiguration(
	...
	if (!isFolderOpen()) {
		...
		void selectClient().then(
			(client) => client.handleConfigurationEditCommand(viewColumn),
			logAndReturn.undefined,
		);
	}
}
...
async function onRemoveCodeAnalysisProblems(
	refreshSquigglesOnSave: boolean,
	identifiersAndUris: CodeAnalysisDiagnosticIdentifiersAndUri[],
): Promise<void> {
	return getActiveClient().handleRemoveCodeAnalysisProblems(
...
const codeActionAbortedString: string = localize(
	"code.action.aborted",
	"The code analysis fix could not be applied because the document has changed.",
);

async function onFixThisCodeAnalysisProblem(
	version: number,
	...
	const codeActions: CodeActionDiagnosticInfo[] | undefined =
		codeAnalysisFileToCodeActions.get(identifiersAndUris[0].uri);
	if (codeActions === undefined) {
		return;
	}
	for (const codeAction of codeActions) {
		if (
			codeAction.code === identifiersAndUris[0].identifiers[0].code &&
			rangeEquals(
				codeAction.range,
				identifiersAndUris[0].identifiers[0].range,
			)
		) {
	...
	return getActiveClient().handleFixCodeAnalysisProblems(
		workspaceEdit,
		refreshSquigglesOnSave,
		identifiersAndUris,
	);
...
async function onFixAllTypeCodeAnalysisProblems(
	type: string,
	version: number,
	workspaceEdit: vscode.WorkspaceEdit,
	refreshSquigglesOnSave: boolean,
...
async function onFixAllCodeAnalysisProblems(
	version: number,
	workspaceEdit: vscode.WorkspaceEdit,
	refreshSquigglesOnSave: boolean,
	identifiersAndUris: CodeAnalysisDiagnosticIdentifiersAndUri[],
...
async function onDisableAllTypeCodeAnalysisProblems(
	code: string,
	identifiersAndUris: CodeAnalysisDiagnosticIdentifiersAndUri[],
): Promise<void> {
	return getActiveClient().handleDisableAllTypeCodeAnalysisProblems(
		code,
		identifiersAndUris,
	);
}
...
async function onCopyDeclarationOrDefinition(args?: any): Promise<void> {
	const sender: any | undefined = util.isString(args?.sender)
		? args.sender
		: args;
...
async function onExtractToFunction(
	extractAsGlobal: boolean,
	extractAsMemberFunction: boolean,
): Promise<void> {
	if (extractAsGlobal) {
...
function onTakeSurvey(): void {
	...
	const uri: vscode.Uri = vscode.Uri.parse(
		`https://www.research.net/r/VBVV6C6?o=${os.platform()}&m=${vscode.env.machineId}`,
	);
	void vscode.commands.executeCommand("vscode.open", uri);
}

function onVcpkgOnlineHelpSuggested(dummy?: any): void {
	telemetry.logLanguageServerEvent("vcpkgAction", {
		"source": dummy ? "CodeAction" : "CommandPalette",
		"action": "vcpkgOnlineHelpSuggested",
	});
	const uri: vscode.Uri = vscode.Uri.parse(`https://aka.ms/vcpkg`);
	void vscode.commands.executeCommand("vscode.open", uri);
}

async function onVcpkgClipboardInstallSuggested(
	ports?: string[],
): Promise<void> {
	let source: string;
	if (ports && ports.length) {
		...
		vscode.languages.getDiagnostics().forEach((uriAndDiagnostics) => {
			...
			const textDocument: vscode.TextDocument | undefined =
				vscode.workspace.textDocuments.find(
					(doc) => doc.uri.fsPath === uriAndDiagnostics[0].fsPath,
				);
			if (!textDocument) {
				return;
			}
			...
			let lines: number[] = uriAndDiagnostics[1]
				.filter(isMissingIncludeDiagnostic)
				.map<number>((d) => d.range.start.line);
			if (!lines.length) {
				return;
			}
		...
		missingIncludeLocations.forEach((docAndLineNumbers) => {
			docAndLineNumbers[1].forEach((line) => {
				portsPromises.push(
					lookupIncludeInVcpkg(docAndLineNumbers[0], line),
				);
			});
		});
		...
		ports = ports2.filter(
			(port: string, index: number) => ports2.indexOf(port) === index,
		);
	}
	...
	telemetry.logLanguageServerEvent("vcpkgAction", {
		"source": source,
		"action": "vcpkgClipboardInstallSuggested",
...
function onGenerateDoxygenComment(
	arg: DoxygenCodeActionCommandArguments,
): Promise<void> {
...
function onGetActiveConfigCustomVariable(
	variableName: string,
): Thenable<string> {
...
function onShowRefCommand(arg?: TreeNode): void {
	...
	if (node === NodeType.reference) {
		...
		if (referenceLocation) {
			void vscode.window
				.showTextDocument(referenceLocation.uri, {
					selection: referenceLocation.range.with({
						start: referenceLocation.range.start,
						end: referenceLocation.range.end,
					}),
				})
				.then(undefined, logAndReturn.undefined);
		}
...
function reportMacCrashes(): void {
	if (process.platform === "darwin") {
		...
		const crashFolder: string = path.resolve(
			home,
		...
		fs.stat(crashFolder, (err) => {
			...
			try {
				fs.watch(crashFolder, (event, filename) => {
					...
					setTimeout(() => {
						fs.readFile(
							path.resolve(crashFolder, filename),
							"utf8",
							(err, data) => {
								if (err) {
									// Try again?
		...
		fs.stat(crashDirectory, (err) => {
			...
			try {
				fs.watch(crashDirectory, (event, filename) => {
					...
					setTimeout(() => {
						fs.readFile(
							path.resolve(crashDirectory, filename),
							"utf8",
							(err, data) => {
								void handleCrashFileRead(
									crashDirectory,
									filename,
									crashDate,
									err,
...
function logCrashTelemetry(
	...
	previousCrashCount =
		data === previousCrashData ? previousCrashCount + 1 : 0;
...
function handleMacCrashFileRead(
	...
	if (startVersion >= 0) {
		...
		const binaryVersionMatches: string[] | null = data.match(
			/^Version:\s*(\d*\.\d*\.\d*\.\d*|\d)/,
		);
		binaryVersion =
			binaryVersionMatches && binaryVersionMatches.length > 1
				? binaryVersionMatches[1]
				: "";
	}
	...
	if (startDynamicLoadError >= 0) {
		...
		const endDynamicLoadError: number = data.indexOf(
			dynamicLoadErrorEnd,
			startDynamicLoadError,
		);
		if (endDynamicLoadError >= 0) {
			dynamicLoadError = data.substring(
				startDynamicLoadError,
				endDynamicLoadError,
			);
...
async function handleCrashFileRead(
	...
	const telemetryHeader: string =
		(isCppToolsSrv ? "cpptools-srv.txt" : crashFile) + "\n";
	...
	for (let lineNum: number = 0; lineNum < lines.length - 3; ++lineNum) {
		// skip last lines
		...
		if (startPos === -1 || line[startPos + (isMac ? 1 : 4)] === "+") {
			...
			const endAddressPos: number = line.indexOf(
				endOffsetStr,
				startAddressPos + 2,
			);
			addressData += "\n";
		...
		const offsetPos: number = line.indexOf(
			offsetStr,
		...
		if (filtPath && filtPath.length !== 0) {
			let ret: util.ProcessReturnType | undefined = await util
				.spawnChildProcess(
					filtPath,
			...
			if (ret?.output === funcStr) {
				ret = await util
					.spawnChildProcess(filtPath, [funcStr], undefined, true)
					.catch(logAndReturn.undefined);
			}
			if (ret !== undefined && ret.succeeded) {
				...
				funcStr = funcStr.replace(
					/, std::(?:allocator|char_traits)<char>/g,
					"",
				);
		...
		if (funcStr.includes("/")) {
			...
			!validFrameFound &&
			(funcStr.startsWith("crash_handler(") ||
				funcStr.startsWith("_sigtramp"))
		) {
	...
	if (crashCallStack !== prevCppCrashCallStackData) {
		...
		const settings: vscode.WorkspaceConfiguration =
			vscode.workspace.getConfiguration("C_Cpp", null);
		if (
			lines.length >= 6 &&
			util.getNumericLoggingLevel(settings.get<string>("loggingLevel")) >=
				1
		) {
	...
	await util
		.deleteFile(path.resolve(crashDirectory, crashFile))
		.catch(logAndReturn.undefined);
...
export function UpdateInsidersAccess(): void {
	...
	const settings: CppSettings = new CppSettings();
	const migratedInsiders: PersistentState<boolean> =
		new PersistentState<boolean>("CPP.migratedInsiders", false);
	if (settings.updateChannel === "Insiders") {
		...
		if (
			!migratedInsiders.Value &&
			vscode.workspace
				.getConfiguration("extensions", null)
				.get<boolean>("autoUpdate")
		) {
	...
	if (util.isVsCodeInsiders()) {
		const insidersMitigationDone: PersistentState<boolean> =
			new PersistentState<boolean>("CPP.insidersMitigationDone", false);
		if (!insidersMitigationDone.Value) {
			if (
				...
				if (
					settings.getStringWithUndefinedDefault("updateChannel") ===
					undefined
				) {
					installPrerelease = true;
				}
...
export async function preReleaseCheck(): Promise<void> {
	const displayedPreReleasePrompt: PersistentState<boolean> =
		new PersistentState<boolean>("CPP.displayedPreReleasePrompt", false);
	const isOnPreRelease: PersistentState<boolean> =
		new PersistentState<boolean>("CPP.isOnPreRelease", false);
	...
	if (
		!isOnPreRelease.Value &&
		!displayedPreReleasePrompt.Value &&
		util.getCppToolsTargetPopulation() === TargetPopulation.Public
	) {
		...
		telemetry.logLanguageServerEvent("marketplaceFetch", undefined, {
		...
		const preReleaseAvailable =
			data?.results[0].extensions[0].versions[0].properties.some(
				(e: object) =>
...
async function onCopilotHover(): Promise<void> {
	...
	const copilotHoverProvider = clients
		.getDefaultClient()
	...
	await vscode.window.showTextDocument(hoverDocument, {
		preserveFocus: true,
		selection: new vscode.Selection(hoverPosition, hoverPosition),
	});

	if (
		!(await showCopilotContent(
			copilotHoverProvider,
			hoverDocument,
			hoverPosition,
		))
	) {
	...
	const requestInfo = await copilotHoverProvider.getRequestInfo(
		hoverDocument,
		hoverPosition,
	);
	if (requestInfo.length === 0) {
		...
		telemetry.logLanguageServerEvent("CopilotHover", {
			"Message": "Copilot summary is not available for this symbol.",
		});
		await showCopilotContent(
			copilotHoverProvider,
			hoverDocument,
			hoverPosition,
			localize(
				"copilot.hover.unavailable",
				"Copilot summary is not available for this symbol.",
			),
		);
		return;
	}
	...
	const messages = [
		vscode.LanguageModelChatMessage.User(requestInfo + locale),
	];
	...
	if (!chatResponse) {
		await reportCopilotFailure(
			...
			hoverDocument,
			hoverPosition,
			"Invalid chat response from Copilot.",
		);
		return;
	}
	...
	if (content.length === 0) {
		await reportCopilotFailure(
			...
			hoverDocument,
			hoverPosition,
			"No content in response from Copilot.",
		);
		return;
	}
...
async function reportCopilotFailure(
	copilotHoverProvider: CopilotHoverProvider,
	hoverDocument: vscode.TextDocument,
	hoverPosition: vscode.Position,
	errorMessage: string,
): Promise<void> {
	telemetry.logLanguageServerEvent("CopilotHoverError", {
	...
	await showCopilotContent(
		...
		localize(
			"copilot.hover.error",
			"An error occurred while generating Copilot summary.",
		),
	);
}

async function showCopilotContent(
	copilotHoverProvider: CopilotHoverProvider,
	hoverDocument: vscode.TextDocument,
	hoverPosition: vscode.Position,
	content?: string,
