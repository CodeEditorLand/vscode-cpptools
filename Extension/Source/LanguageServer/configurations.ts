...
import {
	CustomConfigurationProviderCollection,
...
export class CppProperties {
	...
	private currentConfigurationIndex:
		| PersistentFolderState<number>
		| undefined;
	...
	private compileCommandsFileWatcherFallbackTime: Map<string, Date> = new Map<
		string,
		Date
	>(); // Used when file watching fails.
	...
	private prevSquiggleMetrics: Map<string, { [key: string]: number }> =
	...
	constructor(
		...
		if (workspaceFolder) {
			this.currentConfigurationIndex = new PersistentFolderState<number>(
				"CppProperties.currentConfigurationIndex",
				-1,
				workspaceFolder,
			);
		}
		...
		this.diagnosticCollection =
			vscode.languages.createDiagnosticCollection(rootPath);
		...
		this.disposables.push(
			vscode.Disposable.from(
				...
				this.compileCommandsChanged,
			),
		);
	}

	public get ConfigurationsChanged(): vscode.Event<CppProperties> {
		return this.configurationsChanged.event;
	}
	public get SelectionChanged(): vscode.Event<number> {
		return this.selectionChanged.event;
	}
	public get CompileCommandsChanged(): vscode.Event<string> {
		return this.compileCommandsChanged.event;
	}
	public get Configurations(): Configuration[] | undefined {
		return this.configurationJson
			? this.configurationJson.configurations
			: undefined;
	}
	public get CurrentConfigurationIndex(): number {
		return this.currentConfigurationIndex === undefined
			? 0
			: this.currentConfigurationIndex.Value;
	}
	public get CurrentConfiguration(): Configuration | undefined {
		return this.Configurations
			? this.Configurations[this.CurrentConfigurationIndex]
			: undefined;
	}
	public get KnownCompiler(): KnownCompiler[] | undefined {
		return this.knownCompilers;
	}
	...
	public get ConfigurationNames(): string[] | undefined {
		...
		if (this.configurationJson) {
			this.configurationJson.configurations.forEach(
	...
	public setupConfigurations(): void {
		...
		vscode.workspace.onDidChangeTextDocument((e) => {
			if (
				e.document.uri.fsPath === settingsPath &&
				this.isCppPropertiesJsonVisible
			) {
		...
		vscode.window.onDidChangeVisibleTextEditors((editors) => {
			...
			editors.forEach((editor) => {
				if (editor.document.uri.fsPath === settingsPath) {
					...
					if (!wasVisible) {
						void this.handleSquiggles().catch(
		...
		vscode.workspace.onDidSaveTextDocument((doc: vscode.TextDocument) => {
			...
			const savedDocWorkspaceFolder: vscode.WorkspaceFolder | undefined =
				vscode.workspace.getWorkspaceFolder(doc.uri);

			const notifyingWorkspaceFolder: vscode.WorkspaceFolder | undefined =
				vscode.workspace.getWorkspaceFolder(
					vscode.Uri.file(settingsPath),
				);

			if (
				(!savedDocWorkspaceFolder &&
					vscode.workspace.workspaceFolders &&
					vscode.workspace.workspaceFolders.length > 0 &&
					notifyingWorkspaceFolder ===
				...
				if (fileType) {
					...
					telemetry.logLanguageServerEvent("cmakeFileWrite", {
						filetype: fileType,
						outside: (
							savedDocWorkspaceFolder === undefined
						).toString(),
					});
				}
			}
	...
	public set CompilerDefaults(compilerDefaults: CompilerDefaults) {
		this.defaultCompilerPath = compilerDefaults.trustedCompilerFound
			? compilerDefaults.compilerPath
			: null;
		...
		this.defaultIntelliSenseMode =
			compilerDefaults.intelliSenseMode !== ""
				? compilerDefaults.intelliSenseMode
				: undefined;
	...
	private resetToDefaultSettings(resetIndex: boolean): void {
		...
		if (
			...
			this.CurrentConfigurationIndex >=
				this.configurationJson.configurations.length
		) {
			const index: number | undefined = this.getConfigIndexForPlatform(
	...
	private applyDefaultConfigurationValues(
		...
		const isUnset: (input: any) => boolean = (input: any) =>
		...
		if (
			isUnset(settings.defaultMacFrameworkPath) &&
			process.platform === "darwin"
		) {
			configuration.macFrameworkPath = this.defaultFrameworks;
		}
		if (
			(isUnset(settings.defaultWindowsSdkVersion) ||
				settings.defaultWindowsSdkVersion === "") &&
			this.defaultWindowsSdkVersion &&
			process.platform === "win32"
		) {
			configuration.windowsSdkVersion = this.defaultWindowsSdkVersion;
		}
		if (
			isUnset(settings.defaultCompilerPath) &&
			this.defaultCompilerPath &&
			(isUnset(settings.defaultCompileCommands) ||
			...
			const compiler: string = path
				.basename(this.defaultCompilerPath)
				.toLowerCase();
		...
		if (
			(isUnset(settings.defaultCStandard) ||
				settings.defaultCStandard === "") &&
			this.defaultCStandard
		) {
			configuration.cStandard = this.defaultCStandard;
		}
		if (
			(isUnset(settings.defaultCppStandard) ||
				settings.defaultCppStandard === "") &&
			this.defaultCppStandard
		) {
			configuration.cppStandard = this.defaultCppStandard;
		}
		if (
			isUnset(settings.defaultIntelliSenseMode) ||
			settings.defaultIntelliSenseMode === ""
		) {
			configuration.intelliSenseMode = this.defaultIntelliSenseMode;
		}
		if (
			!settings.defaultCustomConfigurationVariables ||
			Object.keys(settings.defaultCustomConfigurationVariables).length ===
				0
		) {
			configuration.customConfigurationVariables =
				this.defaultCustomConfigurationVariables;
		}
	}
	...
	private get ExtendedEnvironment(): Environment {
		...
		result["workspaceFolderBasename"] = this.rootUri
			? path.basename(this.rootUri.fsPath)
			: "";
		...
		try {
			...
			if (vcpkgRoot) {
				...
				if (list !== undefined) {
					...
					list.forEach((entry) => {
						if (entry !== "vcpkg") {
							const pathToCheck: string = path.join(
								vcpkgRoot,
								entry,
							);
	...
	private async readNodeAddonIncludeLocations(
		rootPath: string,
	): Promise<void> {
		...
		if (!error) {
			try {
				...
				const nodeAddonMap: [string, string][] = [
					[
						"node-addon-api",
						`"${pathToNode}" --no-warnings -p "require('node-addon-api').include"`,
					],
					[
						"nan",
						`"${pathToNode}" --no-warnings -e "require('nan')"`,
					],
				];
				// Yarn (2) PnP support
				const pathToYarn: string | null = which.sync("yarn", {
					nothrow: true,
				});

				if (
					pathToYarn &&
					(await util.checkDirectoryExists(
						path.join(rootPath, ".yarn/cache"),
					))
				) {
					nodeAddonMap.push(
						[
							"node-addon-api",
							`"${pathToYarn}" node --no-warnings -p "require('node-addon-api').include"`,
						],
						[
							"nan",
							`"${pathToYarn}" node --no-warnings -e "require('nan')"`,
						],
					);
				}
				...
				for (const [dep, execCmd] of nodeAddonMap) {
					if (dep in packageJson.dependencies) {
						try {
							let stdout: string = await util.execChildProcess(
								execCmd,
								rootPath,
							);
							...
							if (!(await util.checkDirectoryExists(stdout))) {
								...
								if (
									!(await util.checkDirectoryExists(stdout))
								) {
									error = new Error(
										`${dep} directory ${stdout} doesn't exist`,
									);
									stdout = "";
								}
							}
	...
	private getConfigIndexForPlatform(config: any): number | undefined {
		...
		for (
			let i: number = 0;
			i < this.configurationJson.configurations.length;
			i++
		) {
	...
	private validateIntelliSenseMode(configuration: Configuration): string {
		...
		const resolvedCompilerPath: string = this.resolvePath(
			configuration.compilerPath,
			false,
			false,
		);
		...
		const isValid: boolean =
			...
			compilerPathAndArgs.compilerName.toLowerCase() === "nvcc.exe" ||
			compilerPathAndArgs.compilerName.toLowerCase() === "nvcc";
		...
		void this.handleConfigurationEditCommand(
			() => {
				...
				if (config) {
					...
					config.includePath.splice(
						config.includePath.length,
						0,
						path,
					);
					this.writeToJson();
	...
	public async updateCustomConfigurationProvider(
		providerId: string,
	): Promise<void> {
		if (!this.propertiesFile) {
			const settings: CppSettings = new CppSettings(this.rootUri);
	...
	public select(index: number): Configuration | undefined {
		if (this.configurationJson) {
			if (index === this.configurationJson.configurations.length) {
				void this.handleConfigurationEditUICommand(
					() => {},
			...
			if (index === this.configurationJson.configurations.length + 1) {
				void this.handleConfigurationEditJSONCommand(
					() => {},
					vscode.window.showTextDocument,
				).catch(logAndReturn.undefined);
	...
	private resolveDefaultsDictionary(
		...
		for (const property in entries) {
			if (property === "${default}") {
				if (defaultValue) {
					for (const defaultProperty in defaultValue) {
						if (!(defaultProperty in entries)) {
							result[defaultProperty] = util.resolveVariables(
				...
				result[property] = util.resolveVariables(
					entries[property],
					env,
				);
			}
	...
	private resolve(
		...
		if (entries) {
			...
			entries.forEach((entry) => {
				...
				const entryResolved: string = util.resolveVariables(
					entry,
					env,
					entriesResolved,
				);
				result = result.concat(
					entriesResolved.length === 0
						? entryResolved
						: entriesResolved,
				);
			});
	...
	private resolveAndSplit(
		...
		paths.forEach((entry) => {
			...
			if (resolvedVariable.includes("env:")) {
				...
				const entries: string[] = resolvedVariable
					.split(path.delimiter)
					.map((e) =>
						glob ? this.resolvePath(e, false, assumeRelative) : e,
					)
					.filter((e) => e);
	...
	private updateConfigurationStringArray(
	...
	private updateConfigurationPathsArray(
		paths: string[] | undefined,
		defaultValue: string[] | undefined,
		env: Environment,
		assumeRelative: boolean = true,
	...
	private updateConfigurationStringOrBoolean(
	...
	private updateConfigurationBoolean(
	...
	private updateConfigurationStringDictionary(
		...
		if (dotConfigPath !== undefined) {
			...
			try {
				...
				getOutputChannelLogger().appendLine(
					`Invalid input, cannot resolve .config path: ${err.message}`,
				);
			}
		}
	...
	private updateServerOnFolderSettingsChange(): void {
		...
		for (
			...
			const configuration: Configuration =
				this.configurationJson.configurations[i];
			configuration.compilerPathInCppPropertiesJson =
				configuration.compilerPath;
			configuration.compileCommandsInCppPropertiesJson =
				configuration.compileCommands;
			configuration.configurationProviderInCppPropertiesJson =
				configuration.configurationProvider;
			configuration.includePath = this.updateConfigurationPathsArray(
				configuration.includePath,
				settings.defaultIncludePath,
				env,
			);
			// in case includePath is reset below
			const origIncludePath: string[] | undefined =
				configuration.includePath;

			if (userSettings.addNodeAddonIncludePaths) {
				const includePath: string[] = origIncludePath || [];
				configuration.includePath = includePath.concat(
					this.nodeAddonIncludes.filter(
						(i) => includePath.indexOf(i) < 0,
					),
				);
			}
			configuration.defines = this.updateConfigurationStringArray(
				configuration.defines,
			...
			configuration.dotConfig = this.updateConfigurationString(
				configuration.dotConfig,
				settings.defaultDotconfig,
				env,
			);

			if (configuration.dotConfig !== undefined) {
				configuration.defines = configuration.defines || [];
				configuration.defines = configuration.defines.concat(
					this.getDotconfigDefines(configuration.dotConfig),
				);
			}

			configuration.macFrameworkPath =
				this.updateConfigurationStringArray(
					configuration.macFrameworkPath,
					settings.defaultMacFrameworkPath,
					env,
				);
			configuration.windowsSdkVersion = this.updateConfigurationString(
				configuration.windowsSdkVersion,
				settings.defaultWindowsSdkVersion,
				env,
			);
			configuration.forcedInclude = this.updateConfigurationPathsArray(
				configuration.forcedInclude,
				settings.defaultForcedInclude,
				env,
				false,
			);
			configuration.compileCommands = this.updateConfigurationString(
				configuration.compileCommands,
				settings.defaultCompileCommands,
				env,
			);
			configuration.compilerArgs = this.updateConfigurationStringArray(
				configuration.compilerArgs,
				settings.defaultCompilerArgs,
				env,
			);
			configuration.cStandard = this.updateConfigurationString(
				configuration.cStandard,
				settings.defaultCStandard,
				env,
			);
			configuration.cppStandard = this.updateConfigurationString(
				configuration.cppStandard,
				settings.defaultCppStandard,
				env,
			);
			configuration.intelliSenseMode = this.updateConfigurationString(
				configuration.intelliSenseMode,
				settings.defaultIntelliSenseMode,
				env,
			);
			configuration.intelliSenseModeIsExplicit =
				configuration.intelliSenseModeIsExplicit ||
				settings.defaultIntelliSenseMode !== "";
			configuration.cStandardIsExplicit =
				configuration.cStandardIsExplicit ||
				settings.defaultCStandard !== "";
			configuration.cppStandardIsExplicit =
				configuration.cppStandardIsExplicit ||
				settings.defaultCppStandard !== "";
			configuration.mergeConfigurations = this.updateConfigurationBoolean(
				configuration.mergeConfigurations,
				settings.defaultMergeConfigurations,
			);

			if (!configuration.compileCommands) {
				// compile_commands.json already specifies a compiler. compilerPath overrides the compile_commands.json compiler so
				// don't set a default when compileCommands is in use.
				configuration.compilerPath = this.updateConfigurationString(
					configuration.compilerPath,
					settings.defaultCompilerPath,
					env,
					true,
				);
				configuration.compilerPathIsExplicit =
					configuration.compilerPathIsExplicit ||
				...
				if (configuration.compilerPath === null) {
					...
					configuration.compilerPath = util.resolveVariables(
						configuration.compilerPath,
						env,
					);
					configuration.compilerPathIsExplicit = true;
			...
			configuration.customConfigurationVariables =
				this.updateConfigurationStringDictionary(
					configuration.customConfigurationVariables,
					settings.defaultCustomConfigurationVariables,
					env,
				);
			...
			if (!configuration.browse.path) {
				if (settings.defaultBrowsePath) {
					...
					configuration.browse.path =
						configuration.includePath.slice(0);

					if (
						configuration.includePath.findIndex(
							(value: string) =>
								!!value.match(
									/^\$\{(workspaceRoot|workspaceFolder)\}(\\\*{0,2}|\/\*{0,2})?$/g,
								),
						) === -1
					) {
				...
				configuration.browse.path = this.updateConfigurationPathsArray(
					configuration.browse.path,
			...
			configuration.browse.limitSymbolsToIncludedHeaders =
				this.updateConfigurationStringOrBoolean(
					configuration.browse.limitSymbolsToIncludedHeaders,
					settings.defaultLimitSymbolsToIncludedHeaders,
					env,
				);
			configuration.browse.databaseFilename =
				this.updateConfigurationString(
					configuration.browse.databaseFilename,
					settings.defaultDatabaseFilename,
					env,
				);
			...
			if (i === this.CurrentConfigurationIndex) {
				...
				const providers: CustomConfigurationProviderCollection =
					getCustomConfigProviders();

				const hasEmptyConfiguration: boolean =
					!this.propertiesFile &&
					!settings.defaultIncludePath &&
					!settings.defaultDefines &&
					!settings.defaultMacFrameworkPath &&
				...
				if (hasEmptyConfiguration) {
					...
					this.client.lastCustomBrowseConfigurationProviderId !==
					undefined
				) {
					keepCachedBrowseConfig =
						configuration.configurationProvider ===
				...
				if (
					!keepCachedBrowseConfig &&
					this.client.lastCustomBrowseConfiguration !== undefined
				) {
			...
			if (configuration.macFrameworkPath) {
				configuration.macFrameworkPath =
					configuration.macFrameworkPath.map((path: string) =>
						this.resolvePath(path),
					);
			}

			if (configuration.dotConfig) {
				configuration.dotConfig = this.resolvePath(
					configuration.dotConfig,
				);
			}

			if (configuration.compileCommands) {
				configuration.compileCommands = this.resolvePath(
					configuration.compileCommands,
				);

				if (
					!this.compileCommandsFileWatcherFallbackTime.has(
						configuration.compileCommands,
					)
				) {
					// Start tracking the fallback time for a new path.
					this.compileCommandsFileWatcherFallbackTime.set(
						configuration.compileCommands,
						new Date(),
					);
				}
			...
			if (configuration.forcedInclude) {
				configuration.forcedInclude = configuration.forcedInclude.map(
					(path: string) => this.resolvePath(path, true, false),
				);
			}

			if (configuration.includePath) {
				configuration.includePath = configuration.includePath.map(
					(path: string) => this.resolvePath(path, false),
				);
			}
		}
	...
	public updateCompileCommandsFileWatchers(): void {
		if (this.configurationJson) {
			this.compileCommandsFileWatchers.forEach((watcher: fs.FSWatcher) =>
				watcher.close(),
			);
			...
			this.configurationJson.configurations.forEach((c) => {
				if (c.compileCommands) {
					const fileSystemCompileCommandsPath: string =
						this.resolvePath(c.compileCommands);
							...
							this.compileCommandsFileWatcherTimer = setTimeout(
								() => {
									...
									if (this.compileCommandsFileWatcherTimer) {
										clearInterval(
											this
												.compileCommandsFileWatcherTimer,
										);
									}
	...
	public async handleConfigurationEditCommand(
		...
		if (otherSettings.workbenchSettingsEditor === "ui") {
			await this.handleConfigurationEditUICommand(
				onBeforeOpen,
				showDocument,
				viewColumn,
			);
		} else {
			await this.handleConfigurationEditJSONCommand(
				onBeforeOpen,
				showDocument,
				viewColumn,
			);
	...
	public async handleConfigurationEditJSONCommand(
		...
		if (this.propertiesFile) {
			const document: vscode.TextDocument =
				await vscode.workspace.openTextDocument(this.propertiesFile);
	...
	private ensureSettingsPanelInitlialized(): void {
		if (this.settingsPanel === undefined) {
			...
			this.settingsPanel.setKnownCompilers(
				this.knownCompilers,
				settings.preferredPathSeparator,
			);
			this.settingsPanel.SettingsPanelActivated(() => {
				if (this.settingsPanel?.initialized) {
					void this.onSettingsPanelActivated().catch(
						logAndReturn.undefined,
					);
				}
			});
			this.settingsPanel.ConfigValuesChanged(() =>
				this.saveConfigurationUI(),
			);
			this.settingsPanel.ConfigSelectionChanged(() =>
				this.onConfigSelectionChanged(),
			);
			this.settingsPanel.AddConfigRequested((e) =>
				this.onAddConfigRequested(e),
			);
			...
			if (this.parsePropertiesFile()) {
				...
				if (this.settingsPanel) {
					...
					if (configNames && this.configurationJson) {
						...
						this.settingsPanel.selectedConfigIndex =
							this.CurrentConfigurationIndex;
				...
				if (this.parsePropertiesFile()) {
					...
					if (
						...
						void vscode.workspace
							.openTextDocument(this.propertiesFile)
	...
	private saveConfigurationUI(): void {
		...
		if (this.settingsPanel && this.configurationJson) {
			const config: Configuration =
				this.settingsPanel.getLastValuesFromConfigUI();
			this.configurationJson.configurations[
				this.settingsPanel.selectedConfigIndex
			] = config;
			this.configurationJson.configurations[
				this.settingsPanel.selectedConfigIndex
			].includePath = this.trimPathWhitespace(
				this.configurationJson.configurations[
					this.settingsPanel.selectedConfigIndex
				].includePath,
			);
			this.settingsPanel.updateErrors(
				this.getErrorsForConfigUI(
					this.settingsPanel.selectedConfigIndex,
				),
			);
	...
	private onAddConfigRequested(configName: string): void {
		...
		if (configNames && this.settingsPanel && this.configurationJson) {
			...
			// Update UI
			this.settingsPanel.selectedConfigIndex =
				this.configurationJson.configurations.length - 1;
			this.settingsPanel.updateConfigUI(
				configNames,
				this.configurationJson.configurations[
					this.settingsPanel.selectedConfigIndex
	...
	public handleConfigurationChange(): void {
		...
		void this.applyDefaultIncludePathsAndFrameworks().catch(
			logAndReturn.undefined,
		);
	...
	private async ensurePropertiesFile(): Promise<void> {
		if (
			this.propertiesFile &&
			(await util.checkFileExists(this.propertiesFile.fsPath))
		) {
			return;
		} else {
			try {
				...
				const fullPathToFile: string = path.join(
					this.configFolder,
					"c_cpp_properties.json",
				);
				...
				if (this.configurationJson) {
					if (!providerId) {
						providerId =
							this.configurationJson.configurations[0]
				...
				if (providerId) {
					if (this.configurationJson) {
						this.configurationJson.configurations[0].configurationProvider =
							providerId;
					}
				}
				...
				await util.writeFileText(
					fullPathToFile,
					jsonc.stringify(this.configurationJson, null, 4),
				);

				this.propertiesFile = vscode.Uri.file(
					path.join(this.configFolder, "c_cpp_properties.json"),
				);
			} catch (errJS) {
				const err: Error = errJS as Error;

				const failedToCreate: string = localize(
					"failed.to.create.config.folder",
					'Failed to create "{0}"',
					this.configFolder,
				);
				void vscode.window.showErrorMessage(
					`${failedToCreate}: ${err.message}`,
				);
			}
		}
		return;
	...
	private parsePropertiesFile(): boolean {
		...
		try {
			const readResults: string = fs.readFileSync(
			...
			const newJson: ConfigurationJson = jsonc.parse(
				readResults,
				undefined,
				true,
			) as any;
			...
			if (
				!newJson ||
				!newJson.configurations ||
				newJson.configurations.length === 0
			) {
				throw {
					message: localize(
			...
			if (
				...
				for (
					let i: number = 0;
					i < newJson.configurations.length;
					i++
				) {
					if (
						newJson.configurations[i].name ===
						this.configurationJson.configurations[
							this.CurrentConfigurationIndex
						].name
					) {
			...
			if (
				this.CurrentConfigurationIndex < 0 ||
				this.CurrentConfigurationIndex >= newJson.configurations.length
			) {
				const index: number | undefined =
					this.getConfigIndexForPlatform(newJson);
			...
			for (
				...
				const newId: string | undefined =
					getCustomConfigProviders().checkId(
						this.configurationJson.configurations[i]
							.configurationProvider,
					);
			...
			for (
				...
				if (
					this.configurationJson.configurations[i]
						.compilerPathIsExplicit !== undefined ||
					this.configurationJson.configurations[i]
						.cStandardIsExplicit !== undefined ||
					this.configurationJson.configurations[i]
						.cppStandardIsExplicit !== undefined ||
					this.configurationJson.configurations[i]
						.intelliSenseModeIsExplicit !== undefined
				) {
					dirty = true;
			...
			const failedToParse: string = localize(
				"failed.to.parse.properties",
				'Failed to parse "{0}"',
				this.propertiesFile.fsPath,
			);
			void vscode.window.showErrorMessage(
				`${failedToParse}: ${err.message}`,
			);
			success = false;
		}
	...
	private resolvePath(
		input_path: string | undefined | null,
		replaceAsterisks: boolean = true,
		assumeRelative: boolean = true,
	): string {
	...
	private getErrorsForConfigUI(configIndex: number): ConfigurationErrors {
		...
		const config: Configuration =
			this.configurationJson.configurations[configIndex];
		...
		if (config.compilerPath) {
			resolvedCompilerPath = which.sync(config.compilerPath, {
				nothrow: true,
			});
		}
		...
		if (
			...
			const compilerPathNeedsQuotes: boolean =
				...
				compilerPathAndArgs.compilerArgsFromCommandLineInPath.length >
					0 &&
			...
			if (compilerPathNeedsQuotes) {
				compilerPathErrors.push(
					localize(
						"path.with.spaces",
						'Compiler path with spaces and arguments is missing double quotes " around the path.',
					),
				);
			}
			...
			if (resolvedCompilerPath) {
				...
				const existsWithExeAdded: (path: string) => boolean = (
					path: string,
				) =>
					isWindows &&
					!path.startsWith("/") &&
					fs.existsSync(path + ".exe");
				...
				if (!pathExists) {
					...
					const message: string = localize(
						"cannot.resolve.compiler.path",
						"Invalid input, cannot resolve compiler path",
					);
					compilerPathErrors.push(message);
				} else if (
					!util.checkExecutableWithoutExtensionExistsSync(
						resolvedCompilerPath,
					)
				) {
					const message: string = localize(
		...
		errors.browsePath = this.validatePath(
			config.browse ? config.browse.path : undefined,
		);

		// Validate files
		errors.forcedInclude = this.validatePath(config.forcedInclude, {
			isDirectory: false,
			assumeRelative: false,
		});
		errors.compileCommands = this.validatePath(config.compileCommands, {
			isDirectory: false,
		});
		errors.dotConfig = this.validatePath(config.dotConfig, {
			isDirectory: false,
		});
		errors.databaseFilename = this.validatePath(
			config.browse ? config.browse.databaseFilename : undefined,
			{ isDirectory: false },
		);

		// Validate intelliSenseMode
		if (isWindows) {
			const intelliSenesModeError: string =
				this.validateIntelliSenseMode(config);
	...
	private validatePath(
		...
		paths = this.resolveAndSplit(
			paths,
			undefined,
			this.ExtendedEnvironment,
			assumeRelative,
			globPaths,
		);
		...
		for (const p of paths) {
			...
			if (!fs.existsSync(resolvedPath)) {
				if (assumeRelative && !path.isAbsolute(resolvedPath)) {
					...
					const relativePath: string =
						this.rootUri.fsPath + path.sep + resolvedPath;
			...
			if (isDirectory && !util.checkDirectoryExistsSync(resolvedPath)) {
				...
				!isDirectory &&
				!util.checkFileExistsSync(resolvedPath)
			) {
				const message: string = localize(
					"path.is.not.a.file",
					"Path is not a file: {0}",
	...
	private isConfigNameUnique(configName: string): string | undefined {
		...
		const occurrences: number | undefined = this.ConfigurationNames?.filter(
	...
	private async handleSquiggles(): Promise<void> {
		...
		if (
			(this.configurationJson.enableConfigurationSquiggles !==
				undefined &&
				!this.configurationJson.enableConfigurationSquiggles) ||
			(this.configurationJson.enableConfigurationSquiggles ===
				undefined &&
				!settings.defaultEnableConfigurationSquiggles)
		) {
		...
		// TODO?: Handle when jsonc.parse() throws an exception due to invalid JSON contents.
		const configurations: ConfigurationJson = jsonc.parse(
			configurationsText,
			undefined,
			true,
		) as any;
		...
		const currentConfiguration: Configuration =
			configurations.configurations[this.CurrentConfigurationIndex];
		...
		while (configStart !== -1) {
			...
			configNameStart =
				allConfigText.indexOf('"', allConfigText.indexOf(":") + 1) + 1;
			configNameEnd = allConfigText.indexOf('"', configNameStart);
			configName = allConfigText.substring(
				configNameStart,
				configNameEnd,
			);
			...
			const newRange: vscode.Range = new vscode.Range(
				0,
				allConfigTextOffset + configNameStart,
				0,
				allConfigTextOffset + configNameEnd,
			);

			const allRanges: vscode.Range[] | undefined =
				configNames.get(configName);
			...
			if (allRanges && allRanges.length > 1) {
				...
				allRanges.forEach((nameRange) => {
					const diagnostic: vscode.Diagnostic = new vscode.Diagnostic(
						new vscode.Range(
							document.positionAt(nameRange.start.character),
		...
		configStart = curText.search(
			new RegExp(
				`{\\s*"name"\\s*:\\s*"${escapeStringRegExp(currentConfiguration.name)}"`,
			),
		);

		if (configStart === -1) {
			telemetry.logLanguageServerEvent("ConfigSquiggles", {
				"error": "config name not first",
			});
		...
		const nextNameStart: number = curText.search(
			new RegExp('"name"\\s*:\\s*"'),
		);

		if (nextNameStart !== -1) {
			curText = curText.substring(0, nextNameStart + 6); // Remove later configs.
			const nextNameStart2: number = curText.search(
				new RegExp('\\s*}\\s*,\\s*{\\s*"name"'),
			);

			if (nextNameStart2 === -1) {
				telemetry.logLanguageServerEvent("ConfigSquiggles", {
					"error": "next config name not first",
				});
		...
		if (
			this.prevSquiggleMetrics.get(currentConfiguration.name) ===
			undefined
		) {
			this.prevSquiggleMetrics.set(currentConfiguration.name, {
				PathNonExistent: 0,
				PathNotAFile: 0,
				PathNotADirectory: 0,
				CompilerPathMissingQuotes: 0,
				CompilerModeMismatch: 0,
				MultiplePathsNotAllowed: 0,
			});
		}
		const newSquiggleMetrics: { [key: string]: number } = {
			PathNonExistent: 0,
			PathNotAFile: 0,
			PathNotADirectory: 0,
			CompilerPathMissingQuotes: 0,
		...
		if (isWindows) {
			...
			const intelliSenseModeStart: number = curText.search(
				/\s*\"intelliSenseMode\"\s*:\s*\"/,
			);

			if (intelliSenseModeStart !== -1) {
				const intelliSenseModeValueStart: number = curText.indexOf(
					'"',
					curText.indexOf(":", intelliSenseModeStart),
				);

				const intelliSenseModeValueEnd: number =
					intelliSenseModeStart === -1
						? -1
						: curText.indexOf('"', intelliSenseModeValueStart + 1) +
							1;

				const intelliSenseModeError: string =
					this.validateIntelliSenseMode(currentConfiguration);
				...
				if (intelliSenseModeError.length > 0) {
					...
					const diagnostic: vscode.Diagnostic = new vscode.Diagnostic(
						new vscode.Range(
							document.positionAt(
								curTextStartOffset + intelliSenseModeValueStart,
							),
							document.positionAt(
								curTextStartOffset + intelliSenseModeValueEnd,
							),
						),
						message,
						vscode.DiagnosticSeverity.Warning,
		...
		for (const pathArray of [
			currentConfiguration.browse
				? currentConfiguration.browse.path
				: undefined,
			currentConfiguration.includePath,
			currentConfiguration.macFrameworkPath,
		]) {
			if (pathArray) {
		...
		const forcedIncludeStart: number = curText.search(
			/\s*\"forcedInclude\"\s*:\s*\[/,
		);

		const forcedeIncludeEnd: number =
			forcedIncludeStart === -1
				? -1
				: curText.indexOf("]", forcedIncludeStart);

		const compileCommandsStart: number = curText.search(
			/\s*\"compileCommands\"\s*:\s*\"/,
		);

		const compileCommandsEnd: number =
			compileCommandsStart === -1
				? -1
				: curText.indexOf(
						'"',
						curText.indexOf(
							'"',
							curText.indexOf(":", compileCommandsStart),
						) + 1,
					);

		const compilerPathStart: number = curText.search(
			/\s*\"compilerPath\"\s*:\s*\"/,
		);

		const compilerPathValueStart: number = curText.indexOf(
			'"',
			curText.indexOf(":", compilerPathStart),
		);

		const compilerPathEnd: number =
			compilerPathStart === -1
				? -1
				: curText.indexOf('"', compilerPathValueStart + 1) + 1;

		const dotConfigStart: number = curText.search(
			/\s*\"dotConfig\"\s*:\s*\"/,
		);

		const dotConfigValueStart: number = curText.indexOf(
			'"',
			curText.indexOf(":", dotConfigStart),
		);

		const dotConfigEnd: number =
			dotConfigStart === -1
				? -1
				: curText.indexOf('"', dotConfigValueStart + 1) + 1;
		...
		const compilerLowerCase: string =
			compilerPathAndArgs.compilerName.toLowerCase();

		const isClCompiler: boolean =
			compilerLowerCase === "cl" || compilerLowerCase === "cl.exe";
		...
		if (compilerPathAndArgs.compilerPath && !isClCompiler) {
			...
			if (
				compilerPathNeedsQuotes ||
				(compilerPath && !which.sync(compilerPath, { nothrow: true }))
			) {
				if (compilerPathNeedsQuotes) {
					compilerMessage = localize(
						"path.with.spaces",
						'Compiler path with spaces and arguments is missing double quotes " around the path.',
					);
		...
		if (this.rootUri && !isClCompiler) {
			const checkPathExists: any = util.checkPathExistsSync(
				compilerPath,
				this.rootUri.fsPath + path.sep,
				isWindows,
				true,
			);
		...
		if (compilerMessage) {
			const diagnostic: vscode.Diagnostic = new vscode.Diagnostic(
				new vscode.Range(
					document.positionAt(
						curTextStartOffset + compilerPathValueStart,
					),
		...
		if (dotConfigMessage) {
			const diagnostic: vscode.Diagnostic = new vscode.Diagnostic(
				new vscode.Range(
					document.positionAt(
						curTextStartOffset + dotConfigValueStart,
					),
		...
		for (const curPath of paths) {
			...
			const expandedPaths: string[] = this.resolveAndSplit(
				[curPath],
				undefined,
				this.ExtendedEnvironment,
				true,
			...
			if (this.rootUri) {
				for (const [index, expandedPath] of expandedPaths.entries()) {
					if (expandedPath.includes("${workspaceFolder}")) {
						expandedPaths[index] = this.resolvePath(
							expandedPath,
							false,
						);
					} else {
			...
			for (const [index, expandedPath] of expandedPaths.entries()) {
				...
				if (path.sep === "/") {
					expandedPaths[index] = expandedPath.replace(
						/\\/g,
						path.sep,
					);
				} else {
			...
			if (asteriskPosition !== -1) {
				if (
					asteriskPosition !== curPath.length - 1 &&
					asteriskPosition !== curPath.length - 2
				) {
					globPath = true;
			...
			if (configMatches && !globPath) {
				...
				for (const curMatch of configMatches) {
					curOffset =
						curText.substring(endOffset).search(pattern) +
						endOffset;
					endOffset = curOffset + curMatch.length;

					if (
						curOffset >= compilerPathStart &&
						curOffset <= compilerPathEnd
					) {
						continue;
					}
					let message: string = "";
					...
					if (!pathExists) {
						if (
							curOffset >= forcedIncludeStart &&
							curOffset <= forcedeIncludeEnd &&
						...
						if (incorrectExpandedPaths.length > 0) {
							badPath = incorrectExpandedPaths
								.map((s) => `"${s}"`)
								.join(", ");
						} else {
						...
						if (
							(curOffset >= forcedIncludeStart &&
								curOffset <= forcedeIncludeEnd) ||
							(curOffset >= compileCommandsStart &&
								curOffset <= compileCommandsEnd)
						) {
							...
							for (const expandedPath of expandedPaths) {
								const resolvedPath =
									this.resolvePath(expandedPath);

								if (
									!util.checkDirectoryExistsSync(resolvedPath)
								) {
					...
					const diagnostic: vscode.Diagnostic = new vscode.Diagnostic(
						new vscode.Range(
							document.positionAt(curTextStartOffset + curOffset),
				...
				if (envMatches) {
					...
					for (const curMatch of envMatches) {
						curOffset =
							envText.substring(endOffset).search(pattern) +
							endOffset;
						endOffset = curOffset + curMatch.length;
						...
						if (!pathExists) {
							...
							const diagnostic: vscode.Diagnostic =
								new vscode.Diagnostic(
									new vscode.Range(
										document.positionAt(
											envTextStartOffSet + curOffset,
										),
										document.positionAt(
											envTextStartOffSet + endOffset,
										),
									),
									message,
									vscode.DiagnosticSeverity.Warning,
		...
		if (
			...
			changedSquiggleMetrics.PathNonExistent =
				newSquiggleMetrics.PathNonExistent;
		}
		if (
			newSquiggleMetrics.PathNotAFile !==
			...
			changedSquiggleMetrics.PathNotAFile =
				newSquiggleMetrics.PathNotAFile;
		}
		if (
			newSquiggleMetrics.PathNotADirectory !==
			...
			changedSquiggleMetrics.PathNotADirectory =
				newSquiggleMetrics.PathNotADirectory;
		}
		if (
			newSquiggleMetrics.CompilerPathMissingQuotes !==
			this.prevSquiggleMetrics.get(currentConfiguration.name)
				?.CompilerPathMissingQuotes
		) {
			changedSquiggleMetrics.CompilerPathMissingQuotes =
				newSquiggleMetrics.CompilerPathMissingQuotes;
		}
		if (
			newSquiggleMetrics.CompilerModeMismatch !==
			this.prevSquiggleMetrics.get(currentConfiguration.name)
				?.CompilerModeMismatch
		) {
			changedSquiggleMetrics.CompilerModeMismatch =
				newSquiggleMetrics.CompilerModeMismatch;
		}
		if (
			newSquiggleMetrics.MultiplePathsNotAllowed !==
			this.prevSquiggleMetrics.get(currentConfiguration.name)
				?.MultiplePathsNotAllowed
		) {
			changedSquiggleMetrics.MultiplePathsNotAllowed =
				newSquiggleMetrics.MultiplePathsNotAllowed;
		}
	...
	private updateToVersion4(): void {
		if (this.configurationJson) {
			...
			for (
				...
				if (
					config.intelliSenseMode === undefined &&
					!settings.defaultIntelliSenseMode
				) {
					config.intelliSenseMode =
						this.getIntelliSenseModeForPlatform(config.name);
				}
	...
	private writeToJson(): void {
		...
		if (this.configurationJson) {
			this.configurationJson.configurations.forEach((e) => {
				...
				savedIntelliSenseModeIsExplicit.push(
					!!e.intelliSenseModeIsExplicit,
				);
		...
		if (this.configurationJson) {
			for (
				...
				this.configurationJson.configurations[
					i
				].compilerPathIsExplicit = savedCompilerPathIsExplicit[i];
				this.configurationJson.configurations[i].cStandardIsExplicit =
					savedCStandardIsExplicit[i];
				this.configurationJson.configurations[i].cppStandardIsExplicit =
					savedCppStandardIsExplicit[i];
				this.configurationJson.configurations[
					i
				].intelliSenseModeIsExplicit =
					savedIntelliSenseModeIsExplicit[i];
			}
		}
	}
	...
	public checkCppProperties(): void {
		...
		const propertiesFile: string = path.join(
			this.configFolder,
			"c_cpp_properties.json",
		);
	...
	public checkCompileCommands(): void {
		...
		const compileCommands: string | undefined =
			this.CurrentConfiguration?.compileCommands;

		if (!compileCommands) {
			return;
		}
		const compileCommandsFile: string | undefined =
			this.resolvePath(compileCommands);
		fs.stat(compileCommandsFile, (err, stats) => {
			if (err) {
				if (err.code === "ENOENT" && this.compileCommandsFile) {
					this.compileCommandsFileWatchers.forEach(
						(watcher: fs.FSWatcher) => watcher.close(),
					);
				...
				const compileCommandsLastChanged: Date | undefined =
					this.compileCommandsFileWatcherFallbackTime.get(
						compileCommandsFile,
					);

				if (
					compileCommandsLastChanged !== undefined &&
					stats.mtime > compileCommandsLastChanged
				) {
					this.compileCommandsFileWatcherFallbackTime.set(
						compileCommandsFile,
						new Date(),
					...
					this.onCompileCommandsChanged(compileCommandsFile);
					this.compileCommandsFile =
						vscode.Uri.file(compileCommandsFile); // File created.
				}
			}
		});
	}
