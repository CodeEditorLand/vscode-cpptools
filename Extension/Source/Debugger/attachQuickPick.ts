...
class RefreshButton implements vscode.QuickInputButton {
	get iconPath(): { dark: vscode.Uri; light: vscode.Uri } {
		const refreshImagePathDark: string = util.getExtensionFilePath(
			"assets/Refresh_inverse.svg",
		);

		const refreshImagePathLight: string =
			util.getExtensionFilePath("assets/Refresh.svg");
...
export async function showQuickPick(
	...
	return new Promise<string | undefined>((resolve, reject) => {
		...
		quickPick.onDidTriggerButton(
			async () => {
				quickPick.items = await getAttachItems();
			},
			undefined,
			disposables,
		);
