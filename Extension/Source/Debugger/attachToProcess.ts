...
export class AttachPicker {
	...
	public async ShowAttachEntries(
		token?: vscode.CancellationToken,
	): Promise<string | undefined> {
		return showQuickPick(() =>
			this.attachItemsProvider.getAttachItems(token),
		);
	}
}
...
export class RemoteAttachPicker {
	...
	public async ShowAttachEntries(config: any): Promise<string | undefined> {
		...
		const useExtendedRemote: any = config
			? config.useExtendedRemote
			: undefined;

		const miDebuggerPath: any = config ? config.miDebuggerPath : undefined;

		const miDebuggerServerAddress: any = config
			? config.miDebuggerServerAddress
			: undefined;
		...
		if (pipeTransport) {
			...
			if (
				...
				const pipeProgramStr: string = pipeTransport.pipeProgram
					.toLowerCase()
					.trim();

				const expectedArch: debugUtils.ArchType =
					debugUtils.ArchType[
			...
			const argList: string =
				RemoteAttachPicker.createArgumentList(pipeArgs);
			...
			if (!miDebuggerPath || !miDebuggerServerAddress) {
				throw new Error(
					localize(
						"debugger.path.and.server.address.required",
						"{0} in debug configuration requires {1} and {2}",
						"useExtendedRemote",
						"miDebuggerPath",
			...
			throw new Error(
				localize(
					"no.pipetransport.useextendedremote",
					"Chosen debug configuration does not contain {0} or {1}",
					"pipeTransport",
					"useExtendedRemote",
				),
			);
		...
		const item: AttachItem | undefined = await vscode.window.showQuickPick(
			processes,
			attachPickOptions,
		);
		...
		if (item) {
			...
			throw new Error(
				localize("process.not.selected", "Process not selected."),
			);
		}
	}
	...
	private getRemoteProcessCommand(): string {
		...
		return (
			`${outerQuote}${shPrefix}sh -c ${innerQuote}uname && if [ ${parameterBegin}uname -o${parameterEnd} = ${escapedQuote}Toybox${escapedQuote} ] ; ` +
	...
	private async getRemoteOSAndProcesses(
		...
		if (lines.length === 0) {
			throw new Error(
				localize(
					"pipe.failed",
					"Pipe transport failed to get OS and processes.",
				),
			);
		} else {
			...
			if (remoteOS !== "Linux" && remoteOS !== "Darwin") {
				throw new Error(
					`Operating system "${remoteOS}" not supported.`,
				);
			}
			...
			if (lines.length === 1) {
				throw new Error(
					localize(
						"no.process.list",
						"Transport attach could not obtain processes list.",
					),
				);
			} else {
	...
	private async getRemoteProcessesExtendedRemote(
		miDebuggerPath: string,
		miDebuggerServerAddress: string,
	): Promise<AttachItem[]> {
		const args: string[] = [
			`-ex "target extended-remote ${miDebuggerServerAddress}"`,
			'-ex "info os processes"',
			"-batch",
		];

		let processListOutput: util.ProcessReturnType =
			await util.spawnChildProcess(miDebuggerPath, args);
		...
		if (!processListOutput.succeeded) {
			throw new Error(
				localize(
					"failed.to.make.gdb.connection",
					'Failed to make GDB connection: "{0}".',
    ...
    */
	private parseProcessesFromInfoOsProcesses(
		processList: string,
	): AttachItem[] {
		...
		for (const line of lines) {
			...
			if (!trimmedLine.endsWith("?")) {
				const matches: RegExpMatchArray | null = trimmedLine.match(
					/^(\d+)\s+(.+?)\s+(?:\d+,)*\d+$/,
				);
