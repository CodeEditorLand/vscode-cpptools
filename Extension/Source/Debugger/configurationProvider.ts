	...
	implements vscode.DebugConfigurationProvider
{
	...
	public constructor(
		assetProvider: IConfigurationAssetProvider,
		type: DebuggerType,
	) {
	...
	async provideDebugConfigurations(
	...
	): Promise<CppDebugConfiguration[]> {
		let configs: CppDebugConfiguration[] | null | undefined =
			await this.provideDebugConfigurationsForType(
				this.type,
				folder,
				token,
			);
		...
		const defaultConfig: CppDebugConfiguration[] =
			this.findDefaultConfig(configs);
		...
		const items: ConfigMenu[] = configs.map<ConfigMenu>((config) => {
			...
			const menuItem: ConfigMenu = {
				label: config.name,
				configuration: quickPickConfig,
				description: config.detail,
				detail: config.taskStatus,
			};
		...
		const selection: ConfigMenu | undefined =
			await vscode.window.showQuickPick(
				this.localizeConfigDetail(items),
				{
	...
	async resolveDebugConfiguration(
	...
	): Promise<CppDebugConfiguration | null | undefined> {
		if (!config || !config.type) {
			...
			const configs: CppDebugConfiguration[] =
				await this.provideDebugConfigurations(folder);
			if (!configs || configs.length === 0) {
				Telemetry.logDebuggerEvent(DebuggerEvent.debugPanel, {
					...
					"configMode": ConfigMode.noLaunchConfig,
					"cancelled": "true",
					"succeeded": "true",
				});
		...
		if (config.preLaunchTask) {
			...
			const isIntelliSenseDisabled: boolean =
				new CppSettings(
			...
			if (isIntelliSenseDisabled) {
				try {
					...
					Telemetry.logDebuggerEvent(DebuggerEvent.debugPanel, {
						"debugType": DebugType.debug,
						"configSource":
							config.configSource || ConfigSource.unknown,
			...
			const isDebugPanel: boolean =
				!config.debuggerEvent ||
				(config.debuggerEvent &&
					config.debuggerEvent === DebuggerEvent.debugPanel);
			const singleFile: boolean =
				config.configSource === ConfigSource.singleFile;
			const isExistingConfig: boolean = this.isExistingConfig(
				config,
				folder,
			);
			const isExistingTask: boolean = await this.isExistingTask(
				config,
				folder,
			);
			if (singleFile) {
			...
			config.debugType = config.debugType
				? config.debugType
				: DebugType.debug;
			const configMode: ConfigMode = isExistingConfig
				? ConfigMode.launchConfig
				: ConfigMode.noLaunchConfig;
			...
			Telemetry.logDebuggerEvent(
				config.debuggerEvent || DebuggerEvent.debugPanel,
				{
					"debugType": config.debugType || DebugType.debug,
					"configSource": config.configSource || ConfigSource.unknown,
	...
	async resolveDebugConfigurationWithSubstitutedVariables(
		...
		if (config.type === DebuggerType.cppvsdbg) {
			...
			if (!config.enableDebugHeap) {
				const disableDebugHeapEnvSetting: Environment = {
					"name": "_NO_DEBUG_HEAP",
					"value": "1",
				};
		...
		if (
			...
			replacedPipeProgram =
				debugUtils.ArchitectureReplacer.checkAndReplaceWSLPipeProgram(
					pipeProgramStr,
					debugUtils.ArchType.ia32,
				);

			// If pipeProgram does not get replaced and there is a pipeCwd, concatenate with pipeProgramStr and attempt to replace.
			if (
				!replacedPipeProgram &&
				!path.isAbsolute(pipeProgramStr) &&
				config.pipeTransport.pipeCwd
			) {
				const pipeCwdStr: string = config.pipeTransport.pipeCwd
					.toLowerCase()
					.trim();
				const newPipeProgramStr: string = path.join(
					pipeCwdStr,
					pipeProgramStr,
				);
		...
		const macOSMIDebuggerPath: string =
			config.osx?.miDebuggerPath ?? config.miDebuggerPath;

		const lldb_mi_10_x_path: string = path.join(
			util.extensionPath,
			"debugAdapters",
			"lldb-mi",
			"bin",
		...
		// Validate LLDB-MI
		if (
			os.platform() === "darwin" && // Check for macOS
			...
			const frameworkPath: string | undefined =
				this.getLLDBFrameworkPath();
			...
			if (!frameworkPath) {
				...
				const LLDBFrameworkMissingMessage: string = localize(
		...
		if (config.deploySteps && config.deploySteps.length !== 0) {
			const codeVersion: number[] = vscode.version
				.split(".")
				.map((num) => parseInt(num, undefined));
			if (
				(util.isNumber(codeVersion[0]) && codeVersion[0] < 1) ||
				(util.isNumber(codeVersion[0]) &&
					codeVersion[0] === 1 &&
					util.isNumber(codeVersion[1]) &&
					codeVersion[1] < 69)
			) {
				void logger
					.getOutputChannelLogger()
		...
		if (config.request === "attach" && !config.processId) {
			...
			if (processId) {
				...
				void logger
					.getOutputChannelLogger()
					.showErrorMessage("No process was selected.");
				return undefined;
			}
		}
	...
	async provideDebugConfigurationsForType(
		type: DebuggerType,
		folder?: vscode.WorkspaceFolder,
		_token?: vscode.CancellationToken,
	): Promise<CppDebugConfiguration[]> {
		const defaultTemplateConfig: CppDebugConfiguration = this.assetProvider
			.getInitialConfigurations(type)
			.find(
				(config: any) =>
					isDebugLaunchStr(config.name) &&
					config.request === "launch",
		...
		const dedupDetectedBuildTasks: CppBuildTask[] =
			DebugConfigurationProvider.detectedBuildTasks.filter(
				(taskDetected) =>
					!configuredBuildTasks.some(
						(taskJson) =>
				...
				await Promise.all(
					buildTasks.map<Promise<CppDebugConfiguration | undefined>>(
						async (task) => {
							...
							const newConfig: CppDebugConfiguration = {
								...defaultTemplateConfig,
							}; // Copy enumerables and properties
							...
							if (platformInfo.platform !== "darwin") {
								...
								if (compilerName.startsWith("clang")) {
									...
									if (isWindows) {
										...
										if (
											...
											compilerName !== "clang-cpp.exe"
										) {
											const suffixIndex: number =
												compilerName.indexOf("-");
											if (suffixIndex !== -1) {
												const suffix: string =
								...
								const compilerDirname: string =
									path.dirname(compilerPath);
								const debuggerPath: string = path.join(
									compilerDirname,
									debuggerName,
								);
								...
								if (await util.checkFileExists(debuggerPath)) {
									...
									(await util.whichAsync(debuggerName)) !==
									undefined
								) {
									...
									const usrDebuggerPath: string = path.join(
										"/usr",
										"bin",
										debuggerName,
									);
									if (
										!isWindows &&
										(await util.checkFileExists(
											usrDebuggerPath,
										))
									) {
										newConfig.miDebuggerPath =
											usrDebuggerPath;
									} else {
										logger
											.getOutputChannelLogger()
		...
		const existingConfigs: CppDebugConfiguration[] | undefined =
			this.getLaunchConfigs(folder, type)?.map((config) => {
				if (!config.detail && config.preLaunchTask) {
					config.detail = localize(
						"pre.Launch.Task",
						"preLaunchTask: {0}",
						config.preLaunchTask,
					);
		...
		if (existingConfigs) {
			...
			const dedupExistingConfigs: CppDebugConfiguration[] =
				configs.filter(
					(detectedConfig) =>
						!existingConfigs.some((config) => {
							if (
								config.preLaunchTask ===
									detectedConfig.preLaunchTask &&
								config.type === detectedConfig.type &&
								config.request === detectedConfig.request
							) {
								// Carry the default task information.
								config.isDefault = detectedConfig.isDefault
	...
	private async loadDetectedTasks(): Promise<void> {
		...
		if (fileIsCpp) {
			if (
				!DebugConfigurationProvider.detectedCppBuildTasks ||
				DebugConfigurationProvider.detectedCppBuildTasks.length === 0
			) {
				DebugConfigurationProvider.detectedCppBuildTasks =
			...
			if (
				!DebugConfigurationProvider.detectedCBuildTasks ||
				DebugConfigurationProvider.detectedCBuildTasks.length === 0
			) {
				DebugConfigurationProvider.detectedCBuildTasks =
					await cppBuildTaskProvider.getTasks(true);
			}
		...
		if (!process.env.DevEnvDir || process.env.DevEnvDir.length === 0) {
			void vscode.window.showErrorMessage(
				localize(
					...
					"{0} build and debug is only usable when VS Code is run from the Developer Command Prompt for VS.",
					"cl.exe",
				),
			);
			return true;
	...
	private resolveEnvFile(
		config: CppDebugConfiguration,
		folder?: vscode.WorkspaceFolder,
	): void {
		if (config.envFile) {
			// replace ${env:???} variables
			let envFilePath: string = util.resolveVariables(
				config.envFile,
				undefined,
			);
			...
			try {
				...
				const parsedFile: ParsedEnvironmentFile =
					ParsedEnvironmentFile.CreateFromFile(
						envFilePath,
						config["environment"],
					);
				...
				if (parsedFile.Warning) {
					void DebugConfigurationProvider.showFileWarningAsync(
						parsedFile.Warning,
						config.envFile,
					);
				}
	...
	private resolveSourceFileMapVariables(config: CppDebugConfiguration): void {
		...
		if (config.sourceFileMap) {
			for (const sourceFileMapSource of Object.keys(
				config.sourceFileMap,
			)) {
				let message: string = "";
				const sourceFileMapTarget: string =
					config.sourceFileMap[sourceFileMapSource];
				...
				if (util.isString(sourceFileMapTarget)) {
					const newSourceFileMapTarget: string =
						util.resolveVariables(sourceFileMapTarget, undefined);
	...
	private static async showFileWarningAsync(
		message: string,
		fileName: string,
	): Promise<void> {
		const openItem: vscode.MessageItem = {
			title: localize("open.envfile", "Open {0}", "envFile"),
		};
		const result: vscode.MessageItem | undefined =
			await vscode.window.showWarningMessage(message, openItem);
	...
	private findDefaultConfig(
		configs: CppDebugConfiguration[],
	): CppDebugConfiguration[] {
		// eslint-disable-next-line no-prototype-builtins
		return configs.filter(
			(config: CppDebugConfiguration) =>
				config.hasOwnProperty("isDefault") && config.isDefault,
		);
	}

	private async isExistingTask(
		config: CppDebugConfiguration,
		folder?: vscode.WorkspaceFolder,
	): Promise<boolean> {
		if (config.taskStatus && config.taskStatus !== TaskStatus.detected) {
			...
			config.taskStatus &&
			config.taskStatus === TaskStatus.detected
		) {
			return false;
		}
	...
	private isExistingConfig(
		...
		if (configs && configs.length > 0) {
			const selectedConfig: any | undefined = configs.find(
				(item: any) => item.name && item.name === config.name,
			);
			if (selectedConfig) {
				return true;
			}
	...
	private getDebugConfigSource(
		...
		const configs: CppDebugConfiguration[] | undefined =
			this.getLaunchConfigs(folder, config.type);
		const matchingConfig: CppDebugConfiguration | undefined = configs?.find(
			(item: any) => item.name && item.name === config.name,
		);
	...
	public getLaunchConfigs(
		folder?: vscode.WorkspaceFolder,
		type?: DebuggerType | string,
	): CppDebugConfiguration[] | undefined {
		// Get existing debug configurations from launch.json or user/workspace "launch" settings.
		const WorkspaceConfigs: vscode.WorkspaceConfiguration =
			vscode.workspace.getConfiguration("launch", folder);
		const configs: any =
			WorkspaceConfigs.inspect<vscode.DebugConfiguration>(
				"configurations",
			);
		if (!configs) {
			return undefined;
		...
		if (configs.workspaceFolderValue !== undefined) {
			detailedConfigs = detailedConfigs.concat(
				configs.workspaceFolderValue.map(
					(item: CppDebugConfiguration) => {
	...
	public async writeDebugConfig(
		...
		if (!launchJsonPath) {
			throw new Error(
				"Failed to get tasksJsonPath in checkBuildTaskExists()",
			);
		}
	...
	public async addDebugConfiguration(
		...
		if (
			!isExistingConfig &&
			selectedConfig.preLaunchTask &&
			selectedConfig.taskStatus &&
			selectedConfig.taskStatus === TaskStatus.detected
		) {
			await cppBuildTaskProvider.writeBuildTask(
				selectedConfig.preLaunchTask,
			);
		}
	...
	public async buildAndDebug(
		textEditor: vscode.TextEditor,
		debugModeOn: boolean = true,
	): Promise<void> {
		let folder: vscode.WorkspaceFolder | undefined =
			vscode.workspace.getWorkspaceFolder(textEditor.document.uri);
		const selectedConfig: CppDebugConfiguration | undefined =
			await this.selectConfiguration(textEditor);
		if (!selectedConfig) {
			Telemetry.logDebuggerEvent(DebuggerEvent.playButton, {
				"debugType": debugModeOn ? DebugType.debug : DebugType.run,
				"configSource": ConfigSource.unknown,
				"cancelled": "true",
				"succeeded": "true",
			});
		...
		if (
			selectedConfig.preLaunchTask &&
			selectedConfig.configSource &&
			(selectedConfig.configSource === ConfigSource.global ||
				selectedConfig.configSource === ConfigSource.workspace) &&
		...
		await vscode.debug.startDebugging(folder, selectedConfig, {
			noDebug: !debugModeOn,
		});
	}
	...
	private async selectConfiguration(
		textEditor: vscode.TextEditor,
		pickDefault: boolean = true,
		onlyWorkspaceFolder: boolean = false,
	): Promise<CppDebugConfiguration | undefined> {
		const folder: vscode.WorkspaceFolder | undefined =
			vscode.workspace.getWorkspaceFolder(textEditor.document.uri);
		...
		const defaultConfig: CppDebugConfiguration[] | undefined = pickDefault
			? this.findDefaultConfig(configs)
			: undefined;
		...
		if (defaultConfig && defaultConfig.length === 1) {
			selection = {
				label: defaultConfig[0].name,
				configuration: defaultConfig[0],
				description: defaultConfig[0].detail,
				detail: defaultConfig[0].taskStatus,
			};
		} else {
			let sortedItems: ConfigMenu[] = [];
			// Find the recently used task and place it at the top of quickpick list.
			const recentTask: ConfigMenu[] = items.filter(
				(item) =>
					item.configuration.preLaunchTask &&
					item.configuration.preLaunchTask ===
						DebugConfigurationProvider.recentBuildTaskLabelStr,
			);
			if (
			...
			selection = await vscode.window.showQuickPick(
				this.localizeConfigDetail(sortedItems),
				{
					placeHolder:
						items.length === 0
							? localize("no.compiler.found", "No compiler found")
							: localize(
		...
		if (
			...
			this.showErrorIfClNotAvailable(selection.configuration.name)
		) {
			return;
		}
		return selection?.configuration;
	...
	private async resolvePreLaunchTask(
		config: CppDebugConfiguration,
		configMode: ConfigMode,
		folder?: vscode.WorkspaceFolder | undefined,
	): Promise<void> {
		if (config.preLaunchTask) {
			try {
				if (config.configSource === ConfigSource.singleFile) {
					...
					await cppBuildTaskProvider.writeDefaultBuildTask(
						config.preLaunchTask,
						folder,
					);
				}
	...
	private async expand(
		...
		vars.workspaceFolderBasename = folderPath
	...
	private async deploySteps(
		...
		for (const step of config.deploySteps) {
			succeeded = await this.singleDeployStep(
				config,
				step,
				cancellationToken,
			);
	...
	private async singleDeployStep(
		config: vscode.DebugConfiguration,
		step: any,
		cancellationToken?: vscode.CancellationToken,
	): Promise<boolean> {
		if (
			(config.noDebug && step.debug === true) ||
			(!config.noDebug && step.debug === false)
		) {
		...
		switch (stepType) {
			...
			case StepType.rsync: {
				...
				if (util.isString(step.files)) {
					files = files.concat(
						(await globAsync(step.files)).map((file) =>
					...
					for (const fileGlob of step.files as string[]) {
						files = files.concat(
							(await globAsync(fileGlob)).map((file) =>
								vscode.Uri.file(file),
							),
						);
					}
			...
			case StepType.ssh: {
				...
				const jumpHosts: util.ISshHostInfo[] = step.host.jumpHosts;
				const localForwards: util.ISshLocalForwardInfo[] =
					step.host.localForwards;
				const continueOn: string = step.continueOn;
				const sshResult: util.ProcessReturnType = await ssh(
					host,
					step.command,
					config.sshPath,
					jumpHosts,
			...
			case StepType.shell: {
				...
				if (
					...
					void logger
						.getOutputChannelLogger()
						.showErrorMessage(taskResult.output);
					return false;
				}
				break;
			}
...
export class ConfigurationAssetProviderFactory {
	public static getConfigurationProvider(): IConfigurationAssetProvider {
		switch (os.platform()) {
			...
			default:
				throw new Error(
					localize("unexpected.os", "Unexpected OS type"),
				);
		}
	}
}

abstract class DefaultConfigurationProvider
	implements IConfigurationAssetProvider
{
	...
	public getInitialConfigurations(debuggerType: DebuggerType): any {
		...
		const initialConfigurations: any = configurationSnippet
			.filter(
				(snippet) =>
					snippet.debuggerType === debuggerType &&
					snippet.isInitialConfiguration,
			)
	...
	constructor() {
		...
		this.configurations = [
			new MIConfigurations(
				...
				this.executable,
				this.pipeProgram,
				this.setupCommandsBlock,
			),
			new PipeTransportConfigurations(
				this.MIMode,
				this.executable,
				this.pipeProgram,
				this.setupCommandsBlock,
...
function convertConfigurationSnippetToCompletionItem(
	snippet: IConfigurationSnippet,
...
export class ConfigurationSnippetProvider
