...
export class NativeAttachItemsProviderFactory {
	static Get(): AttachItemsProvider {
		if (os.platform() === "win32") {
			...
			return pwsh
				? new CimAttachItemsProvider(pwsh)
				: new WmicAttachItemsProvider();
		} else {
...
abstract class NativeAttachItemsProvider implements AttachItemsProvider {
	protected abstract getInternalProcessEntries(
		token?: vscode.CancellationToken,
	): Promise<Process[]>;

	async getAttachItems(
		token?: vscode.CancellationToken,
	): Promise<AttachItem[]> {
		const processEntries: Process[] =
			await this.getInternalProcessEntries(token);
		...
		const attachItems: AttachItem[] = processEntries.map((p) =>
			p.toAttachItem(),
		);
...
export class PsAttachItemsProvider extends NativeAttachItemsProvider {
	...
	protected async getInternalProcessEntries(
		token?: vscode.CancellationToken,
	): Promise<Process[]> {
...
export class PsProcessParser {
	...
	public static ParseProcessFromPsArray(processArray: string[]): Process[] {
		...
		for (let i: number = 1; i < processArray.length; i++) {
			...
			const processEntry: Process | undefined =
				PsProcessParser.parseLineFromPs(line);
	...
	private static parseLineFromPs(line: string): Process | undefined {
		...
		const psEntry: RegExp = new RegExp(
			`^\\s*([0-9]+)\\s+(.{${PsProcessParser.secondColumnCharacters - 1}})\\s+(.*)$`,
		);
...
export class WmicProcessParser {
	...
	public static ParseProcessFromWmic(processes: string): Process[] {
		...
		let currentProcess: Process = new Process(
			"current process",
			undefined,
			undefined,
		);
