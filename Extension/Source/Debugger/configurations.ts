...
`,
		[
			type === "cppdbg"
				? `${os.EOL}"program": "${localize("enter.program.name", "enter program name, for example {0}", "${workspaceFolder}" + "/" + executable).replace(/"/g, "")}",`
				: "",
		],
	);
}

function createRemoteAttachString(
	name: string,
	type: string,
	executable: string,
): string {
...
function createPipeTransportString(
	pipeProgram: string,
...
\t"MIMode": "${this.MIMode}"{0}{1}
			...
			[
				...
				this.additionalProperties
					? `,${os.EOL}\t${indentJsonString(this.additionalProperties)}`
					: "",
			],
		);
...
export class WSLConfigurations extends Configuration {
	...
	public bashPipeProgram =
		process.arch === "ia32"
			? "${env:windir}\\\\sysnative\\\\bash.exe"
