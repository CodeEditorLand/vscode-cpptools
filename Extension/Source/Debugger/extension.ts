...
import {
	...
	parseFailures,
	writeSshConfiguration,
} from "../SSH/sshHosts";
import {
	addSshTargetCmd,
	BaseNode,
	refreshCppSshTargetsViewCmd,
} from "../SSH/TargetsView/common";
import {
	getActiveSshTarget,
	initializeSshTargets,
	selectSshTarget,
	SshTargetsProvider,
} from "../SSH/TargetsView/sshTargetsProvider";
import {
	setActiveSshTarget,
	TargetLeafNode,
} from "../SSH/TargetsView/targetNodes";
import {
	AttachItemsProvider,
	AttachPicker,
	RemoteAttachPicker,
...
export async function initialize(
	...
	disposables.push(
		vscode.commands.registerCommand(
			...
			(any) => remoteAttacher.ShowAttachEntries(any),
		),
	);

	// Activate ConfigurationProvider
	const assetProvider: IConfigurationAssetProvider =
		ConfigurationAssetProviderFactory.getConfigurationProvider();
	...
	if (os.platform() === "win32") {
		cppVsDebugProvider = new DebugConfigurationProvider(
			assetProvider,
	...
	const debugProvider: DebugConfigurationProvider =
		new DebugConfigurationProvider(assetProvider, DebuggerType.all);
	...
	disposables.push(
		vscode.commands.registerTextEditorCommand(
			...
			async (
				...
				await debugProvider.buildAndDebug(textEditor);
			},
		),
	);
	// eslint-disable-next-line @typescript-eslint/no-misused-promises
	disposables.push(
		vscode.commands.registerTextEditorCommand(
			...
			async (
				...
				await debugProvider.buildAndRun(textEditor);
			},
		),
	);
	// eslint-disable-next-line @typescript-eslint/no-misused-promises
	...
	disposables.push(
		vscode.languages.registerCompletionItemProvider(
			...
			new ConfigurationSnippetProvider(assetProvider),
		),
	);
	...
	disposables.push(
		vscode.commands.registerCommand(
			refreshCppSshTargetsViewCmd,
			(node?: BaseNode) =>
				enableSshTargetsViewAndRun(
					(node?: BaseNode) => sshTargetsProvider.refresh(node),
					node,
				),
		),
	);
	...
	if (
		sshTargetsViewSetting === "enabled" ||
		(sshTargetsViewSetting === "default" &&
			(await getActiveSshTarget(false)))
	) {
		// Don't wait
...
async function enableSshTargetsViewAndRun<T>(
	func: (...paras: any[]) => T | Promise<T>,
	...args: any[]
): Promise<T> {
	await enableSshTargetsView();
...
async function addSshTargetImpl(): Promise<string> {
	...
	for (const configFile of getSshConfigurationFiles()) {
		if (
			(await pathAccessible(configFile)) &&
			parseFailures.get(configFile)
		) {
	...
	const newEntry: { [key: string]: string } = sshCommandToConfig(
		command,
		name,
	);

	const targetFile: string | undefined = await vscode.window.showQuickPick(
		validConfigFiles,
		{
			title: localize(
	...
	const parsedSshConfig: Configuration = await getSshConfiguration(
		targetFile,
		false,
	);
