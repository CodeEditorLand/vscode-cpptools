...
export function GetOSName(
	processPlatform: string | undefined,
): string | undefined {
...
export class PlatformInformation {
	...
	public static async GetPlatformInformation(): Promise<PlatformInformation> {
		...
		switch (platform) {
			...
			default:
				throw new Error(
					localize("unknown.os.platform", "Unknown OS platform"),
				);
		}
	...
	private static GetDarwinVersion(): Promise<string> {
		const DARWIN_SYSTEM_VERSION_PLIST: string =
		...
		if (fs.existsSync(DARWIN_SYSTEM_VERSION_PLIST)) {
			const systemVersionPListBuffer: Buffer = fs.readFileSync(
				DARWIN_SYSTEM_VERSION_PLIST,
			);

			const systemVersionData: any = plist.parse(
				systemVersionPListBuffer.toString(),
			);
			...
			if (systemVersionData) {
				...
				errorMessage = localize(
					"missing.plist.productversion",
					"Could not get ProduceVersion from SystemVersion.plist",
				);
			}
		} else {
			errorMessage = localize(
				"missing.darwin.systemversion.file",
				"Failed to find SystemVersion.plist in {0}.",
				DARWIN_SYSTEM_VERSION_PLIST,
			);
		}
