	...
	if (flattenedPackageJson === undefined) {
		...
		for (const subheading of packageJson.contributes.configuration) {
			for (const setting in subheading.properties) {
				flattenedPackageJson.set(
					setting,
					subheading.properties[setting],
				);
			}
		}
	}
...
export function isEditorFileCpp(file: string): boolean {
	const editor: vscode.TextEditor | undefined =
		vscode.window.visibleTextEditors.find(
			(e) => e.document.uri.toString() === file,
		);
	if (!editor) {
		return false;
	}
...
export function isCppOrRelated(document: vscode.TextDocument): boolean {
	return (
		isCpp(document) ||
		isCppPropertiesJson(document) ||
		(document.uri.scheme === "output" &&
			document.uri.fsPath.startsWith(
				"extension-output-ms-vscode.cpptools",
			)) ||
		(isWorkspaceCpp &&
...
export function displayExtensionNotReadyPrompt(): void {
	if (!isExtensionNotReadyPromptDisplayed) {
		...
		void getOutputChannelLogger()
			.showInformationMessage(extensionNotReadyString)
			.then(
				() => {
...
const intelliSenseProgressStr: string =
	"CPP." + packageJson.version + ".IntelliSenseProgress";

export function getProgress(): number {
	return extensionContext
		? extensionContext.globalState.get<number>(installProgressStr, -1)
		: -1;
}

export function getIntelliSenseProgress(): number {
	return extensionContext
		? extensionContext.globalState.get<number>(intelliSenseProgressStr, -1)
		: -1;
}
...
export function getProgressInstallSuccess(): number {
	return progressInstallSuccess;
} // Download/install was successful (i.e. not blocked by component acquisition).
export function getProgressExecutableStarted(): number {
	return progressExecutableStarted;
} // The extension was activated and starting the executable was attempted.
export function getProgressExecutableSuccess(): number {
	return progressExecutableSuccess;
} // Starting the exe was successful (i.e. not blocked by 32-bit or glibc < 2.18 on Linux)
export function getProgressParseRootSuccess(): number {
	return progressParseRootSuccess;
} // Parse root was successful (i.e. not blocked by processing taking too long).
export function getProgressIntelliSenseNoSquiggles(): number {
	return progressIntelliSenseNoSquiggles;
} // IntelliSense was successful and the user got no squiggles.
...
export function resolveCachePath(
	...
	additionalEnvironment: Record<string, string | string[]>,
): string {
				...
				switch (varType) {
					case "env": {
						if (additionalEnvironment) {
							const v: string | string[] | undefined =
								additionalEnvironment[name];
							if (isString(v)) {
								newValue = v;
...
export function resolveVariablesArray(
	...
	if (variables) {
		variables.forEach((variable) => {
			...
			const variableResolved: string = resolveVariables(
				variable,
...
/** Test whether a file exists */
export async function checkExecutableWithoutExtensionExists(
	filePath: string,
): Promise<boolean> {
	...
	if (os.platform() === "win32") {
		if (filePath.length > 4) {
			const possibleExtension: string = filePath
				...
				.toLowerCase();
			if (
				possibleExtension === ".exe" ||
				possibleExtension === ".cmd" ||
				possibleExtension === ".bat"
			) {
				return false;
			}
		}
...
export function checkExecutableWithoutExtensionExistsSync(
	filePath: string,
): boolean {
...
export function checkPathExistsSync(
	path: string,
	relativePath: string,
	_isWindows: boolean,
	isCompilerPath: boolean,
): { pathExists: boolean; path: string } {
	let pathExists: boolean = true;
	const existsWithExeAdded: (path: string) => boolean = (path: string) =>
		isCompilerPath && _isWindows && fs.existsSync(path + ".exe");
...
export async function spawnChildProcess(
	...
	const exitCode: number | NodeJS.Signals | undefined =
		programOutput.exitCode;
	if (programOutput.exitCode) {
		return {
			succeeded: false,
			exitCode,
			outputError: programOutput.stderr,
			output:
...
async function spawnChildProcessImpl(
	program: string,
	args: string[],
	continueOn?: string,
	skipLogging?: boolean,
	...
	const settings: vscode.WorkspaceConfiguration =
		vscode.workspace.getConfiguration("C_Cpp", null);
	const loggingLevel: number =
		skipLogging === undefined || !skipLogging
			? getNumericLoggingLevel(settings.get<string>("loggingLevel"))
			: 0;

	let proc: child_process.ChildProcess;
	if (await isExecutable(program)) {
		proc = child_process.spawn(
			`.${isWindows ? "\\" : "/"}${path.basename(program)}`,
			args,
			{ shell: true, cwd: path.dirname(program) },
		);
	} else {
		proc = child_process.spawn(program, args, { shell: true });
	}

	const cancellationTokenListener: vscode.Disposable | undefined =
 ...
 */
export function pathAccessible(
	filePath: string,
	permission: number = fs.constants.F_OK,
): Promise<boolean> {
	if (!filePath) {
...
export async function promptReloadWindow(message: string): Promise<void> {
	...
	const value: string | undefined =
		await vscode.window.showInformationMessage(message, reload);
	if (value === reload) {
		return vscode.commands.executeCommand("workbench.action.reloadWindow");
	}
}

export function createTempFileWithPostfix(
	postfix: string,
...
function extractArgs(argsString: string): string[] {
	...
	if (os.platform() === "win32") {
		...
		try {
			const wordexpResult: any = child_process.execFileSync(
				getExtensionFilePath("bin/cpptools-wordexp"),
				[argsString],
				{ shell: false },
			);
...
export function isCl(compilerPath: string): boolean {
	...
	return (
		compilerPathLowercase === "cl" ||
		compilerPathLowercase === "cl.exe" ||
		compilerPathLowercase.endsWith("\\cl.exe") ||
		compilerPathLowercase.endsWith("/cl.exe") ||
		compilerPathLowercase.endsWith("\\cl") ||
		compilerPathLowercase.endsWith("/cl")
	);
}
...
export function extractCompilerPathAndArgs(
	...
	if (compilerPath) {
		...
		if (
			isCl(compilerPath) ||
			checkExecutableWithoutExtensionExistsSync(compilerPath)
		) {
			...
			(os.platform() !== "win32" && compilerPath.startsWith("'"))
		) {
			...
			if (useLegacyBehavior) {
				compilerArgsFromCommandLineInPath =
					legacyExtractArgs(compilerPath);
				if (compilerArgsFromCommandLineInPath.length > 0) {
					...
					if (compilerPath) {
						...
						const tempCompilerPath: string[] | undefined =
							extractArgs(compilerPath);
			...
			if (spaceStart !== -1) {
				...
				const potentialArgs: string[] = useLegacyBehavior
					? legacyExtractArgs(compilerPath)
					: extractArgs(compilerPath);
				let potentialCompilerPath: string | undefined =
					potentialArgs.shift();
				if (useLegacyBehavior) {
					if (potentialCompilerPath) {
						const tempCompilerPath: string[] | undefined =
							extractArgs(potentialCompilerPath);
						if (tempCompilerPath && compilerPath.length > 0) {
				...
				if (potentialCompilerPath) {
					if (
						...
						checkExecutableWithoutExtensionExistsSync(
							potentialCompilerPath,
						)
					) {
...
export function isValidIdentifier(candidate: string): boolean {
	...
	for (
		...
		const disallowedCharacters: number[] =
			disallowedFirstCharacterIdentifierUnicodeRanges[i];
		if (
			decoded[0] >= disallowedCharacters[0] &&
			decoded[0] <= disallowedCharacters[1]
		) {
			return false;
		}
	}
	...
	for (let position: number = 0; position < decoded.length; position++) {
		...
		for (
			...
			const allowedCharacters: number[] =
				allowedIdentifierUnicodeRanges[i];
			if (
				decoded[position] >= allowedCharacters[0] &&
				decoded[position] <= allowedCharacters[1]
			) {
				found = true;
				break;
			}
...
export function getCacheStoragePath(): string {
	...
	return pathEnvironmentVariable
		? path.join(pathEnvironmentVariable, defaultCachePath)
		: "";
}

function getUniqueWorkspaceNameHelper(
	workspaceFolder: vscode.WorkspaceFolder,
	addSubfolder: boolean,
): string {
	const workspaceFolderName: string = workspaceFolder
	...
	for (let i: number = 0; i < workspaceFolder.index; ++i) {
		if (
			...
			vscode.workspace.workspaceFolders[i].name === workspaceFolderName
		) {
			return addSubfolder
				? path.join(workspaceFolderName, String(workspaceFolder.index)) // Use the index as a subfolder.
...
export function getUniqueWorkspaceName(
	workspaceFolder: vscode.WorkspaceFolder,
): string {
	return getUniqueWorkspaceNameHelper(workspaceFolder, false);
}

export function getUniqueWorkspaceStorageName(
	workspaceFolder: vscode.WorkspaceFolder,
): string {
...
export function sequentialResolve<T>(
	items: T[],
	promiseBuilder: (item: T) => Promise<void>,
): Promise<void> {
...
export function findPowerShell(): string | undefined {
	...
	for (const name of names) {
		const candidates: string[] = dirs.reduce<string[]>(
			(paths, dir) => [
				...paths,
				...exts.map((ext) => path.join(dir, name + ext)),
			],
			[],
		);
...
export function isVsCodeInsiders(): boolean {
	return (
		extensionPath.includes(".vscode-insiders") ||
...
export function getNumericLoggingLevel(
	...
	for (let currentIndex = 0; currentIndex < ranges.length; currentIndex++) {
		...
		while (nextIndex < ranges.length) {
			...
			if (
				...
				(nextRange.start.line === currentRange.end.line &&
					nextRange.start.character > currentRange.end.character)
			) {
				break;
			}
