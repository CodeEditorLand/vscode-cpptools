...
export function getOutputChannel(): vscode.OutputChannel {
	if (!outputChannel) {
		...
		const loggingLevel: string | undefined =
			settings.get<string>("loggingLevel");
...
export function getCrashCallStacksChannel(): vscode.OutputChannel {
	if (!crashCallStacksChannel) {
		crashCallStacksChannel = vscode.window.createOutputChannel(
			localize(
...
export function getSshChannel(): vscode.OutputChannel {
	if (!sshChannel) {
		sshChannel = vscode.window.createOutputChannel(
...
export function logDebugProtocol(output: DebugProtocolParams): void {
	if (!debugChannel) {
		debugChannel = vscode.window.createOutputChannel(
