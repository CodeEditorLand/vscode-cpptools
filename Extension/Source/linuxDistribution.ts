...
export class LinuxDistribution {
	...
	public static GetDistroInformation(): Promise<LinuxDistribution> {
		...
		const linuxDistro: Promise<LinuxDistribution> =
			LinuxDistribution.getDistroInformationFromFile("/etc/os-release")
				.catch(() =>
					LinuxDistribution.getDistroInformationFromFile(
						"/usr/lib/os-release",
					),
				)
				.catch(() =>
					Promise.resolve(
						new LinuxDistribution("unknown", "unknown"),
					),
